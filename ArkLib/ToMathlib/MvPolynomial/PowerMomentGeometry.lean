...
For positive `D`, the power-moment map is surjective and the quotient by its moment ideal is
isomorphic to the source polynomial ring. Weighted filtrations bound the Hilbert function and affine
degree of the kernel. The point maps identify the lifted zero locus with the source coordinates,
and the incidence theorem transfers finite agreement bounds through this parametrization.
