...
`Finset.sumRangeFrom f start length` sums `f` over the half-open natural interval beginning at
`start`. The splitting and substitution theorems combine adjacent chunks in any additive
commutative monoid.

## Main statements

* `Finset.sumRangeFrom`
* `Finset.sumRangeFrom_add`
* `Finset.sumRangeFrom_four`, `Finset.sumRangeFrom_four_eq`
* `Finset.sumRangeFrom_two_eq`
