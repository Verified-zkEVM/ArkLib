...
preserves the natural degree of the affine Hilbert polynomial. In particular, if
`MvPolynomial σ k ⧸ I` is generated by `m` elements as a `k`-algebra, then `natDegree P_I ≤ m`.
...
* `MvPolynomial.natDegree_affineHilbertPolynomial_le_card_of_surjective`,
  `MvPolynomial.natDegree_affineHilbertPolynomial_le_card_of_adjoin_eq_top`: a quotient generated
  by `m` elements has affine Hilbert polynomial of natural degree at most `m`.
-/
...
/-- A surjective algebra map from a polynomial ring in `τ` onto `MvPolynomial σ k ⧸ I` bounds the
natural degree of the affine Hilbert polynomial of `I` by `Nat.card τ`: the quotient is then a
quotient of the coordinate ring of the kernel, whose polynomial has natural degree at most
`Nat.card τ`. -/
theorem natDegree_affineHilbertPolynomial_le_card_of_surjective [Finite σ] [Finite τ]
    (g : MvPolynomial τ k →ₐ[k] (MvPolynomial σ k ⧸ I)) (hg : Function.Surjective g) :
...
/-- If finitely many elements `x i`, indexed by `τ`, generate `MvPolynomial σ k ⧸ I` as a
`k`-algebra, the natural degree of the affine Hilbert polynomial of `I` is at most `Nat.card τ`.
This is the generator form of `natDegree_affineHilbertPolynomial_le_card_of_surjective`. -/
theorem natDegree_affineHilbertPolynomial_le_card_of_adjoin_eq_top [Finite σ] [Finite τ]
    (x : τ → MvPolynomial σ k ⧸ I) (hx : Algebra.adjoin k (Set.range x) = ⊤) :
    (affineHilbertPolynomial I).natDegree ≤ Nat.card τ := by
  refine natDegree_affineHilbertPolynomial_le_card_of_surjective (aeval x) ?_
