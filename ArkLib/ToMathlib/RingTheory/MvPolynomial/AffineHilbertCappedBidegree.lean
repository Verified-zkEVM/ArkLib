...
For `σ = Fin 2`, `i = 1`, positive bounds and `c ≤ b`, the count of exponents on `Fin 2` with
degree at most `B` and second coordinate at most `C ≤ B` is `(C + 1) * (2 * B + 2 - C) / 2`. The
difference of counts is then eventually a quadratic polynomial in `N` with leading coefficient
`(h * c * (2 * b - c) + 2 * a * (j * c + r * (b - c))) / 2`, while the pulled-back hypersurface has
affine Hilbert polynomial of natural degree `2`. This bounds its affine degree by
`h * c * (2 * b - c) + 2 * a * (j * c + r * (b - c))`, and the same bound holds for the total affine
degree of its minimal primes. For `c = b` the cap is no condition and the bound is that of the
bidegree hypersurface.
...
`(a * N - h + 1) * #{e | e.degree ≤ b * N - j ∧ e i ≤ c * N - r}` is at most
`(a * N + 1) * #{e | e.degree ≤ b * N ∧ e i ≤ c * N}`. -/
theorem affineHilbertFunction_comap_cappedBidegree_span_singleton_add_le [Finite σ] {i : σ}
    ...
    affineHilbertFunction
        ...
        (a * N - h + 1) * {e : σ →₀ ℕ | e.degree ≤ b * N - j ∧ e i ≤ c * N - r}.ncard ≤
      (a * N + 1) * {e : σ →₀ ℕ | e.degree ≤ b * N ∧ e i ≤ c * N}.ncard := by
...
/-- For `C ≤ B`, twice the number of exponents on `Fin 2` of degree at most `B` with second
coordinate at most `C`, cast to `ℚ`. -/
private theorem two_mul_ncard_cast (B C : ℕ) (hCB : C ≤ B) :
    (2 : ℚ) * {e : Fin 2 →₀ ℕ | e.degree ≤ B ∧ e 1 ≤ C}.ncard =
      (C + 1) * (2 * B + 2 - C) := by
...
`MvPolynomial (Option (Fin 2)) k` with cap on `some 1`. For positive `a`, `b` and `c` with `c ≤ b`,
the pullback of the hypersurface `span {g}` along the monomial map of
`cappedBidegreeExponents (Fin 2) 1 a b c` has affine degree at most
...
leading coefficient `(h * c * (2 * b - c) + 2 * a * (j * c + r * (b - c))) / 2`. For `c = b` this
is the bound for the bidegree hypersurface. -/
theorem affineDegree_comap_cappedBidegree_span_singleton_le {a b c h j r : ℕ} (ha : 0 < a)
    (hb : 0 < b) (hc : 0 < c) (hcb : c ≤ b) {g : MvPolynomial (Option (Fin 2)) k} (hg0 : g ≠ 0)
...
`cappedBidegreeExponents (Fin 2) 1 a b c` sum to at most
`h * c * (2 * b - c) + 2 * a * (j * c + r * (b - c))`. -/
theorem sum_affineDegree_minimalPrimes_comap_cappedBidegree_span_singleton_le {a b c h j r : ℕ}
  ...
  (sum_affineDegree_minimalPrimes_comap_span_singleton_le_of_surjective _
    ...
    (affineDegree_comap_cappedBidegree_span_singleton_le ha hb hc hcb hg0 hg)
