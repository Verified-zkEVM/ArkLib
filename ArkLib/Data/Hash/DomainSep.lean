...
/-- Create a [`ProverState`] instance from the domain separator.
...
```
-/
def toProverState (ds : DomainSeparator U H) : Unit :=
  -- TODO: define ProverState
  sorry

/-- Create a [`VerifierState`] instance from the domain separator and the protocol transcript (a
  `ByteArray`).
...
```
-/
def toVerifierState (ds : DomainSeparator U H) (transcript : ByteArray) : Unit :=
  -- TODO: define VerifierState
  sorry
