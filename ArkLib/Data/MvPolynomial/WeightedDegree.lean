...
`R`-submodule and proves that the support formulation agrees with Mathlib's
`weightedTotalDegree`. In particular, the zero polynomial belongs to every bound, including bound
zero.
