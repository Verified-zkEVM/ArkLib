...
Within the `Fin` namespace, we also provide convenience functions:
- `take' m h v` for `take m h v`
- `drop' m h v` for `drop m h v`
...
-/
syntax:max (name := finTakeSlice) term "[" ":" term "]" : term
macro_rules (kind := finTakeSlice)
  | `($v[: $m]) => `(Fin.take $m (by omega) $v)
...
macro_rules (kind := finTakeSliceWithProof)
  | `($v[: $m]' $h) => `(Fin.take $m $h $v)

/-- Notation `v[m:]` for dropping the first `m` elements of a Fin tuple -/
syntax:max (name := finDropSlice) term "[" term ":" "]" : term
macro_rules (kind := finDropSlice)
  | `($v[$m :]) => `(Fin.drop $m (by omega) $v)

/-- Notation `v[m:]'h` for dropping the first `m` elements with explicit proof -/
syntax (name := finDropSliceWithProof) term "[" term ":" "]'" term:max : term
macro_rules (kind := finDropSliceWithProof)
  | `($v[$m :]' $h) => `(Fin.drop $m $h $v)

/-- Notation `v[m₁:m₂]` for taking elements from index `m₁` to `m₂ - 1`
(e.g., `v[1:3] = v[1] ++ v[2]`), with range proofs automatically synthesized -/
syntax:max (name := finSliceRange) term "[" term ":" term "]" : term
macro_rules (kind := finSliceRange)
  | `($v[$m₁ : $m₂]) => `(Fin.drop $m₁ (by omega) (Fin.take $m₂ (by omega) $v))

/-- Notation `v[m₁:m₂]'⟨h₁, h₂⟩` for taking elements from index `m₁` to `m₂ - 1`
(e.g., `v[1:3] = v[1] ++ v[2]`), with explicit proofs -/
syntax (name := finSliceRangeWithProof)
  term "[" term ":" term "]'⟨" term:max "," term:max "⟩" : term
macro_rules (kind := finSliceRangeWithProof)
  | `($v[$m₁ : $m₂]'⟨$h₁, $h₂⟩) => `(Fin.drop $m₁ $h₁ (Fin.take $m₂ $h₂ $v))
...
example (h : 3 ≤ n) : v[:3] = Fin.take 3 h v := rfl
example (h : 2 ≤ n) : v[2:] = Fin.drop 2 h v := rfl
