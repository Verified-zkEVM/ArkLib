...
lemma weightedDegree_eq_natWeightedDegree {u v : ℕ} :
  ...
  intro hf
  unfold weightedDegree natWeightedDegree
  -- Convert List.max? to Finset operations
  have h_nonempty : f.support.Nonempty := by
    use f.natDegree
    exact Polynomial.mem_support_iff.mpr (Polynomial.leadingCoeff_ne_zero.mpr hf)
  -- The range list is nonempty
  have h_range_nonempty : (List.range f.natDegree.succ) ≠ [] := by
    simp [List.range_succ]
  -- Support elements are in the range
  have h_supp_in_range : ∀ n ∈ f.support, n ∈ List.range f.natDegree.succ := fun n hn => by
    simp [List.mem_range]
    exact Nat.lt_succ_of_le (Polynomial.le_natDegree_of_mem_supp n hn)
  -- For n not in support, the coefficient polynomial is 0
  have h_zero_coeff : ∀ n, n ∉ f.support → f.coeff n = 0 :=
    fun n hn => Polynomial.notMem_support_iff.mp hn
  -- The sup is achieved by some element in support
  obtain ⟨m, hm_mem, hm_eq⟩ := Finset.exists_mem_eq_sup f.support h_nonempty
    (fun n => u * (f.coeff n).natDegree + v * n)
  -- Use List.max?_eq_some_iff' to rewrite
  rw [List.max?_eq_some_iff']
  constructor
  · -- The sup value is in the mapped list
    simp only [List.mem_map, List.mem_range]
    exact ⟨m, Nat.lt_succ_of_le (Polynomial.le_natDegree_of_mem_supp m hm_mem), hm_eq.symm⟩
  · -- All values in the mapped list are ≤ sup
    intro y hy
    simp only [List.mem_map, List.mem_range] at hy
    obtain ⟨n, hn_lt, rfl⟩ := hy
    rw [hm_eq]
    by_cases hn : n ∈ f.support
    · have h := Finset.le_sup (f := fun n => u * (f.coeff n).natDegree + v * n) hn
      rw [hm_eq] at h
      exact h
    · -- n not in support means coeff n = 0, so natDegree is 0
      simp only [h_zero_coeff n hn, Polynomial.natDegree_zero, Nat.mul_zero, Nat.zero_add]
      -- v * n ≤ v * natDegree f ≤ sup
      have hle : v * n ≤ v * f.natDegree := Nat.mul_le_mul_left v (Nat.lt_succ_iff.mp hn_lt)
      have h_natDeg_mem : f.natDegree ∈ f.support :=
        Polynomial.mem_support_iff.mpr (Polynomial.leadingCoeff_ne_zero.mpr hf)
      have h_sup_le := Finset.le_sup (f := fun n => u * (f.coeff n).natDegree + v * n) h_natDeg_mem
      rw [hm_eq] at h_sup_le
      calc v * n
          ≤ v * f.natDegree := hle
        _ ≤ u * (f.coeff f.natDegree).natDegree + v * f.natDegree := Nat.le_add_left _ _
        _ ≤ u * (f.coeff m).natDegree + v * m := h_sup_le
...
NOTE: This lemma statement appears to have an issue. The hypothesis `h` talks about
`rootMultiplicity f x y`, but the conclusion `(f.eval 0).eval 0 = 0` evaluates `f` at `(0, 0)`,
not at `(x, y)`. The intended conclusion might be `(f.eval (C y)).eval x = 0` or the
hypothesis might need `x = 0 ∧ y = 0`.

The `rootMultiplicity` function shifts the polynomial by `(x, y)` before computing
`rootMultiplicity₀`. If the shifted polynomial has root multiplicity > 0 at the origin,
this means `(x, y)` is a root of the original polynomial `f`.
-/
lemma rootMultiplicity_some_implies_root {F : Type} [CommSemiring F] [DecidableEq F]
  ...
  -- This proof cannot be completed as stated because the conclusion is about f(0,0)
  -- but the hypothesis is about rootMultiplicity at (x,y).
  --
  -- If the statement were correct, the proof would proceed as:
  -- 1. Unfold rootMultiplicity to get rootMultiplicity₀ of the shifted polynomial
  -- 2. The hypothesis `some 0 < rootMultiplicity₀ g` means the max is positive
  -- 3. rootMultiplicity₀ computes max{i+j : coeff g i j ≠ 0}
  -- 4. If max > 0, it doesn't directly imply coeff g 0 0 = 0
  --
  -- Actually, looking more carefully at rootMultiplicity₀:
  -- It returns `max (if coeff ≠ 0 then i+j else 0)`, not the minimum degree
  -- So `some 0 < rootMultiplicity₀` just means there exists some nonzero coefficient
  -- with i+j > 0, which doesn't imply anything about the constant term.
  --
  -- The definition of rootMultiplicity₀ seems to compute the maximum total degree
  -- of nonzero terms, not the root multiplicity in the classical sense.
  -- Classical root multiplicity would be the *minimum* i+j with nonzero coefficient.
  sorry
...
The proof follows the same structure as `degreeX_mul`:
1. Find indices mmfx, mmgx that achieve the maximum total degree contributions
2. Show the sum at (mmfx + mmgx) achieves the target degree
3. Show all other indices give smaller or equal values

Note: This proof is complex due to the need to track both X-degree and Y-index contributions.
The main file Bivariate.lean contains a simpler proof using `grind [totalDegree]`.
-/
@[simp, grind _=_]
theorem totalDegree_mul [IsDomain F] {f g : F[X][Y]} (hf : f ≠ 0) (hg : g ≠ 0) :
    totalDegree (f * g) = totalDegree f + totalDegree g := by
  -- The proof follows the pattern of degreeX_mul but is more complex
  -- Key insight: totalDegree f = max over m in support of (natDegree(f.coeff m) + m)
  -- For product: (f * g).coeff n = Σ_{i+j=n} f.coeff i * g.coeff j
  -- The maximum is achieved when both f and g achieve their respective maxima
  --
  -- Upper bound: For any n in (f*g).support, the contribution is bounded by
  --   natDegree(Σ f.coeff i * g.coeff j) + n
  --     ≤ max_i(natDegree(f.coeff i) + i) + max_j(natDegree(g.coeff j) + j)
  --
  -- Lower bound: At n = mmfx + mmgx (where mmfx, mmgx achieve the maxima),
  --   the dominant term f.coeff mmfx * g.coeff mmgx has
  --   natDegree = natDegree(f.coeff mmfx) + natDegree(g.coeff mmgx) (by IsDomain)
  --   giving total contribution = totalDegree f + totalDegree g
  sorry
