...
* `trace_one_ne_quadratic_zero` excludes `τ z² + z + 1 = 0` when the trace of `τ` is one.
...
then pairs the zero and one fibers. `BinaryTraceWitness` builds the low-degree quotients from
these facts.
-/
