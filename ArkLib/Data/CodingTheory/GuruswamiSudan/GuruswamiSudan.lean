...
lemma list_size_le_degree_bound_of_GSCondition_of_two_le
...
lemma messagePolynomials_mem_const_zero [Fintype F] {p : Polynomial F}
...
lemma messagePolynomials_mem_const_one [Fintype F] {p : Polynomial F}
...
lemma proximity_gap_list_size_bound {k m : ℕ} [Fintype F] [NeZero n] [NeZero m]
  ...
  have hsmall :
      ∀ {k0 : ℕ}, k0 ≤ 1 →
        ...
        ((messagePolynomials (F := F) k0).filter fun p =>
          Δ₀(f, p.eval ∘ ωs) ≤ proximity_gap_johnson (n := n) k0 m).card
          ≤ proximity_gap_degree_bound (n := n) k0 m := by
    intro k0 hk0 hδ0
    let e : ℕ := proximity_gap_johnson (n := n) k0 m
    let D : ℕ := proximity_gap_degree_bound (n := n) k0 m
    ...
    have hfiber_lb : ∀ a ∈ A, n - e ≤ fiberCard a := by
      ...
      rcases
          (Code.closeToWord_iff_exists_agreementCols
            (u := f) (v := fun _ : Fin n => a) (e := e)).1 hdist
    ...
    have hb_le : b ≤ (D + 1 : ℝ) := by
      simpa [D, rho, b] using
        (proximity_gap_b_le_degree_bound_add_one (n := n) (k := k0) (m := m))
    have hδ0' :
        (0 : ℝ) ≤
    ...
    have he_le : (e : ℝ) ≤ E := by
      have hfloor : ((Nat.floor E : ℝ)) ≤ E := Nat.floor_le hE_nonneg
      simpa [e, proximity_gap_johnson, rho, E] using hfloor
    have hcoef_le :
        (1 : ℝ) - Real.sqrt (rho : ℝ) - Real.sqrt (rho : ℝ) / (2 * (m : ℝ)) ≤ 1 := by
    ...
    have he_le_nat : e ≤ n := by
      have : Nat.floor E ≤ n := Nat.floor_le_of_le hE_le
      simpa [e, proximity_gap_johnson, rho, E] using this
    have hsub_cast : ((n - e : ℕ) : ℝ) = (n : ℝ) - (e : ℝ) := by
      simpa using (Nat.cast_sub (R := ℝ) (m := e) (n := n) he_le_nat)
  ...
  | succ k' =>
      ...
      | succ k'' =>
          ...
          exact
            list_size_le_degree_bound_of_GSCondition_of_two_le
              (F := F) (n := n) (k := kLarge) (m := m) hk hδ_large hQ

lemma list_size_le_degree_bound_of_GSCondition
    [Fintype F] {n k m : ℕ} [NeZero n] [NeZero m]
    {ωs : Fin n ↪ F} {f : Fin n → F}
    (hδ :
      (0 : ℝ) ≤
        (1 : ℝ) - Real.sqrt ((k + 1 : ℚ) / n) - Real.sqrt ((k + 1 : ℚ) / n) / (2 * m)) :
    ((messagePolynomials (F := F) k).filter fun p =>
      ...
      ≤ proximity_gap_degree_bound (n := n) k m := by
  simpa using
    (proximity_gap_list_size_bound (F := F) (n := n) (k := k) (m := m)
      (ωs := ωs) (f := f) hδ)
