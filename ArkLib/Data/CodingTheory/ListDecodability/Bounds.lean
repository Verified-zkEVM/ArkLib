...
theorem rs_lambda_large_prime (α β : ℝ) (_hα_pos : 0 < α) (_hα_lt : α < 1) (_hβ_pos : 0 < β) (_hβ_lt : β < 1)
    (_hαβ_le_one : α + β ≤ 1) :
    ∃ (c : ℝ) (_ : 0 < c) (p₀ : ℕ),
      ∀ p : ℕ, Nat.Prime p → p₀ ≤ p →
        ∀ {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  ...
  have hEbound : ∀ᶠ p : ℕ in Filter.atTop, A * (p : ℝ) ^ α ≤ (p : ℝ) := by
    ...
    calc
      ...
      _ = (p : ℝ) := by
        ...
        convert Real.rpow_one (p : ℝ) using 2 <;> ring
  ...
  simpa only [cnt, x] using hw -- external admit: [GHSZ02, Corollary 20].
