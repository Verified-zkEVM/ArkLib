...
noncomputable def entropyStirlingSeq (n : ℕ) : ℝ :=
  (Nat.factorial n : ℝ) /
    (Real.sqrt (2 * (n : ℝ)) * ((n : ℝ) / Real.exp 1) ^ n)

def entropyStirlingSeq_log_formula (n : ℕ) :
    Real.log (entropyStirlingSeq n) =
      Real.log (Nat.factorial n : ℝ) -
        (1 : ℝ) / 2 * Real.log (2 * (n : ℝ)) -
          (n : ℝ) * Real.log ((n : ℝ) / Real.exp 1) := by
  cases n
  · simp [entropyStirlingSeq]
  · rw [entropyStirlingSeq, Real.log_div, Real.log_mul,
      Real.sqrt_eq_rpow, Real.log_rpow, Real.log_pow, tsub_tsub] <;>
      positivity

def entropyStirlingSeq_one_sq :
    entropyStirlingSeq 1 * entropyStirlingSeq 1 =
      2 * entropyStirlingSeq 2 := by
  unfold entropyStirlingSeq
...
def entropyStirlingSeq_pos (n : ℕ) (hn : 0 < n) :
    0 < entropyStirlingSeq n := by
  unfold entropyStirlingSeq
  positivity

set_option maxHeartbeats 1000000 in
def entropyStirlingSeq_sdiff_hasSum (m : ℕ) :
    HasSum
      (fun k : ℕ =>
        (1 : ℝ) / (2 * ((k + 1 : ℕ) : ℝ) + 1) *
          (((1 : ℝ) / (2 * ((m + 1 : ℕ) : ℝ) + 1)) ^ (2 : ℕ)) ^
            (k + 1))
      (Real.log (entropyStirlingSeq (m + 1)) -
        Real.log (entropyStirlingSeq (m + 2))) := by
  let f (k : ℕ) :=
    (1 : ℝ) / (2 * k + 1) *
      (((1 : ℝ) / (2 * ((m + 1 : ℕ) : ℝ) + 1)) ^ (2 : ℕ)) ^ k
  change HasSum (fun k => f (k + 1)) _
  rw [hasSum_nat_add_iff]
  convert!
    (Real.hasSum_log_one_add_inv m.cast_add_one_pos).mul_left
      (((m + 1 : ℕ) : ℝ) + 1 / 2) using 1
  · ext k
    dsimp only [f]
    rw [← pow_mul, pow_add]
  ...
  · have h (x : ℝ) (hx : x ≠ 0) : 1 + x⁻¹ = (x + 1) / x := by
      field_simp
    simp (disch := positivity) only [entropyStirlingSeq_log_formula,
      Real.log_div, Real.log_mul, Real.log_exp, Nat.factorial_succ,
      Nat.cast_mul, Nat.cast_succ, Finset.range_one, Finset.sum_singleton, h]
    dsimp [f]
    ring

def entropy_choose_cast_factorial (d m : ℕ) :
    (((d + m).choose d : ℕ) : ℝ) =
      (Nat.factorial (d + m) : ℝ) /
        ((Nat.factorial d : ℝ) * (Nat.factorial m : ℝ)) := by
  simpa only [Nat.add_sub_cancel_left] using
    (Nat.cast_choose (K := ℝ) (show d ≤ d + m by omega))

set_option maxHeartbeats 1000000 in
def binomial_mean_mass_eq_entropyStirlingSeq
    (d m : ℕ) (hd : 0 < d) (hm : 0 < m) :
    (((d + m).choose d : ℕ) : ℝ) *
        ((d : ℝ) / ((d + m : ℕ) : ℝ)) ^ d *
        ((m : ℝ) / ((d + m : ℕ) : ℝ)) ^ m =
      entropyStirlingSeq (d + m) /
          (entropyStirlingSeq d * entropyStirlingSeq m) *
        (Real.sqrt (2 * ((d + m : ℕ) : ℝ)) /
          (Real.sqrt (2 * (d : ℝ)) * Real.sqrt (2 * (m : ℝ)))) := by
  rw [entropy_choose_cast_factorial]
  unfold entropyStirlingSeq
...
def entropy_radius_integer_bounds (n d : ℕ) (δ : ℝ) (hn : 0 < n)
...
def entropy_sqrt_factor_identity
    (d m : ℕ) (hd : 0 < d) (hm : 0 < m) :
    (1 : ℝ) / 2 *
        (Real.sqrt (2 * ((d + m : ℕ) : ℝ)) /
...
def log_entropyStirlingSeq_sdiff_antitone :
    Antitone (fun m : ℕ =>
      Real.log (entropyStirlingSeq (m + 1)) -
        Real.log (entropyStirlingSeq (m + 2))) := by
  intro a b hab
  apply hasSum_le (fun k => ?_)
    (entropyStirlingSeq_sdiff_hasSum b)
    (entropyStirlingSeq_sdiff_hasSum a)
  have habR : (a : ℝ) ≤ b := by exact_mod_cast hab
  gcongr

def entropyStirlingSeq_succ_ratio_mono
    (a b : ℕ) (ha : 0 < a) (hab : a ≤ b) :
    entropyStirlingSeq (a + 1) / entropyStirlingSeq a ≤
      entropyStirlingSeq (b + 1) / entropyStirlingSeq b := by
  have hb : 0 < b := lt_of_lt_of_le ha hab
  have hab' : a - 1 ≤ b - 1 := Nat.sub_le_sub_right hab 1
  have hdiff := log_entropyStirlingSeq_sdiff_antitone hab'
  dsimp only at hdiff
  ...
  have hleft : 0 < entropyStirlingSeq (a + 1) / entropyStirlingSeq a :=
    div_pos (entropyStirlingSeq_pos (a + 1) (by omega))
      (entropyStirlingSeq_pos a ha)
  have hright : 0 < entropyStirlingSeq (b + 1) / entropyStirlingSeq b :=
    div_pos (entropyStirlingSeq_pos (b + 1) (by omega))
      (entropyStirlingSeq_pos b hb)
  apply (Real.log_le_log_iff hleft hright).mp
  rw [Real.log_div (entropyStirlingSeq_pos (a + 1) (by omega)).ne'
      (entropyStirlingSeq_pos a ha).ne',
    Real.log_div (entropyStirlingSeq_pos (b + 1) (by omega)).ne'
      (entropyStirlingSeq_pos b hb).ne']
  linarith

def entropyStirlingSeq_mul_le_two_mul_add
    (d m : ℕ) (hd : 0 < d) (hm : 0 < m) :
    entropyStirlingSeq d * entropyStirlingSeq m ≤
      2 * entropyStirlingSeq (d + m) := by
  obtain ⟨r, rfl⟩ := Nat.exists_eq_succ_of_ne_zero (Nat.ne_of_gt hm)
  induction r with
  | zero =>
      have hratio := entropyStirlingSeq_succ_ratio_mono 1 d
        Nat.zero_lt_one (by omega)
      have hS1 := entropyStirlingSeq_pos 1 Nat.zero_lt_one
      have hSd := entropyStirlingSeq_pos d hd
      have hbase :
          entropyStirlingSeq d * entropyStirlingSeq 1 ≤
            2 * entropyStirlingSeq (d + 1) := by
        calc
          entropyStirlingSeq d * entropyStirlingSeq 1 =
              2 * entropyStirlingSeq d *
                (entropyStirlingSeq 2 / entropyStirlingSeq 1) := by
                  field_simp [hS1.ne']
                  nlinarith [entropyStirlingSeq_one_sq]
          _ ≤ 2 * entropyStirlingSeq d *
                (entropyStirlingSeq (d + 1) / entropyStirlingSeq d) := by
                  gcongr
          _ = 2 * entropyStirlingSeq (d + 1) := by
                  field_simp [hSd.ne']
      simpa using hbase
  | succ r ih =>
      let t : ℕ := r + 1
      ...
      have ih' :
          entropyStirlingSeq d * entropyStirlingSeq t ≤
            2 * entropyStirlingSeq (d + t) := by
        simpa [t, Nat.succ_eq_add_one] using ih
      have hratio := entropyStirlingSeq_succ_ratio_mono t (d + t) ht (by omega)
      have hSt := entropyStirlingSeq_pos t ht
      have hSt1 := entropyStirlingSeq_pos (t + 1) (by omega)
      have hSdt := entropyStirlingSeq_pos (d + t) (by omega)
      have hstep :
          entropyStirlingSeq d * entropyStirlingSeq (t + 1) ≤
            2 * entropyStirlingSeq (d + (t + 1)) := by
        calc
          entropyStirlingSeq d * entropyStirlingSeq (t + 1) =
              (entropyStirlingSeq d * entropyStirlingSeq t) *
                (entropyStirlingSeq (t + 1) / entropyStirlingSeq t) := by
                  field_simp [hSt.ne']
          _ ≤ (2 * entropyStirlingSeq (d + t)) *
                (entropyStirlingSeq (t + 1) / entropyStirlingSeq t) := by
                  gcongr
          _ ≤ (2 * entropyStirlingSeq (d + t)) *
                (entropyStirlingSeq (d + t + 1) /
                  entropyStirlingSeq (d + t)) := by
                  gcongr
          _ = 2 * entropyStirlingSeq (d + t + 1) := by
                  field_simp [hSdt.ne']
      simpa [t, Nat.succ_eq_add_one, Nat.add_assoc] using hstep

def binomial_mean_mass_ge
    (d m : ℕ) (hd : 0 < d) (hm : 0 < m) :
    1 / Real.sqrt
        (8 * (d : ℝ) * (m : ℝ) / ((d + m : ℕ) : ℝ)) ≤
      (((d + m).choose d : ℕ) : ℝ) *
        ...
        ((m : ℝ) / ((d + m : ℕ) : ℝ)) ^ m := by
  have hSd := entropyStirlingSeq_pos d hd
  have hSm := entropyStirlingSeq_pos m hm
  have hprod : 0 < entropyStirlingSeq d * entropyStirlingSeq m :=
    mul_pos hSd hSm
  have hcross := entropyStirlingSeq_mul_le_two_mul_add d m hd hm
  have hratio :
      (1 : ℝ) / 2 ≤
        entropyStirlingSeq (d + m) /
          (entropyStirlingSeq d * entropyStirlingSeq m) := by
  ...
  rw [← binomial_mean_mass_eq_entropyStirlingSeq d m hd hm] at hmul
  exact hmul

def binomial_integral_mean_mass_ge
    (n d : ℕ) (δ : ℝ) (hn : 0 < n)
    (hδpos : 0 < δ) (hδlt : δ < 1)
    (hd : (d : ℝ) = δ * n) :
...
def qEntropy_power_mul_mass_eq
...
def linear_lambda_ge_entropy_volume_compiled
    (C : Submodule F (ι → F)) (δ : ℝ) (_hδ_pos : 0 < δ) (_hδ_lt : δ < 1)
...
def two_mul_log_le_sub_inv (x : ℝ) (hx : 1 ≤ x) :
    2 * Real.log x ≤ x - x⁻¹ := by
  let f : ℝ → ℝ := fun y => y - y⁻¹ - 2 * Real.log y
  have hder (y : ℝ) (hy0 : y ≠ 0) :
      HasDerivAt f (1 - (-1 / y ^ 2) - 2 * y⁻¹) y := by
    have hid : HasDerivAt (fun z : ℝ => z) 1 y := hasDerivAt_id y
    have hinv : HasDerivAt (fun z : ℝ => z⁻¹) (-1 / y ^ 2) y := hid.inv hy0
    have hlog : HasDerivAt (fun z : ℝ => Real.log z) y⁻¹ y :=
      Real.hasDerivAt_log hy0
    have htwo : HasDerivAt (fun z : ℝ => 2 * Real.log z) (2 * y⁻¹) y :=
      hlog.const_mul 2
    dsimp [f]
    exact (hid.sub hinv).sub htwo
  have hmono : MonotoneOn f (Set.Ici 1) := by
    refine monotoneOn_of_deriv_nonneg (convex_Ici (1 : ℝ)) ?_ ?_ ?_
    · apply continuousOn_of_forall_continuousAt
      intro y hy
      have hy0 : y ≠ 0 := (lt_of_lt_of_le zero_lt_one hy).ne'
      exact (hder y hy0).continuousAt
    · intro y hy
      have hy1 : (1 : ℝ) < y := by simpa only [interior_Ici, Set.mem_Ioi] using hy
      have hy0 : y ≠ 0 := (lt_trans zero_lt_one hy1).ne'
      exact (hder y hy0).differentiableAt.differentiableWithinAt
    · intro y hy
      have hy1 : (1 : ℝ) < y := by simpa only [interior_Ici, Set.mem_Ioi] using hy
      have hy0 : y ≠ 0 := (lt_trans zero_lt_one hy1).ne'
      rw [(hder y hy0).deriv]
  ...
  have hbase : f 1 = 0 := by simp [f]
  have hfx : f 1 ≤ f x := hmono (by simp) hx hx
  rw [hbase] at hfx
  dsimp [f] at hfx
  linarith
...
theorem linear_lambda_ge_entropy_volume
    ...
    ENNReal.ofReal
        ((q : ℝ) ^ ((n : ℝ) * (ρ - 1 + qEntropy q δ))
          / (8 * n * δ * (1 - δ)) ^ ((1 : ℝ) / 2))
      ≤ (Lambda ((C : Set (ι → F))) δ : ENNReal) := by
  exact linear_lambda_ge_entropy_volume_compiled
    C δ _hδ_pos _hδ_lt _hδn_int
