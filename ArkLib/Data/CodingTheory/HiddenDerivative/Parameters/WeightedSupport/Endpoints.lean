...
* `highRate_denominator_sq_le_rate`: `(ρ + θ δ) ^ 2 ≤ ρ` for `δ ≤ ρ ≤ 1 - δ` and
  `θ ^ 2 δ ≤ (1 - 2 θ) (1 - δ)`.
* `highRate_baseline_lower`, `highRate_variance_lower`, `lowRate_baseline_lower`: lower bounds on
  the two terms.
...
theorem highRate_baseline_lower {θ ξ δ ρ H logd : ℝ} (hθ : 0 ≤ θ) (hξ : 0 ≤ ξ) (hδ : 0 < δ)
...
theorem highRate_variance_lower {θ ξ δ ρ logd : ℝ} (hθ : 0 ≤ θ) (hξ : 0 ≤ ξ) (hδ : 0 < δ)
...
`logd ≥ ξ / δ ≥ ξ / δ₀`. -/
theorem lowRate_baseline_lower {θ ξ δ₀ c δ ρ H logd : ℝ} (hθ : 0 ≤ θ) (hξ : 0 ≤ ξ) (hc : 0 ≤ c)
...
`highRate_baseline_lower` and `highRate_variance_lower`; for `ρ < δ`, `lowRate_baseline_lower`
with an eighth-degree Taylor polynomial of `exp (162 / 55)` gives more than `29 / 10` from `F`
alone. -/
theorem normalizedDimensionRankSurplus_gt (δ ρ H logd : ℝ) (hδ : 0 < δ) (hδmax : δ ≤ 1 / 4)
  ...
  · have hg : rateGap δ ρ = δ / ρ := by rw [rateGap_eq_div_max hδ hρ0, max_eq_right hhigh]
    have hF : (729 / 100 : ℝ) * (11 / 4) < dimensionBaselineFactor δ ρ H logd := by
      ...
      refine lt_of_lt_of_le ?_ (highRate_baseline_lower theta_pos.le xi_pos.le hδ hhigh
    ...
    have hJ : (81 / 110 : ℝ) * (163 / 60) < dimensionVarianceFactor δ ρ logd := by
      ...
      refine lt_of_lt_of_le ?_ (highRate_variance_lower theta_pos.le xi_pos.le hδ hhigh hlog)
  ...
  · have hg : rateGap δ ρ = 1 := by
    ...
    have hF : 1 / 3 * xi ^ 2 / (1 / 4) * (1 / (1 + theta)) ^ 2 *
      ...
      rw [dimensionBaselineFactor, hg]
      simpa only [mul_one, one_mul] using lowRate_baseline_lower theta_pos.le xi_pos.le
