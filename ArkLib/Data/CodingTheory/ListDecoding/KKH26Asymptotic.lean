...
- `rs_asymptotic_list_lower_bound_kkh26` — the theorem above, stated over the in-tree
  `ListDecodable.Lambda` list notion that `choose_le_Lambda_rs_vanilla` bounds, with the
...
open Polynomial Finset ListDecodable ProximityGap Real Filter Topology
