...
lemma sum_K_eq_card : ∑ α : F, K B i α = B.card := by
  ...
  rw [card_biUnion (by rw [coe_univ]; exact Fis_pairwise_disjoint)]

/-- `K B i α` expressed as a sum of indicators over `B`. -/
@[simp]
lemma K_eq_sum {α : F} :
    K B i α = ∑ x : B, if x.1 i = α then 1 else 0 := by
  rw [sum_coe_sort B (fun x ↦ if x i = α then 1 else 0), sum_boole, Nat.cast_id, K, Fi]
  congr 1
  ext x
...
@[simp]
lemma K_le_card {α : F} : K B i α ≤ B.card :=
  card_le_card fun _ ha ↦ (mem_filter.1 ha).2.1
...
lemma sum_choose_K' [Zero F] (h_card : 2 ≤ card F) :
  ...
  have hx1 : (0 : ℚ) < card F - 1 := sub_pos.2 (by exact_mod_cast h_card)
  have hsum : (B.card : ℚ) - K B i 0 = ∑ α ∈ univ.erase 0, (K B i α : ℚ) := by
    rw [← sum_K_eq_card (i := i), Nat.cast_sum, ← add_sum_erase _ _ (mem_univ (0 : F)),
      add_sub_cancel_left]
  have jensen := ConvexOn.map_sum_le choose_2_convex
    (t := univ.erase (0 : F)) (w := fun _ ↦ (card F - 1 : ℚ)⁻¹) (p := fun α ↦ (K B i α : ℚ))
    (fun _ _ ↦ inv_nonneg.2 hx1.le)
    (by rw [sum_const, card_erase_of_mem (mem_univ _), card_univ, nsmul_eq_mul,
      Nat.cast_sub (by omega), Nat.cast_one, mul_inv_cancel₀ hx1.ne'])
    (fun _ _ ↦ Set.mem_univ _)
  simp only [smul_eq_mul] at jensen
  rw [← mul_sum, ← mul_sum] at jensen
  rw [filter_ne', hsum, div_eq_inv_mul]
  exact (mul_le_mul_of_nonneg_left jensen hx1.le).trans_eq
    (by rw [← mul_assoc, mul_inv_cancel₀ hx1.ne', one_mul])
...
lemma hamming_weight_eq_sum [Zero F] {x : Fin n → F} :
    ‖x‖₀ = ∑ i, if x i = 0 then 0 else 1 := by
  rw [hammingNorm, card_filter]
  exact sum_congr rfl fun i _ ↦ ite_not _ _ _
...
lemma k_and_e [Zero F] (h_n : n ≠ 0) (h_B : B.card ≠ 0) :
  ...
  have hB : (B.card : ℚ) ≠ 0 := Nat.cast_ne_zero.2 h_B
  have hH : ((∑ x ∈ B, Δ₀((0 : Fin n → F), x) : ℕ) : ℚ) = n * B.card - ∑ i, K B i 0 := by
    rw [Nat.cast_sum]
    simp only [hammingDist_zero_left]
    exact sum_hamming_weight_sum
  have hn : (n : ℚ) ≠ 0 := Nat.cast_ne_zero.2 h_n
  rw [k, e, hH, mul_sub, ← mul_assoc (B.card : ℚ), mul_one_div_cancel hB, one_mul, eq_div_iff hn,
    mul_right_comm, one_div_mul_cancel hn, one_mul]
  ring
...
lemma k_and_e' [Zero F] (h_n : n ≠ 0) (h_B : B.card ≠ 0) :
    k B / B.card = (n - e B 0) / n := by
  rw [k_and_e h_n h_B, mul_div_assoc, mul_div_cancel_left₀ _ (Nat.cast_ne_zero.2 h_B)]
...
lemma k_choose_2 [Zero F] (h_n : n ≠ 0) :
  ...
  have jensen := ConvexOn.map_sum_le choose_2_convex
    ...
    (fun _ _ ↦ inv_nonneg.mpr hn_pos.le)
    (by rw [sum_const, card_univ, Fintype.card_fin, nsmul_eq_mul, mul_inv_cancel₀ hn_pos.ne'])
    (fun _ _ ↦ Set.mem_univ _)
  simp only [smul_eq_mul] at jensen
  rw [← mul_sum, ← mul_sum] at jensen
  calc (n : ℚ) * choose_2 (k B) = n * choose_2 ((n : ℚ)⁻¹ * ∑ i, (K B i 0 : ℚ)) := by
        rw [k, one_div, Nat.cast_sum]
    _ ≤ n * ((n : ℚ)⁻¹ * ∑ i, choose_2 (K B i 0)) := mul_le_mul_of_nonneg_left jensen hn_pos.le
    _ = _ := by rw [← mul_assoc, mul_inv_cancel₀ hn_pos.ne', one_mul]
...
lemma sum_1_over_n_aux_frac_k_i [Zero F] (h_n : 0 < n) :
  ...
  simp only [aux_frac]
  rw [← sum_div, sum_sub_distrib, sum_const, card_univ, Fintype.card_fin, nsmul_eq_mul, k,
    mul_div_assoc', mul_sub, ← mul_assoc, one_div_mul_cancel hn0, one_mul, Nat.cast_sum]
...
lemma le_sum_sum_choose_K [Zero F] (h_n : 0 < n) (h_card : 2 ≤ card F) :
  ...
  have h_ineq2 : n * ((card F - 1 : ℚ) *
    ...
    have hmean : ∑ i, (1 / n : ℚ) * ((B.card - K B i 0) / (card F - 1)) =
        (B.card - k B) / (card F - 1) :=
      (mul_sum _ _ _).symm.trans (sum_1_over_n_aux_frac_k_i h_n)
    rw [← hmean]
...
lemma F2i_card {α : F} : (F2i B i α).card = K B i α * (K B i α - 1) := by
  have h : F2i B i α = (Fi B i α).offDiag := by
    ext x
    simp only [F2i, Fi, mem_filter, mem_univ, true_and, mem_product, mem_offDiag]
    tauto
  rw [h, offDiag_card, Nat.mul_sub_one]
...
lemma Bi_card : (Bi B i).card = ∑ α : F, K B i α * (K B i α - 1) := by
  rw [Bi_biUnion_F2i, card_biUnion (by rw [coe_univ]; exact F2i_disjoint)]
  simp_rw [F2i_card]
...
lemma d_eq_sum (h_B : 2 ≤ B.card) :
  ...
  have hc : 2 * choose_2 (B.card : ℚ) ≠ 0 := mul_ne_zero two_ne_zero (choose_2_card_ne_zero h_B)
  rw [d, ← mul_assoc, mul_one_div_cancel hc, one_mul, Nat.cast_sum, sum_comm]
  refine sum_congr rfl fun y _ ↦ ?_
  rw [hamming_dist_eq_sum, Nat.cast_sum]
  refine sum_congr rfl fun i _ ↦ ?_
  rw [Nat.cast_ite, Nat.cast_zero, Nat.cast_one]
  exact (ite_not _ _ _).symm
...
lemma johnson_unrefined [Zero F]
  ...
  have h_rewrite : (k B * (k B - 1) + (B.card - k B) *
    ...
    rw [one_sub_div (Nat.cast_ne_zero.2 h_n.ne')]
    ring
...
lemma johnson_denom [Zero F] (h_card : 2 ≤ card F) :
  ...
  have n₂ : c1 ≠ 0 := sub_ne_zero.2 (Nat.one_lt_cast.2 h_card).ne'
...
protected lemma abs_one_sub_div_le_one {v a : Fin n → F}
  ...
  have hq1 : (1 : ℚ) ≤ card F - 1 := le_sub_iff_add_le.2 (by exact_mod_cast h_card)
  have hc0 : 0 ≤ 1 / ((card F : ℚ) - 1) := one_div_nonneg.2 (zero_le_one.trans hq1)
  have hc1 : 1 / ((card F : ℚ) - 1) ≤ 1 := (div_le_one (zero_lt_one.trans_le hq1)).2 hq1
  have hΔ : (Δ₀(v, a) : ℚ) ≤ n := by
    exact_mod_cast hammingDist_le_card_fintype.trans_eq (Fintype.card_fin n)
  have ht0 : 0 ≤ (1 + 1 / ((card F : ℚ) - 1)) * Δ₀(v, a) / n :=
    div_nonneg (mul_nonneg (add_nonneg zero_le_one hc0) (Nat.cast_nonneg _)) (Nat.cast_nonneg _)
  have ht2 : (1 + 1 / ((card F : ℚ) - 1)) * Δ₀(v, a) / n ≤ 1 + 1 :=
    div_le_of_le_mul₀ (Nat.cast_nonneg _) (by positivity)
      (mul_le_mul (add_le_add_right hc1 1) hΔ (Nat.cast_nonneg _) (by positivity))
  exact abs_le.2 ⟨neg_le_sub_iff_le_add.2 ht2, sub_le_self 1 ht0⟩
...
lemma johnson_hyp_implies_div_ineq {n d e : ℕ}
  ...
  have hnn : 0 ≤ (n : ℝ) * (n - d) :=
    mul_nonneg (Nat.cast_nonneg _) (sub_nonneg_of_le (mod_cast h_dn))
  have h_mul : (n : ℝ) * (n - d) ≤ (n - e) ^ 2 :=
    (sq_sqrt hnn).symm.trans_le (pow_le_pow_left₀ (sqrt_nonneg _) (by linarith) 2)
  rw [one_sub_div hn'.ne', one_sub_div hn'.ne', div_pow, sq (n : ℝ),
    ← mul_div_mul_left _ _ hn'.ne']
  exact div_le_div_of_nonneg_right h_mul (mul_self_nonneg _)
...
lemma johnson_e_div_ne_J {n d e : ℕ} {q : ℚ}
  ...
  exact one_sub_sqrt_ne_of_one_lt ((one_lt_div (sub_pos.2 hq')).2 (sub_one_lt _))
  ...
  have h_e_le_d : (e : ℚ) ≤ d := by
    ...
    have hsq : (n : ℝ) - d ≤ √(n * (n - d)) :=
      Real.le_sqrt_of_sq_le (by
        rw [sq]
        exact mul_le_mul_of_nonneg_right (sub_le_self _ d.cast_nonneg) (sub_nonneg.2 hdn))
    exact_mod_cast (h.trans (sub_le_sub_left hsq _)).trans_eq (sub_sub_cancel _ _)
...
lemma johnson_gap_frac_d_gt_one {n d : ℕ} {F : Type*} [Fintype F] [DecidableEq F]
    ...
    (1 : ℚ) / ((n : ℚ) * ((card F : ℚ) - 1)) ≤
    ((card F : ℚ) / (card F - 1)) * (JohnsonBound.d B) / n - 1 := by
  have hq1 : (0 : ℚ) < card F - 1 := sub_pos.2 (one_lt_two.trans_le q_not_small)
  have hn : (0 : ℚ) < n := by exact_mod_cast n_not_small
  have hqn : (0 : ℚ) < (card F - 1) * n := mul_pos hq1 hn
  have h_qd_ge : (card F : ℚ) * d ≥ (card F - 1) * n + 1 := by
    have : (card F : ℚ) * d > (card F - 1) * n := by
      rwa [div_mul_div_comm, gt_iff_lt, one_lt_div hqn] at h_d_close_n
    exact_mod_cast this
  have hD : (card F : ℚ) * d ≤ card F * JohnsonBound.d B :=
    mul_le_mul_of_nonneg_left hd_le_dB (by linarith)
  rw [mul_comm (n : ℚ), div_mul_eq_mul_div, div_div, le_sub_iff_add_le, div_add_one hqn.ne',
    div_le_div_iff_of_pos_right hqn]
  linarith
...
lemma johnson_den_lb_e_pos {n d e : ℕ} {q frac : ℚ}
  ...
  have h_e_div_n_ge : 1 / (n : ℚ) ^ 2 ≤ (e / n : ℚ) ^ 2 := by
    rw [div_pow]
    exact div_le_div_of_nonneg_right
      (one_le_pow₀ (by exact_mod_cast Nat.one_le_iff_ne_zero.2 he0)) (sq_nonneg _)
  calc (1 : ℚ) / (q * (n : ℚ) ^ 2) = 1 / q * (1 / (n : ℚ) ^ 2) := (one_div_mul_one_div _ _).symm
    _ ≤ (frac - 1) * (e / n : ℚ) ^ 2 :=
      mul_le_mul one_div_q_le h_e_div_n_ge (by positivity) hfrac1_pos.le
    _ ≤ _ := by linarith
...
lemma johnson_d_le_n {n : ℕ} {F : Type*} [DecidableEq F]
  ...
  have h2 : (2 : ℚ) ≤ B.card := by exact_mod_cast hB
  have hoff : ({x ∈ B ×ˢ B | x.1 ≠ x.2} : Finset _) = B.offDiag := by
    ext; simp only [mem_filter, mem_product, mem_offDiag, and_assoc]
  have hpairs : ((B.offDiag.card : ℕ) : ℚ) = 2 * choose_2 B.card := by
    rw [offDiag_card, Nat.cast_sub (Nat.le_mul_self _), Nat.cast_mul, choose_2]
    ring
  have hpos : (0 : ℚ) < 2 * choose_2 B.card := by
    rw [← hpairs, offDiag_card, Nat.cast_sub (Nat.le_mul_self _), Nat.cast_mul]
    nlinarith
  have h_sum_le : ∑ x ∈ B.offDiag, Δ₀(x.1, x.2) ≤ B.offDiag.card • n :=
    sum_le_card_nsmul _ _ _ fun _ _ ↦ hammingDist_le_card_fintype.trans_eq (Fintype.card_fin n)
  rw [d, hoff, one_div, inv_mul_le_iff₀ hpos, ← hpairs]
