...
* `ReedSolomon.exists_graphLine_pair_of_symbolic_sample`: one sample determines a base-field pair
  that reconstructs every regular symbolic Taylor chart point satisfying the sample equations.
...
/-- One sample determines a base-field pair uniformly for all regular symbolic chart points. The
conclusions identify the reconstructed polynomial, its initial jet, and each cleared Taylor
coefficient. -/
theorem exists_graphLine_pair_of_symbolic_sample
  ...
  have hdegree :
    ...
    simpa only [hcenter, hφ] using degree_rationalTaylorPolynomial_lt_of_symbolic_high_cuts
      φ (Polynomial.C center) Q K k jet hS hhigh
