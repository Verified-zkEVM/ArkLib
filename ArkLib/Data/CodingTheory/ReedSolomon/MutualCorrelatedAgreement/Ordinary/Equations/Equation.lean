...
Factor the actual nonzero equation into distinct irreducible factors and root-independent
...
with the original root-degree and challenge-height budgets in arbitrary characteristic. -/
theorem exists_exceptional_ordinaryEquation
  ...
  have hf : ∀ a ∈ ordinaryRootFactorClasses flat, ∃ ex : Finset E,
    ...
    obtain ⟨ex, hcard, hgood⟩ := exists_exceptional_irreducibleOrdinaryEquation
      domain f g ι (ordinaryUnflatten E (ordinaryFactorRepresentative a)) D
  ...
  obtain ⟨ex, hcard, hgood⟩ := exists_exceptional_ordinaryFactorAssembly flat hflat ev Good height
