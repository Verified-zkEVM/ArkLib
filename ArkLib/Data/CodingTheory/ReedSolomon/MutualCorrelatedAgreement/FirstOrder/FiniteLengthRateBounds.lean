...
theorem automaticFirstOrder_finiteLength_rate_bounds
  ...
  · have hnSelector : (2 : ℝ) ≤ rho * n :=
    ...
    · intro received
      ...
      exact closePolynomialSet_finite_and_card_le_finiteLength_of_selector_certificate
        hrho hrhoOne heta haOne hnSelector hbetaHalf hDrate hA hAn
          domain received _ listCert hchar |>.imp id fun hbound ↦
            hbound.trans (div_finiteLengthSlack_sq_le_div_eta_sq
              (by
    ...
    · intro f g
      ...
      exact exists_exceptional_finiteLengthMCA_of_selector_certificate_inv_eta
  ...
  · have hkOne : k = 1 := by omega
    ...
    constructor
    · intro received
      exact closePolynomialSet_finite_and_card_le_inv_eta_of_dimension_le_one
    ...
    · intro f g
      ...
      have hetaOne : eta < 1 := by
        have hthresholdPos : 0 < automaticFirstOrderThreshold rho :=
          hrho.trans (rho_lt_automaticFirstOrderThreshold hrho hrhoOne)
        linarith
      have hCpow : 1 ≤ finiteLengthMCAParameterConstant rho ^ 6 := one_le_pow₀ hC
      rw [le_div_iff₀ (pow_pos heta 4)]
      have hetaFour : eta ^ 4 ≤ 1 := pow_le_one₀ heta.le hetaOne.le
      have hnSq : (0 : ℝ) ≤ (n : ℝ) ^ 2 := sq_nonneg _
      calc
        (n : ℝ) ^ 2 * eta ^ 4 ≤ (n : ℝ) ^ 2 := by nlinarith
