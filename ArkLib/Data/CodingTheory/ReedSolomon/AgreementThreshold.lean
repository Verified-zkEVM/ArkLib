...
* `ReedSolomon.agreementThreshold`: the threshold `k + ⌈δ n⌉`.
* `ReedSolomon.capacityRadius`: the radius `1 - k / n - δ`.

## Main statements

* `ReedSolomon.agreementThreshold_le_iff_real`: for `0 ≤ δ`, the threshold is at most `a`
  exactly when `k + δ n ≤ a`.
* `ReedSolomon.relHammingDist_le_capacityRadius_iff_agreementThreshold_le`: for `0 ≤ δ` and
...
`k + δ n = 0`. -/
theorem agreementThreshold_le_iff_real {delta : ℝ} (hdelta : 0 ≤ delta)
  ...
  rw [agreementThreshold, add_comm,
...
threshold is `0`. -/
theorem relHammingDist_le_capacityRadius_iff_agreementThreshold_le {ι F : Type*} [Fintype ι]
    ...
    (Code.relHammingDist received codeword : ℝ) ≤
      ...
      agreementThreshold delta (Fintype.card ι) messageDim ≤ Code.agree codeword received := by
  ...
  rw [agreementThreshold_le_iff_real hdelta, hradius]
