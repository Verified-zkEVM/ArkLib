...
/-- Read the table through its public `Fin K` coordinate type. -/
def computableRationalTaylorTableEntry {r : ℕ} (center : F)
    (Q : CPoly.CMvPolynomial (r + 2) F) (K : ℕ) (i : Fin K) :
    CPoly.CMvPolynomial (r + 1) F :=
  let table := computableRationalTaylorTable center Q K
  table[Fin.cast (computableRationalTaylorTable_size center Q K).symm i]

theorem computableRationalTaylorTableEntry_eq {r : ℕ} (center : F)
    (Q : CPoly.CMvPolynomial (r + 2) F) (K : ℕ) (i : Fin K) :
    computableRationalTaylorTableEntry center Q K i =
      computableRationalTaylorNumerator center Q i.val := by
  rw [computableRationalTaylorTableEntry]
