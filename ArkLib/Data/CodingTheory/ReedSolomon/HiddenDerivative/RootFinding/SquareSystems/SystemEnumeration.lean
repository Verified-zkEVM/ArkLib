...
theorem card_map_univ_embedding {ι : Type*} [DecidableEq ι] {r : ℕ}
...
theorem rowSubsetEmbedding_preserves_injective {F V ι : Type*} [Field F]
    [AddCommGroup V] [Module F V] [Fintype ι] [LinearOrder ι] {r : ℕ}
...
def finEmbeddingOfLE {k A : ℕ} (hkA : k ≤ A) : Fin k ↪ Fin A where
  ...
  inj' := fun _ _ hij ↦ Fin.ext (congrArg Fin.val hij)
...
theorem exists_fullCandidateSquareRows_at_solution (center : F)
  ...
  · intro i
    ...
    rw [squareSystemRows, fullCandidatePoolEquation_agreementPoolEmbedding]
...
theorem formalJacobian_squareSystemRows_ne_zero (point : Fin (r + 1) → F)
...
theorem exists_emittedCandidateSquareSystem [DecidableEq F] (center : F)
  ...
  · intro i
    ...
    rw [selectedSet, Finset.mem_map] at hjmem
    obtain ⟨source, _, hsource⟩ := hjmem
    have hsourceZero := hzero source.succ
    simpa only [squareSystemRows, canonical, hsource] using hsourceZero
  · exact squareSystemRows_mem_enumerate _ _ selectedSet hcard
  · apply formalJacobian_squareSystemRows_ne_zero
    simpa only [fullCandidatePoolDifferential] using hcanonicalInjective
