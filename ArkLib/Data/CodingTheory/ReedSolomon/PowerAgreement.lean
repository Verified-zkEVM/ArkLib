...
section ExceptionalAgreement

variable {F E ι : Type*} [Field F] [Field E] [Fintype ι] [DecidableEq F] [DecidableEq E]
  {ℓ k L : ℕ}

/-- Extending the coefficient field preserves the common agreement positions of a tuple. -/
theorem commonCurveAgreementSet_map (domain : ι ↪ F) (w : Fin (ℓ + 1) → ι → F)
    (P : Fin (ℓ + 1) → F[X]) (ι' : F →+* E) :
    commonCurveAgreementSet (domain.trans ⟨ι', ι'.injective⟩)
      (fun t i ↦ ι' (w t i)) (fun t ↦ (P t).map ι') = commonCurveAgreementSet domain w P := by
  classical
  ext i
  ...
  simp only [ι'.injective.eq_iff]

/-- A degree-bounded polynomial tuple has exact power agreement outside at most
`ℓ * (|ι| - L)` extension-field challenges when its members have `L` common agreements. -/
theorem exists_exceptional_exactPowerAgreement (domain : ι ↪ F)
    (w : Fin (ℓ + 1) → ι → F) (P : Fin (ℓ + 1) → F[X]) (ι' : F →+* E)
    (hdegree : ∀ t, (P t).degree < k)
    (hcommon : L ≤ (commonCurveAgreementSet domain w P).card) :
    ∃ exceptional : Finset E, exceptional.card ≤ ℓ * (Fintype.card ι - L) ∧
      ∀ z ∉ exceptional, HasExactPowerAgreement domain w ι' k z
        (powerBatchedPolynomial (fun t ↦ (P t).map ι') z) := by
  classical
  let mappedDomain : ι ↪ E := domain.trans ⟨ι', ι'.injective⟩
  have hmap := commonCurveAgreementSet_map domain w P ι'
  obtain ⟨exceptional, hcard, hgood⟩ := exists_exceptional_powerBatched_agreement
    mappedDomain (fun t i ↦ ι' (w t i)) (fun t ↦ (P t).map ι') L
    (by rw [hmap]; exact hcommon)
  refine ⟨exceptional, hcard, fun z hz ↦ ⟨P, hdegree, rfl, ?_⟩⟩
  simpa only [mappedDomain] using (hgood z hz).trans hmap

/-- One exceptional set works for a finite family of degree-bounded tuples, with total size at
most `family.card * ℓ * (|ι| - L)`. -/
theorem exists_exceptional_exactPowerAgreement_family (domain : ι ↪ F)
    (w : Fin (ℓ + 1) → ι → F) (ι' : F →+* E)
    (family : Finset (Fin (ℓ + 1) → F[X]))
    (hdegree : ∀ P ∈ family, ∀ t, (P t).degree < k)
    (hcommon : ∀ P ∈ family, L ≤ (commonCurveAgreementSet domain w P).card) :
    ∃ exceptional : Finset E,
      exceptional.card ≤ family.card * (ℓ * (Fintype.card ι - L)) ∧
      ∀ P ∈ family, ∀ z ∉ exceptional,
        HasExactPowerAgreement domain w ι' k z
          (powerBatchedPolynomial (fun t ↦ (P t).map ι') z) := by
  classical
  have hex : ∀ P ∈ family, ∃ exceptional : Finset E,
      exceptional.card ≤ ℓ * (Fintype.card ι - L) ∧
      ∀ z ∉ exceptional, HasExactPowerAgreement domain w ι' k z
        (powerBatchedPolynomial (fun t ↦ (P t).map ι') z) :=
    fun P hP ↦ exists_exceptional_exactPowerAgreement domain w P ι' (hdegree P hP)
      (hcommon P hP)
  ...
  · calc
      ...
      _ ≤ ∑ _P ∈ family, ℓ * (Fintype.card ι - L) := by
        apply Finset.sum_le_sum
        intro P hP
        simpa [exceptions, hP] using hcard P hP
      _ = family.card * (ℓ * (Fintype.card ι - L)) := by simp
  · intro P hP z hz
    apply hgood P hP z
    intro hmem
    apply hz
