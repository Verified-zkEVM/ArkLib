...
/-!
# Same-execution first-nonzero coordinate witness

The emitted optional point is exactly the source ordered witness. Each source edge lowers with
one absolute factor, independent of sample count; every delegated base instruction is charged.
-/
...
def represent {K F : Type*} (f : K → Pair F) (input : ResidualWitnessMachine.Input K) :
    ResidualWitnessMachine.Configuration K → Configuration F
...
theorem arithmetic_trace {a : F} {input : Input F} (point : Pair F) (ps : List (Entry F))
    ...
    ∃ d, Trace a input n (.check point ps payload s) d (.check point ps payload t) ∧
...
/-- Both coordinates are tested before the actual Boolean selects or advances past a point. -/
theorem check_lowering (a : F) (input : Input F) (u v : Pair F) (ps : List (Entry F))
    ...
    ∃ n c, Trace a input n (.scan ((u, v) :: ps)) c (checked u ps b) ∧
      n + c.total ≤ 256 := by
  obtain ⟨n, hn, ht⟩ := ArithmeticMachine.execution_trace ⟨a, v, (0, 0)⟩ .equal
  rw [hb] at ht
  obtain ⟨c, hc, he⟩ := arithmetic_trace (a := a) (input := input) u ps ht
  refine ⟨n + 1 + 1, launch + (c + administrative ResidualWitnessMachine.checkCost),
    .cons rfl (hc.trans (single rfl)), ?_⟩
  have hm := ArithmeticMachine.cost_total_le .equal
  simp only [total_add, he]
  change n + 1 + 1 + (14 + ((ArithmeticMachine.cost .equal).total + 3 * n + 5)) ≤ 256
  omega

/-- Every source acceptance edge lowers with one absolute constant. -/
theorem step_lowering {a : F} {input : ResidualWitnessMachine.Input (QuadraticAlgebra F a 0)}
    {s t : ResidualWitnessMachine.Configuration (QuadraticAlgebra F a 0)}
    {c : ResidualWitnessMachine.Cost} (h : ResidualWitnessMachine.Step input s c t) :
  ...
  cases h with
  | enter => exact ⟨1, administrative ResidualWitnessMachine.entryCost + allocation 4,
  ...
  | batch h =>
      ...
      exact ⟨n, d, hd, by omega⟩
  | handoff => exact ⟨1, administrative ResidualWitnessMachine.handoffCost, single rfl, by decide⟩
  | zero hv =>
      subst hv
      apply small
  ...
  | empty => exact ⟨1, administrative ResidualWitnessMachine.emptyCost, single rfl, by decide⟩
  | emit => exact ⟨1, administrative ResidualWitnessMachine.emitCost, single rfl, by decide⟩

/-- A constant factor applies once to the whole source execution. -/
theorem trace_lowering {a : F} {input : ResidualWitnessMachine.Input (QuadraticAlgebra F a 0)}
    {n : ℕ} {s t : ResidualWitnessMachine.Configuration (QuadraticAlgebra F a 0)}
    {c : ResidualWitnessMachine.Cost} (h : ResidualWitnessMachine.Trace input n s c t) :
...
theorem run_lowering (a : F) (input : ResidualWitnessMachine.Input (QuadraticAlgebra F a 0))
    (fuel : ℕ) (s : ResidualWitnessMachine.Configuration (QuadraticAlgebra F a 0)) :
    ∃ k d, runFuel a (mapInput encode input) k (represent encode input s) =
      (represent encode input (ResidualWitnessMachine.runFuel input fuel s).1, d) ∧
      k + d.total ≤ 32768 * fuel := by
  obtain ⟨n, hn, ht⟩ := ResidualWitnessMachine.runFuel_refines input fuel s
  obtain ⟨k, d, hd, hb⟩ := trace_lowering ht
  exact ⟨k, d, hd.runFuel_eq, hb.trans (Nat.mul_le_mul_left _ hn)⟩

/-- Raw coordinate execution returns the exact first source witness, including empty samples. -/
theorem computation_correct (a : F) (input : Input F) (ps : List (Pair F)) :
    ∃ k c, runFuel a input k (.start ps) =
      (.done ((ResidualWitnessMachine.result (mapInput (ArithmeticMachine.decode a) input)
        (ps.map (ArithmeticMachine.decode a))).map encode), c) ∧
      k + c.total ≤ 32768 * ResidualWitnessMachine.fuel
  ...
  obtain ⟨k, c, hr, hb⟩ := run_lowering a (mapInput (ArithmeticMachine.decode a) input)
    (ResidualWitnessMachine.fuel (mapInput (ArithmeticMachine.decode a) input)
      (ps.map (ArithmeticMachine.decode a)).length) (.start (ps.map (ArithmeticMachine.decode a)))
  obtain ⟨sourceCost, hs, _hc⟩ := ResidualWitnessMachine.witness_runFuel
    (mapInput (ArithmeticMachine.decode a) input) (ps.map (ArithmeticMachine.decode a))
  rw [hs] at hr
