...
/-!
# Exact coordinate capacity decoding with an observed primitive-work bound
...
def workCoefficient (d m : ℕ) : ℕ :=
  CoordinateDecoderMachine.fixedCoefficient d m + CoordinateDecoderMachine.zeroCoefficient + 74
...
theorem run_exact (delta : ℝ) (hdelta : 0 < delta) (n k A : ℕ)
    (hblock : (if (1 / 4 : ℝ) ≤ delta then 1 else 8 * weightedSupportMultiplicity delta) ≤ n)
  ...
  · have hd : d = 0 := capacityDerivativeOrder_eq_zero hquarter
    ...
    · obtain ⟨hodd, hL, out, c, hr, he, hc⟩ := CoordinateDecoderMachine.quarter_gap_run_exact
      ...
      have hr' := run_of_coordinate n k d m A
  ...
  · have hsmall : delta < (1 / 4 : ℝ) := lt_of_not_ge hquarter
    ...
    have hm : 0 < m := weightedSupportMultiplicity_pos hdelta hsmall
    have hn : 8 * m ≤ n := by simpa only [if_neg hquarter] using hblock
    obtain ⟨hodd, hL, out, c, hr, he, hc, hlarge⟩ :=
      CoordinateDecoderMachine.small_gap_run_exact delta hdelta hsmall n k A hn hk hkn
    ...
    have hcost (e : ℕ) (he2 : e ≤ 2)
        (hc' : c ≤ CoordinateDecoderMachine.totalBudget k d m A n q e) :
        c + 40 ≤ workCoefficient d m * q ^ (e * d + 29) := by
      have hb := hc'.trans (CoordinateDecoderMachine.budget_fixed k d m A n q e hd hq hAn hnq)
