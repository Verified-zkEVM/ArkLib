...
/-!
# Integer-input capacity decoder

One executable dispatch covers oversized thresholds, exceptional small blocks, and quadratic
...
def run (n k d m A : ℕ) (rows : List (ZMod q × ZMod q)) :
  ...
  if n < A ∨ n ≤ 2 then
    ...
    if h : q ≠ 2 ∧ multiplicity n d m * A ≤ q ^ 2 then
      let decoded := QuadraticDecoderMachine.run k d (multiplicity n d m) A rows h.1 h.2
...
/-- On nonexceptional inputs the same quadratic child result and ledger are returned. -/
theorem run_of_quadratic (n k d m A : ℕ) (rows : List (ZMod q × ZMod q))
    (hA : A ≤ n) (hn : 3 ≤ n) (hodd : q ≠ 2)
    (hL : multiplicity n d m * A ≤ q ^ 2) (out : List (List (ZMod q))) (cost : ℕ)
    (hr : QuadraticDecoderMachine.run k d (multiplicity n d m) A rows hodd hL =
