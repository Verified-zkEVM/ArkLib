...
from the low point (σ₋₁-twisted), and the evaluation `p`. **Sorried** — the
...
/-- **CWSS of the trace handoff (skeleton) — closing the recursion loop.**
...
reinterpretation identity `Com_d(w̃) = Com'_{d′}(ψ(ŵ))` is the `LiftCom`
