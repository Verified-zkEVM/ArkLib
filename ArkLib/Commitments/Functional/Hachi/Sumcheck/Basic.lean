...
— the seam where one `iteration` ends and `EndPiece/`'s `endPiece` takes over. (The §4.5
...
* `Sumcheck/FinalEval.lean` — **Hachi Figure 7 tail**: the closing step — the prover sends the
  ...
  the output is the evaluation claim `mle[w̃](a) = y′` consumed by `EndPiece/`'s `endPiece`.
