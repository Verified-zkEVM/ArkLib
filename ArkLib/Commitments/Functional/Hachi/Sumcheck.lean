...
Umbrella for `Hachi/Sumcheck/`: the sumcheck loop that finishes Hachi's [NOZ26, §4.3] opening. It
...
This umbrella re-exports the folder (`FinalEval` transitively imports `Rounds` and `Bridge`). Its
output relation `relWEvalE` is the input of the §4.5 recursion; the full chain (guarded tail
included) is composed in `Composition.lean`.
