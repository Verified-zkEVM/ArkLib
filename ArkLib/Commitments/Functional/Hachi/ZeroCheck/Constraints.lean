...
/-! ## The table and the batched constraint polynomials (sorried content) -/
...
zero-padded to `2 ^ m₀`. **Sorried**: index bookkeeping over the conventions plus the
`ρ`-digit decomposition. -/
...
the embedding `φF`): `mle[w̃](a) = ∑ᵢ w̃(i)·eq̃(i, a)`. **Sorried.** -/
...
every lifted row of `relLift` vanishes at `α`. **Sorried.** -/
...
Load-bearing for the Kronecker pullback degree bound `< 2 ^ m₀`. **Sorried.** -/
...
Kronecker pullback degree bound `< 2 ^ m₁`. **Sorried.** -/
