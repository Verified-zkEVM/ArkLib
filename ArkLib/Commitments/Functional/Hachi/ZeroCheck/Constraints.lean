...
/-!
  ...
  * `hZero` (Eq. (23)): the `eq̃`-batched range polynomial
    ...
    in `m₀` variables. `H₀ ≡ 0` holds iff every table entry lies in `[−(b−1), b−1]`, assuming
    `2b − 1 < q` so that field roots correspond to centered integer representatives.

  Both are built as multilinear extensions (`MvPolynomial.MLE`), hence have degree `≤ 1` in each
  variable.
...
noncomputable def hAlphaEvals (φF : ZMod q →+* F) (_b : ℕ) (s : RlinStatement Φ n μ) (α : F)
  ...
  fun pt =>
    if h : ((finFunctionFinEquiv pt : Fin (2 ^ m₁)) : ℕ) < n then
      evalAt φF α (rowSum Φ s w.z ⟨_, h⟩)
        - evalAt φF α ((s.yvec ⟨_, h⟩).1.toPoly)
        - evalAt φF α Φ.φ.toPoly * evalAt φF α (w.r ⟨_, h⟩)
    else 0
...
theorem hAlphaEvals_rowPoint (φF : ZMod q →+* F) (b : ℕ) (s : RlinStatement Φ n μ) (α : F)
    ...
    hAlphaEvals Φ m₁ φF b s α w (rowPoint m₁ hn i) =
      evalAt φF α (rowSum Φ s w.z i) - evalAt φF α ((s.yvec i).1.toPoly)
        - evalAt φF α Φ.φ.toPoly * evalAt φF α (w.r i) := by
  simp only [hAlphaEvals, rowPoint, Equiv.apply_symm_apply, Fin.eta, i.isLt, dif_pos]

/-! ## The batched constraint polynomials -/

/-- `H₀` (Eq. (23)): the range polynomial, the multilinear extension of the per-entry range factor
`P_b(w̃(·))` in `τ ∈ F^{m₀}`. `H₀ ≡ 0` iff every table entry is a root of the range product, i.e.
lies in `[−(b−1), b−1]` (assuming `2b − 1 < q`). -/
...
/-- `H_α` (Eq. (22)): the linear-constraint polynomial, the multilinear extension of the
`M̃_α`-contracted per-row values `hAlphaEvals`. `H_α ≡ 0` iff every lifted row of `relLift`
vanishes at `α`. -/
noncomputable def hAlpha (φF : ZMod q →+* F) (b : ℕ) (s : RlinStatement Φ n μ) (α : F)
    (w : LiftedWitness Φ μ n) : MvPolynomial (Fin m₁) F :=
  MLE (hAlphaEvals Φ m₁ φF b s α w)
...
theorem hAlpha_degreeOf_le (φF : ZMod q →+* F) (b : ℕ) (s : RlinStatement Φ n μ) (α : F)
    ...
    (hAlpha Φ m₁ φF b s α w).degreeOf j ≤ 1 :=
  MLE_degreeOf _ j
...
∑_{x ∈ {0,1}^{m₀ − i}} H(cs, x)`, where `cs` fixes the first `i` coordinates. -/
...
(`liftShort`, the shortness precondition of the commitment's weak binding), and both
partial-hypercube-sum claims at the current challenge prefix equal the current targets. The final
conjunct `bound ≤ rlin.bound` ties the global norm parameter to the statement's declared bound. -/
