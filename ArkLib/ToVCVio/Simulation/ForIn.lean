...
/-!
# Loop support and simulation bridge lemmas

Support, safety, and path lemmas for monadic loops, followed by stateful query bridges.
-/
...
lemma simulateQ_forIn_stateful_run_eq {ι : Type} {spec : OracleSpec ι}
  ...
  | cons x xs ih =>
      ...
      cases step with
      | done res => simp only [simulateQ_pure, StateT.run_pure]
  ...
  let rec aux (k : ℕ) (xs : List α) (b₀ : β) (s₀ : σ)
    ...
    | cons y ys ih =>
      ...
      | some step =>
        ...
        subst h_yield_eq
        simp only [h_opt, forIn'_eq_forIn, Option.elim_some] at h_rest_sup
        have h_k_lt : k < l.length := by simp only [List.length_cons] at h_len; omega
        have h_y_eq : y = l.get ⟨k, h_k_lt⟩ := by
          have h_len_drop : 0 < (l.drop k).length := by rw [h_suffix]; exact Nat.zero_lt_succ _
          have : l[k]'h_k_lt = (l.drop k)[0]'h_len_drop := by
        ...
        have h_rel_next : rel ⟨k + 1, by omega⟩ (some b') s' := by
          have h_app := h_step ⟨k, h_k_lt⟩ b₀ s₀ h_rel
            ...
            (by
              ...
              rw [h_y_eq] at hn
              exact hn)
          simp only [ForInStep.state, Fin.succ] at h_app
          exact h_app
        let bs_next : Fin (k + 1 + 1) → β := fun j =>
          if h : j.val ≤ k then bs_acc ⟨j.val, by omega⟩ else b'
        let ss_next : Fin (k + 1 + 1) → σ := fun j =>
...
lemma support_vector_mapM_gen
  ...
  | succ n ih =>
      intro vec x
      obtain ⟨vec0, a, hvec⟩ := Vector.exists_push (xs := vec)
      obtain ⟨x0, b, hx⟩ := Vector.exists_push (xs := x)
      subst hvec
      subst hx
      have hpush : (vec0.push a).mapM f =
          vec0.mapM f >>= fun ys => f a >>= fun last => pure (ys.push last) := by
        have hsingle : (#v[a] : Vector α 1).mapM f =
            (fun last => (#v[last] : Vector β 1)) <$> f a :=
          singleton_mapM_gen f a
      ...
      · intro h
        ...
        · have hiVal : (i : Nat) = n := by omega
          have hi_eq : i = ⟨n, Nat.lt_succ_self n⟩ := Fin.ext hiVal
          subst i
      ...
      · intro h
        ...
        · intro i
          simpa [Vector.getElem_push_lt i.2] using h (Fin.castLT i (by omega))
        · rw [mem_support_bind_iff]
          refine ⟨b, ?_, ?_⟩
          · simpa using h ⟨n, Nat.lt_succ_self n⟩
          · simp
...
lemma OptionT.simulateQ_array_mapM_eq {ι ι' : Type} {spec : OracleSpec ι}
  ...
  apply OptionT.ext
  rw [Array.mapM_eq_mapM_toList, Array.mapM_eq_mapM_toList]
  rw [OptionT.simulateQ_map]
  simpa only [OptionT.run] using congrArg OptionT.run
    (congrArg (fun z : OptionT (OracleComp superSpec) (List β) => List.toArray <$> z)
      (OptionT.simulateQ_list_mapM_eq (so := so) (f := f) xs.toList))
...
lemma OptionT.simulateQ_vector_mapM_eq {ι ι' : Type} {spec : OracleSpec ι}
  ...
  change Vector.toArray <$> (simulateQ so (OptionT.run (Vector.mapM f v)) :
      OptionT (OracleComp superSpec) (Vector β n)) =
    Vector.toArray <$> Vector.mapM (m := OptionT (OracleComp superSpec))
      (fun x ↦ simulateQ so (f x)) v
  rw [← OptionT.simulateQ_map (so := so) (f := Vector.toArray)
    (mx := Vector.mapM f v), Vector.toArray_mapM]
  rw [OptionT.simulateQ_array_mapM_eq]
  rw [← Vector.toArray_mapM]
...
Membership in `support ((m >>= g).run s)` is equivalent to the existence of
`out_forIn ∈ support (m.run s)` such that `x` is in the support of continuing with `g`
from that result (i.e. `(g out_forIn.1).run out_forIn.2`).
...
**Note**: The `++ₛₒ` operator implicitly lifts `challengeQueryImpl` from `ProbComp`
to `StateT σ ProbComp`.
-/
