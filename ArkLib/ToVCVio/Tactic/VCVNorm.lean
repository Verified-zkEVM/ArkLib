...
whose lhs is exactly a logged run); return `(oa, cont)`. -/
meta def matchLogBind (e : Expr) : MetaM (Option (Expr × Expr)) := do
  unless e.isAppOf ``Bind.bind && e.getAppNumArgs == 6 do return none
  let args := e.getAppArgs
  let lhs := args[4]!
  let cont := args[5]!
  let sq := if lhs.isAppOf ``WriterT.run then lhs.getAppArgs.back! else lhs
  unless sq.isAppOf ``simulateQ &&
      sq.getAppArgs.any (fun a => a.getAppFn.constName? == some ``OracleSpec.loggingOracle) do
    return none
...
meta def mkStripEq (e oa cont : Expr) : MetaM (Option (Expr × Expr)) := do
...
simproc_decl loggingStrip (Bind.bind _ _) := fun e => do
  try
    ...
    -- The supporting lemma is Type-0-only; unsupported shapes/universes are not rewrites.
    return .continue
