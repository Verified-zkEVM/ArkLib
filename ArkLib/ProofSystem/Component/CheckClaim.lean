...
`coordinateWiseSpecialSoundWithClassical_of_isEmpty_challengeIdx`) to a transcript-level obligation.
The
...
theorem oracleVerifier_coordinateWiseSpecialSoundWith
    ...
    (oracleVerifier oSpec Statement OStatement).coordinateWiseSpecialSoundWithClassical init impl D
      relIn
