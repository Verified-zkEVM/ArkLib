...
is `t` at the point `w j = getBit j ω`). This is the exact inverse of the index mapping the
novel-coeffs *decoder* uses (`hypercube_evals` above, via `Nat.binaryFinMapToNat`;
roundtrip: `Nat.getBit_of_binaryFinMapToNat`).

NB: this def replaces the previous inline spelling `fun ω => t.val.eval ω`, whose silent
coercion `Fin (2^ℓ) → (Fin ℓ → L)` is the *constant* function `fun _ => ↑ω` (pointwise
`Nat`-cast) — i.e. it read `t`'s diagonal evaluations, making the encoding non-injective
(`X 0` and `X 1` collapsed) and the consistency predicates below vacuously loose. -/
