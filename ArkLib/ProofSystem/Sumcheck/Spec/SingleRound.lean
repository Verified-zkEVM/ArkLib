...
theorem oracleReduction_perfectCompleteness :
  ...
  simp only [StmtIn, InductiveMerkleTree.domain_def, oracleReduction, StmtAfterRandomQuery,
    Nat.reduceAdd, Nat.add_zero, Fin.vcons_fin_zero, StmtAfterCheckClaim, StmtAfterSendClaim]
  ...
  · refine OracleReduction.append_perfectCompleteness
    ...
    · refine OracleReduction.append_perfectCompleteness
      ...
      · sorry
    · sorry
  · simp only [StmtAfterRandomQuery, InductiveMerkleTree.domain_def,
      oracleReduction.reduceClaim, id_eq]
...
where
  proj_complete := by
    simp only [relationRound, Fin.val_castSucc, Set.mem_ofPred_eq, Simple.StmtIn,
      Simple.OStmtIn, Simple.inputRelation, sum_map, InductiveMerkleTree.domain_def,
      Simple.OStmtOut, oCtxLens, forall_const, Prod.forall]
    unfold oStmtLens
    ...
    | succ n ih =>
      ...
      sorry
  lift_complete := by
    simp only [InductiveMerkleTree.domain_def, Simple.OStmtOut, Simple.StmtIn,
      Simple.OStmtIn, oCtxLens, relationRound, Fin.val_castSucc, Set.mem_ofPred_eq,
      Fin.val_succ, Prod.forall]
...
instance extractorLens_rbr_knowledge_soundness :
  ...
  lift_knowledgeSound := by
    simp only [Simple.StmtIn, Simple.OStmtIn, Simple.inputRelation, sum_map,
      Statement.Lens.proj, InductiveMerkleTree.domain_def, Simple.OStmtOut,
      Set.mem_ofPred_eq, relationRound, Fin.val_castSucc, forall_const, Prod.forall]
    unfold oStmtLens
    unfold projectedRoundPolynomial
    induction n with
    | zero => exact Fin.elim0 i
