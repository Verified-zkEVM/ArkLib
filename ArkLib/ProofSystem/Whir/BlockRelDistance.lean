...
lemma listBlock_subset_listHamming
  ...
  intro u hu
  rcases hu with ⟨u_in_C, u_block_dist_le⟩


  exact ⟨u_in_C, le_trans (relHammingDist_le_blockRelDistance i k f u S') u_block_dist_le⟩
  have h₁ : (δᵣ(f, u) : ℝ) ≤ (Δᵣ(i, k, f, S', φ', u) : ℝ) :=
    by exact_mod_cast relHammingDist_le_blockRelDistance i k f u S'
  have h₂ : (Δᵣ(i, k, f, S', φ', u) : ℝ) ≤ (δ : ℝ) :=
    by exact_mod_cast u_block_dist_le

  exact ⟨u_in_C, le_trans h₁ h₂⟩


  have u_hamming_dist_le : δᵣ(f, u) ≤ δ := by
    exact le_trans (relHammingDist_le_blockRelDistance i k f u S') u_block_dist_le
  exact ⟨u_in_C, u_hamming_dist_le⟩


  have u_in_C : u ∈ C := hu.1
  have u_block_dist_le : Δᵣ(i, k, f, S', φ', u) ≤ δ := hu.2
  have dist_inequality : δᵣ(f, u) ≤ Δᵣ(i, k, f, S', φ', u) := by
    exact relHammingDist_le_blockRelDistance i k f u S'
  exact ⟨u_in_C, le_trans dist_inequality u_block_dist_le⟩
