...
rely on: `challengesOfLog_canonChalLog` proves exact reconstruction from the canonical log.
Merely containing those entries in a larger log is insufficient: an earlier conflicting
answer for the same query wins lookup. Extending reconstruction to larger logs requires
an additional consistency or first-match-preservation argument.
...
def canonChalEntry (stmt : Statement) (m : pSpec.Messages) (i : Fin n)
...
`Sum.inr` entry. -/
lemma withQueryLog_getChallengeSR_inr
...
verifier's trace. It is computable and scans the log once per round. No extraction-time
bound is established by this definition. -/
