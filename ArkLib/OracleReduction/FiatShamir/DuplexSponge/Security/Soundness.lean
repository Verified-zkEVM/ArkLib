...
the randomized, verifier-trace-assisted variant of Theorem 6.2. That derivation is completed
in `KnowledgeSoundness.lean`; exact deterministic Definition 3.6 is not established.
...
- `duplex_sponge_fiat_shamir_knowledge_soundness`: **Theorem 6.2 of CO25 (trace-assisted
  variant)**, proved in `KnowledgeSoundness.lean` from the infrastructure here.
...
`adaptiveNARGKnowledgeSoundnessWithExtractor` makes soundness and the trace-assisted KS variant *be
...
theorem basicFS_straightlineKS_withCompiledBound_fixedExtractor
    ...
    Verifier.adaptiveNARGKnowledgeSoundnessWithCoinsWithExtractor (WitIn := WitIn)
      ...
      relIn langOut
      (fun x π tr_P tr_V => OptionT.mk (pure (fsSRDelegatingExtractor E x π tr_P tr_V)))
