...
public import ArkLib.OracleReduction.FiatShamir.SingleSalt
...
Theorem 6.2 adapted to the NARG extractor interface. That derivation is completed in
`KnowledgeSoundness.lean`: extractors receive both logs and may use randomness and oracle queries,
unlike the deterministic, prover-trace-only interface of Definition 3.6.

## Main results

- **Theorem 6.1 of CO25** (`duplex_sponge_fiat_shamir_soundness`): if the interactive proof IP has
...
- **Construction 6.3 adaptation** (`dsfsStraightlineExtractor`): randomized, non-rewinding
  extractor that converts both sponge logs via `D2STrace` and invokes the basic-FS extractor.
  Its concrete single-salt instance then reconstructs the IP transcript and invokes the SR
  extractor using the separated logs.

- `duplex_sponge_fiat_shamir_knowledge_soundness`: **Theorem 6.2 of CO25, adapted to the
  NARG extractor interface**, proved in `KnowledgeSoundness.lean` from the infrastructure here.
...
the Theorem 6.2 variant in `KnowledgeSoundness.lean`—take `KeyLemmaSecurityWitness` explicitly.
This keeps the Section 6 derivations free of `sorryAx` while making Lemma 5.1 the visible
deferred boundary.
...
-- `vcv_norm` / `vcv_init_peel` / `vcv_congr` / `vcv` / `vcv_event` are global
...
/-!
## Construction 6.3: randomized, non-rewinding DSFS extractor
...
/-- CO25 **Construction 6.3 adaptation** — randomized, two-log extractor, built from the
**basic-FS NARG-KS
extractor `E_std`** (delivered by Theorem 3.19,
`single_salt_fiat_shamir_knowledge_soundness`).
The historical name does not assert determinism: `D2STrace` samples via LookAhead, and both
logs are used. The extractor has no prover handle and cannot rewind it. -/
noncomputable def dsfsStraightlineExtractor
    ...
    -- The helper interface supports trace-conversion sampling and base-oracle queries.
    StmtIn →
  ...
  fun stmtIn transcript proveQueryLog verifyQueryLog =>
    ...
    do
      ...
      let tr_std_raw? ← OptionT.lift
        (liftComp (d2sTraceSalted (T_H := T_H) (T_P := T_P) (Salt := Salt) (δ := δ)
          (oSpec := oSpec) (StmtIn := StmtIn) (pSpec := pSpec) (U := U)
          queryLog).run auxSpecE)
      ...
      -- (`tr.fst`) supplied by the coin-bearing KS experiment.
      ...
      -- steps 3-4: run `E_std` on the reconstructed logs in the same helper interface.
...
`adaptiveNARGKnowledgeSoundnessWithExtractor` makes soundness and knowledge soundness *be
about the DSFS NARG* (prover =
...
`Hyb_4` samples its Fiat–Shamir oracle eagerly from `D_IP_salted.sample`, a uniform sampler over
`QueryImpl (fsChallengeOracle (StmtIn × Salt) pSpec) Id`. Its carrier is
...
def srInitDIP :
  ...
  D_IP_salted.sample (StmtIn := StmtIn) (Salt := Salt) pSpec
...
omit [SpongeUnit U] [SpongeSize] [SaltCodec U δ Salt] codec [DecidableEq StmtIn] [DecidableEq U]
  [VCVCompatible StmtIn] [∀ i, VCVCompatible (pSpec.Message i)] [VCVCompatible Salt] in
...
theorem hybChallengeImpl_eq_srAddLift (oSpecImpl : QueryImpl oSpec ProbComp) :
  ...
  · -- challenge slot: `pure (k qC)`, matching `srChallengeQueryImpl'`.
    funext s
    simp only [hybChallengeImpl, srHyb4Impl, srChallengeQueryImpl']
    rfl
...
omit [VCVCompatible StmtIn] [∀ i, VCVCompatible (pSpec.Challenge i)] [SpongeUnit U]
  [SpongeSize] [VCVCompatible U] [∀ i, VCVCompatible (pSpec.Message i)] codec
  [VCVCompatible Salt] [DecidableEq StmtIn] [DecidableEq U] in
theorem withQueryLog_simulateQ_srReassoc_query
...
omit [VCVCompatible StmtIn] [∀ i, VCVCompatible (pSpec.Challenge i)] [SpongeUnit U]
  [SpongeSize] [VCVCompatible U] [∀ i, VCVCompatible (pSpec.Message i)] codec
  [VCVCompatible Salt] [DecidableEq StmtIn] [DecidableEq U] in
theorem withQueryLog_simulateQ_liftFS_query
...
omit [VCVCompatible StmtIn] [∀ i, VCVCompatible (pSpec.Challenge i)] [SpongeUnit U]
  [SpongeSize] [VCVCompatible U] [∀ i, VCVCompatible (pSpec.Message i)] codec
  [VCVCompatible Salt] [DecidableEq StmtIn] [DecidableEq U] in
theorem withQueryLog_simulateQ_liftFS {α : Type}
...
omit [SpongeUnit U] [SpongeSize] [DecidableEq StmtIn] [DecidableEq U] codec
  [VCVCompatible StmtIn] [∀ i, VCVCompatible (pSpec.Message i)] [VCVCompatible Salt] in
...
theorem expVerifyHandler_eq_hybChallengeImpl_compose_liftFS (oSpecImpl : QueryImpl oSpec ProbComp) :
  ...
  rcases q with qO | qC <;>
    ...
    simp [hybChallengeImpl, StateT.lift]
  all_goals rfl
...
theorem isQueryBoundP_simulateQ_of_step
...
theorem isSaltedFSChallengeBound_nargInducedProver
    (maliciousProver : MaliciousProver oSpec pSpec StmtIn U δ)
    (d2sAlgoTransform : D2SAlgoTransform (δ := δ) (Salt := Salt)
      (oSpec := oSpec) (StmtIn := StmtIn) (pSpec := pSpec) (U := U))
    {t : ℕ}
...
theorem isSaltedFSChallengeBound_srInducedProverKS
    (maliciousProver : MaliciousProver oSpec pSpec StmtIn U δ)
    (d2sAlgoTransform : D2SAlgoTransform (δ := δ) (Salt := Salt)
      (oSpec := oSpec) (StmtIn := StmtIn) (pSpec := pSpec) (U := U))
    {t : ℕ}
...
theorem basicFSCompiledKSBound_to_sr
    (d2sAlgoTransform : D2SAlgoTransform (δ := δ) (Salt := Salt)
      (oSpec := oSpec) (StmtIn := StmtIn) (pSpec := pSpec) (U := U))
    (tₕ tₚ tₚᵢ : ℕ)
...
omit codec [SaltCodec U δ Salt] [DecidableEq U] in
/-- Compiled-bound reduction preserving the supplied SR extractor. -/
theorem basicFS_straightlineKS_withCompiledBound_fixedExtractor
...
/-- `basicFSVerifierComp` is `fsSaltedVerify` routed through `liftFS`. -/
...
theorem hyb4_hdist
  ...
  simp only [srInitDIP, fsSaltedNIV_verify]
  rw [map_bind]
...
/-- **Theorem 6.1 of CO25** — Soundness of the duplex-sponge Fiat–Shamir scheme.
...
theorem duplex_sponge_fiat_shamir_soundness
  ...
  -- The DSFS NARG soundness experiment (Def 3.5) is the sponge game `dsfsGameDist` on the
