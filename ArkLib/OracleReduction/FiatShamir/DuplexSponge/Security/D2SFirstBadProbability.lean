...
omit [VCVCompatible U] [DecidableEq StmtIn] [DecidableEq U] [Nonempty U] [SampleableType U] in
/-- Closed form for the finite-run charge.  A length-`q` run sees the consecutive odd
numerators `2j + 1, 2(j + 1) + 1, …`, whose sum is `q (2j + q)`.  This exact identity lets the
eventual Lemma 5.8 arithmetic consume a whole runner without expanding an inductive sum. -/
lemma d2sQueryRunFirstBadCharge_eq
    (j : ℕ) (queries : List (duplexSpongeChallengeOracle StmtIn U).Domain) :
    d2sQueryRunFirstBadCharge (StmtIn := StmtIn) (U := U) j queries =
      ((queries.length * (2 * j + queries.length) : ℕ) : ℝ≥0∞) /
        BadEventDS.capacitySpaceSize (U := U) := by
  induction queries generalizing j with
  | nil =>
      simp [d2sQueryRunFirstBadCharge]
  | cons _ queries ih =>
      rw [d2sQueryRunFirstBadCharge, ih, ← ENNReal.add_div]
      congr 1
      norm_cast
...
/-- Aggregate the uniform one-step monitor-stop bound over the actual finite revised D2SQuery
runner.  The proof follows the executable `nil`/`cons` recursion and uses
`probEvent_bind_le_add` exactly once per list cell.  A monitor stop is charged at that cell; a
continuing step is charged only by the inductive suffix; and an underlying abort contributes zero.
Thus no union is taken over an unexecuted suffix, and no handler case split remains in the proof.

To instantiate this theorem for the live dispatcher, it remains only to prove
`D2SQueryStepContinueBaseLengthLe`; cache coherence of each continuation is already supplied by
`d2sQueryStepRevised_maintainsInvariant`. -/
lemma d2sQueryRunRevised_monitorStop_le_of_stepBaseLength
    (normal : D2SNormalState
      (δ := δ) (T_H := T_H) (T_P := T_P)
      (StmtIn := StmtIn) (pSpec := pSpec) (U := U))
    ...
    (hCoherent : RateOnlyCacheCoherent normal)
    (hBaseLength : (getBaseTrace normal.state.trace).length ≤ j)
    (hContinue : D2SQueryStepContinueBaseLengthLe
      (StmtIn := StmtIn) (pSpec := pSpec) (U := U) (δ := δ) (T_H := T_H) (T_P := T_P)) :
    Pr[ fun result => result.isMonitorStop |
      simulateQ
        (gImpl + ((d2sUnitSampleImpl (U := U)) + QueryImpl.id' unifSpec))
        (d2sQueryRunRevised normal queries)] ≤
      d2sQueryRunFirstBadCharge (StmtIn := StmtIn) (U := U) j queries := by
  classical
  induction queries generalizing normal j with
  | nil =>
      rw [d2sQueryRunRevised_nil, simulateQ_pure, probEvent_pure]
      simp [d2sQueryRunFirstBadCharge]
  | cons q queries ih =>
      let impl : QueryImpl
          (d2sQueryOracles (U := U) (StmtIn := StmtIn) (pSpec := pSpec) (δ := δ)) ProbComp :=
      ...
      let next := fun stepResult : D2SRevisedStepResult
        ...
        | .continue _ normal' => d2sQueryRunRevised normal' queries
        ...
        (by simpa only [not_not] using hStep) (by
          ...
          | «continue» answer normal' =>
              ...
              have hNextLength : (getBaseTrace normal'.state.trace).length ≤ j + 1 :=
                Nat.le_trans (hContinue normal q gImpl answer normal' hResult)
                  (Nat.succ_le_succ hBaseLength)
              simpa only [next, not_not] using
                ih normal' (j + 1) hInvariant.1 hNextLength
          | «stopped» _ _ =>
              simp only [D2SRevisedStepResult.isMonitorStop_stopped, not_true_eq_false] at hNotStop
          | «underlyingAbort» =>
              simp only [next, simulateQ_pure, probEvent_pure,
                D2SRevisedStepResult.isMonitorStop_underlyingAbort, not_false_eq_true,
                not_true_eq_false, ↓reduceIte]
              exact bot_le)
      simpa [d2sQueryRunRevised_cons, impl, next, d2sQueryRunFirstBadCharge] using hBound

/-- The actual revised D2SQuery dispatcher satisfies the branch-free finite-run first-bad bound.
Unlike `d2sQueryRunRevised_monitorStop_le_of_stepBaseLength`, this public form has no abstract
trace-growth premise: the deterministic dispatcher has already discharged it in
`d2sQueryStepRevised_continue_baseTrace_length_le`. -/
lemma d2sQueryRunRevised_monitorStop_le
    (normal : D2SNormalState
      (δ := δ) (T_H := T_H) (T_P := T_P)
      (StmtIn := StmtIn) (pSpec := pSpec) (U := U))
    (queries : List (duplexSpongeChallengeOracle StmtIn U).Domain)
    ...
    Pr[ fun result => result.isMonitorStop |
      simulateQ
        ...
        (d2sQueryRunRevised normal queries)] ≤
      d2sQueryRunFirstBadCharge (StmtIn := StmtIn) (U := U) j queries := by
  exact d2sQueryRunRevised_monitorStop_le_of_stepBaseLength normal queries gImpl j
    hCoherent hBaseLength
    (d2sQueryStepRevised_continue_baseTrace_length_le
      (StmtIn := StmtIn) (pSpec := pSpec) (U := U) (δ := δ) (T_H := T_H) (T_P := T_P))

/-- Closed-form live-run first-bad bound.  For `q` actually executed query positions from a
prefix of base-trace length at most `j`, the total monitor-stop charge is exactly bounded by
`q(2j + q)/|Σ|ᶜ`; no rounded sponge-block counter appears in this interface. -/
lemma d2sQueryRunRevised_monitorStop_le_closed
    (normal : D2SNormalState
      (δ := δ) (T_H := T_H) (T_P := T_P)
      (StmtIn := StmtIn) (pSpec := pSpec) (U := U))
    (queries : List (duplexSpongeChallengeOracle StmtIn U).Domain)
    ...
    Pr[ fun result => result.isMonitorStop |
      ...
      ((queries.length * (2 * j + queries.length) : ℕ) : ℝ≥0∞) /
        BadEventDS.capacitySpaceSize (U := U) := by
  rw [← d2sQueryRunFirstBadCharge_eq]
  exact d2sQueryRunRevised_monitorStop_le normal queries gImpl j hCoherent hBaseLength
