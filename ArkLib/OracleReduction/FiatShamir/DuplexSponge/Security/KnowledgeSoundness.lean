...
This file formalizes the randomized, verifier-trace-assisted variant of Construction 6.3's
reduction, using the Section 5 witness interface in `Soundness.lean` and `KeyLemma.lean`.
...
- `duplex_sponge_fiat_shamir_knowledge_soundness`: **Theorem 6.2 of CO25 (trace-assisted
  variant)**, uniform SR-KS implies uniform DSFS KS, with Lemma 5.1's additive error.
...
`duplex_sponge_fiat_shamir_knowledge_soundness` instead concludes
`adaptiveNARGKnowledgeSoundness` for the entire query-budget family.) -/
...
/-- The DSFS **randomized, trace-assisted knowledge-soundness game** (bespoke, query-bounded).
...
def dsfsKSGameDist
    -- Bare straightline-extractor shape (matching the trace-assisted experiment): the
    -- extractor's spec
  ...
  -- Same five-field read-out as the trace-assisted `adaptiveNARGKnowledgeSoundnessExp`, so the
  -- trace-assisted experiment/game bridge is definitional after the verifier wrapper normalizes.
...
noncomputable def ksFactKernel
  ...
  fun out => match out with
    ...
    | some result => do
        ...
        -- matching the coin-stripped `tr.fst` feed in the trace-assisted experiment.
        let witIn? ← simulateQ (d2sUnitSampleImpl (U := U))
...
`dsfsGame_falseAccept_eq_hyb0`: the DSFS trace-assisted KS game
...
`Hyb₄ >>= ksFactKernel E_std` and the coin-bearing basic-FS NARG trace-assisted KS experiment
(trace-assisted variant of Def 3.6) for `nargInducedProver`, verifier `fsSaltedVerify V`, and
...
/-- **Construction 6.3 in the randomized, trace-assisted NARG interface** — the straightline
...
This proves the randomized, verifier-trace-assisted variant, not determinism. -/
theorem dsfsStraightlineKS_of_basicFS_fixedExtractor
    ...
    (ε_sr : ENNReal)
    (hBasicFS : Verifier.adaptiveNARGKnowledgeSoundnessWithCoinsWithExtractor (WitIn := WitIn)
        (init := srInitDIP (StmtIn := StmtIn) (Salt := Salt) (pSpec := pSpec))
        ...
        ε_sr) :
    -- Randomized, verifier-trace-assisted NARG guarantee: every
  ...
  exact (by
    ...
    -- Step 0: the DSFS NARG-KS trace-assisted experiment IS the sponge KS game
...
/-- Fixed SR-to-DSFS extractor transport for the randomized, verifier-trace-assisted
variant. The constructed extractor contains no query-budget or error parameter. -/
theorem dsfs_randomized_trace_knowledge_soundness_of_fixedExtractor
  ...
  have hExact :=
    ...
    (hBasicFS := basicFS_straightlineKS_withCompiledBound_fixedExtractor
...
/-- **Theorem 6.2 of CO25 (trace-assisted variant)** — One SR extractor yields one DSFS
extractor for the entire budget family, assuming the corresponding Lemma 5.1 witnesses.
...
theorem duplex_sponge_fiat_shamir_knowledge_soundness
    ...
    (Verifier.dsfsNargNIV (U := U) δ V).adaptiveNARGKnowledgeSoundness
        (init := hyb0Init) (impl := hyb0Impl oSpecImpl)
        (auxImplE := d2sUnitSampleImpl (U := U)) relIn langOut
  ...
  refine ⟨dsfsNargExtractor (T_H := T_H) (T_P := T_P)
    (fun x π tr_P tr_V => OptionT.mk (pure (fsSRDelegatingExtractor E x π tr_P tr_V))), ?_⟩
  rintro ⟨tₕ, tₚ, tₚᵢ⟩
