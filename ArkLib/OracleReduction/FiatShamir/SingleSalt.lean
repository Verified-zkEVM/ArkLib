...
/-!
  ...
  - `single_salt_fiat_shamir_knowledge_soundness`: **Theorem 3.19 of CO25, adapted to
    extractors with both query logs and oracle access**, one extractor for the entire budget family.
    This does not establish the deterministic, prover-trace-only contract of Definition 3.6.
-/
...
paper's forwarding of well-formed FS queries to SR moves is the identity in this typed
challenge interface. Malformed queries are excluded by the query type. -/
...
private lemma fsKSExecution_failure_eq {Salt : Type} [VCVCompatible Salt]
    ...
    (P : OracleComp ((oSpec + fsChallengeOracle (StmtIn × Salt) pSpec) + auxSpec)
      (StmtIn × FSSaltedProof pSpec Salt)) :
    Pr[ nargKSFailEvent relIn langOut | do
  ...
  cases u
  cases wi? <;> cases so? <;> exact Iff.rfl
