...
/-!
  ...
  - `singleSaltFiatShamir_completeness`: completeness for Construction 3.17 of CO25
    with a fresh uniform salt; the proof is currently deferred.
  - `single_salt_fiat_shamir_soundness`: **Theorem 3.18 of CO25**, soundness from SR soundness.
  - `single_salt_fiat_shamir_knowledge_soundness`: **Theorem 3.19 of CO25 (trace-assisted
    variant)**, one extractor for the entire budget family. This does not establish the
    deterministic, prover-trace-only contract of Definition 3.6.
-/
...
omit [VCVCompatible StmtIn] [∀ i, VCVCompatible (pSpec.Challenge i)]
  [DecidableEq StmtIn] [∀ i, DecidableEq (pSpec.Message i)]
  [∀ i, DecidableEq (pSpec.Challenge i)] in
/-- **Theorem 3.18 of CO25** — Single-salt FS soundness from IP SR-soundness.
...
/-- **CO25 Construction 3.19 adaptation (pure, two-log, computable wrapper)**:
...
delegating extractor, the coin-bearing trace-assisted NARG KS experiment for the single-salt FS
...
The conclusion is the explicitly randomized, verifier-trace-assisted NARG variant. -/
theorem single_salt_fiat_shamir_knowledge_soundness_of_fixedExtractor
    ...
    (h_sr_ks : Verifier.StateRestoration.knowledgeSoundnessWithCoinsWithExtractor
        ...
        E srBound ε) :
    -- Adaptive, coin-bearing, verifier-trace-assisted KS of the single-salt FS argument,
    -- phrased as a property of the NARG verifier `Verifier.singleSaltFiatShamir V`.
    Verifier.adaptiveNARGKnowledgeSoundnessWithCoinsWithExtractor (WitIn := WitIn)
      ...
      (bound := bound) ε := by
  intro P hP
  refine le_trans (le_of_eq ?_) (h_sr_ks (srInducedProverKS P) (hBound P hP))
...
omit [VCVCompatible StmtIn] [DecidableEq StmtIn]
  [∀ i, DecidableEq (pSpec.Message i)] [∀ i, DecidableEq (pSpec.Challenge i)] in
/-- **Theorem 3.19 of CO25 (trace-assisted variant)** — One SR extractor yields one NARG
extractor for the entire bound/error family. The conclusion permits randomness and both logs;
it does not establish the deterministic, prover-trace-only contract of Definition 3.6. -/
