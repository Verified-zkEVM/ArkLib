...
The NARG-KS APIs below are randomized, verifier-trace-assisted variants: extractors can use
`auxImplE` and receive both logs. They do not assert the deterministic, prover-trace-only
contract stated in CO25 Definition 3.6. Construction 6.3's verifier trace and randomized
LookAhead require a separate reconciliation with that definition. The extractor has no
prover handle: it is non-rewinding, but this does not establish deterministic straightline KS.
No polynomial-time or rewinding-security guarantee is formalized here.
...
/-- **Trace-assisted variant of CO25 Def 3.6** — the NARG-KS game: the prover outputs
...
def adaptiveNARGKnowledgeSoundnessExp
  ...
  -- challenge oracle, or `P`; it reads challenges from `tr, tr_V` and may use fresh randomness.
  -- This is a trace-based variant, not a proof of determinism given the trace.
...
/-- Coin-bearing trace-assisted KS variant: like `adaptiveNARGSoundnessExpWithCoins`, it captures
...
/-- Fixed-extractor, randomized trace-assisted knowledge soundness against provers with private
coins. The extractor receives both logs; this is not the deterministic, prover-trace-only
contract of CO25 Definition 3.6. -/
def Verifier.adaptiveNARGKnowledgeSoundnessWithCoinsWithExtractor
    ...
    (verifier : NonInteractiveVerifier Proof oSpec StmtIn StmtOut)
    (relIn : Set (StmtIn × WitIn)) (langOut : Set StmtOut)
    (extractor : (stmtIn : StmtIn) → (π : Proof) → (tr_P : QueryLog oSpec) →
      (tr_V : QueryLog oSpec) → OptionT (OracleComp auxSpecE) WitIn)
    (bound : OracleComp (oSpec + auxSpec) (StmtIn × Proof) → Prop) (error : ENNReal) :
    Prop :=
  ∀ P : OracleComp (oSpec + auxSpec) (StmtIn × Proof), bound P →
    Pr[ nargKSFailEvent relIn langOut
...
uniform existential closure of the fixed-extractor predicate. -/
