...
theorem Verifier.id_soundness {lang : Set StmtIn} :
  ...
  -- The identity verifier returns its input statement unchanged (`Reduction.run_mk_verifier_id`),
  -- so every output statement is `stmtIn`; since `stmtIn ∉ lang`, the bad event has probability 0.
  ...
  obtain ⟨s, _, hx⟩ := hx
  -- The reduction's output statement is always `stmtIn`.
  rw [Reduction.run_mk_verifier_id] at hx
  simp only [liftM_map, OptionT.run_map, simulateQ_map, StateT.run'_map', support_map,
    Set.mem_image, Option.map_eq_some_iff] at hx
  -- Every output is `(_, stmtIn)`, so `x.2 = stmtIn ∉ lang`.
  obtain ⟨_, _, a, -, rfl⟩ := hx
  exact hIn
