...
% A code is modelled directly as `Set (ι → F)`; the `ListDecodable.Code` abbreviation for it was
% removed in #722, so this node has no single Lean declaration to point at.
...
\begin{definition}[Smooth Reed-Solomon Code]
    ...
    \lean{Domain.SmoothCosetFftDomain}
\end{definition}
