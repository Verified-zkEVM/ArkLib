...
\end{theorem}
\begin{proof}\leanok
Expand in the actual finite bases, interchange finite sums, and apply the basis inverse laws.
\end{proof}
...
\begin{definition}[Checked observation]
  ...
  witness. Concrete adapters prove this identity and the correspondence with their existing
...
\end{theorem}
\begin{proof}\leanok
Instantiate checked observation with the actual layout reconstruction and inverse component
assembly, preserving the packed commitment through the proved original-relation equivalences.
\end{proof}
...
\begin{theorem}[Separation for an actual compatibility relation]
  \lean{RingSwitching.Packing.FullFamily.compatibility_badEvent_le}
  \leanok
...
\begin{theorem}[Native tensor head as shared packing]
  ...
  \label{thm:native_packing_head}
  The actual native Boolean-table packing agrees with the shared DP24 layout and has both
  packing inverses. Its tensor message is the finite observation in faithful carrier
  coordinates. Its scalar guard reads back the original source evaluation, and its exact
  round-zero residual consistency is the shared challenged sumcheck relation.
\end{theorem}
\begin{proof}\leanok
Prove the prefix/suffix index transport and polynomial correspondence, then apply finite
observation to the actual embedded packed evaluation. Instantiate checked observation with
the native packing equivalence. Identify the native multiplier polynomial and Boolean sum
with the shared multiplier and slice target.
\end{proof}

\begin{theorem}[Native batching completeness and knowledge]
  ...
  \leanok
  \uses{thm:native_packing_head, thm:packing_compatibility_separation}
  \label{thm:native_batching_security}
  The native one-tensor-message/vector-challenge head has state-aware perfect completeness.
  Under its explicit functional compatibility and finite-domain challenge hypotheses, its
  actual extractor and knowledge states have worst-case-per-prefix error $\kappa/|L|$.
  The output is the unchanged round-zero relation, retaining the same committed polynomial.
\end{theorem}
\begin{proof}\leanok
Use the exact guarded execution equation and native observation identity for honest runs.
At the message boundary apply checked read-back; at the output boundary recover the structural
residual relation. Include the native challenge bad event in the shared compatibility
...
\begin{theorem}[Production FRI-Binius batching instantiation]
  ...
  \leanok
  \uses{def:binary_tower_profile, thm:native_batching_security}
  \label{thm:binius_native_batching}
  Instantiate the native head with the actual Binius profile and first-codeword compatibility
  relation, supplying its proved unique-distance binding. Completeness reaches precisely the
  existing interleaved-core input relation. Knowledge security retains the same native
  extractor, state and error. The existing full completeness assembly consumes this head
  specialization, but still has downstream admitted dependencies.
\end{theorem}
\begin{proof}\leanok
Apply the native theorems with the production parameters and
\texttt{binaryBasefold\_functional}; no replacement commitment relation is introduced.
\end{proof}
...
its exact execution and structural witness reconstruction. Native batching now has the shared
proofs above; the remaining legacy loop completeness and knowledge leaves are admitted. They
...
\begin{theorem}[Actual FRI-Binius terminal rejection]
  ...
  The actual FRI-Binius terminal verifier rejects a failed multiplier equation and remains
...
\end{theorem}
\begin{proof}\leanok
Interpret the actual guarded message verifier and apply absorbing failure to oracle append.
\end{proof}

Inhabited GF(16) clients check both rejection and nonzero forwarding. These are local execution
results; the existing interleaved final completeness and knowledge admissions, and downstream
FRI opening security, remain separate proof obligations.
...
\begin{theorem}[Exact scalar trace check]
  ...
  For every sent ring value, the actual scaled-trace guard is equivalently the weighted
  observation of its $\psi$ coordinates, with the same monomial index transport.
\end{theorem}
\begin{proof}\leanok
Identify the actual $\psi$ coordinates and monomial indices, apply finite-observation read-back
...
\begin{theorem}[Shared reconstruction in the actual trace head]
  ...
  The actual one-message trace head has perfect completeness and zero-challenge CWSS,
  preserving the same commitment and norm-conditioned weak opening at the existing
  ring-evaluation relation. Its checked-observation instance has the identity witness
  equivalence and the proved monomial observation identity.
\end{theorem}
\begin{proof}\leanok
Use the actual guard and relation equivalences with the shared honest-check and read-back
lemmas, then the existing guarded execution and zero-challenge CWSS framework. The weak-opening
predicate, trace scale and norm parameters remain unchanged.
\end{proof}
