...
**Status: landed** (2026-08-10, branch `tr/computable-extractors`). All ten milestones M0–M9 are
green; §11 carries each one's gate output. The goal is met: the inventory probe (§9 note 2) reports
**0** noncomputable definitions over the CWSS surface, against the baseline's 20, and the migration
shim (§4.5) is gone. What follows is the design record — read §4 for why the notion has the shape it
has. The living regression gate for that shape is
`ArkLib/OracleReduction/Security/TranscriptTree/NonVacuity.lean`, in the library build; the
prototypes under [`prototypes/`](prototypes/README.md) are now historical evidence.
...
is non-empty (M7's data), `rg -n '^def treeExtractor '
ArkLib/ProofSystem/Component/ReduceClaim.lean` is non-empty (M6's canonical witness-only engine),
and `rg -n 'noncomputable def cyclotomicPresentation' ArkLib/` returns nothing (M1).

**M6 left three outgoing twins for this milestone to retire** (its §11 row records why): the
`*PackageClassical`-typed Hachi literals below cannot take a canonical extractor, so M6 renamed
`ReduceClaim.{treeExtractor, verifier_coordinateWiseSpecialSoundWith}` and
`RingSwitching.Lift.{treeExtractor, package}`'s outgoing forms to `*Classical` and repointed the
seven Hachi sites at them. When this milestone retypes those packages, repoint the sites back to
the canonical names and **delete** `ReduceClaim.{treeExtractorClassical,
verifier_coordinateWiseSpecialSoundWithClassical}` and
`RingSwitching.Lift.{treeExtractorClassical, coordinateWiseSpecialSoundWithEscapeClassical,
packageClassical}` in the same commit — they are in `ProofSystem/**`, which M9's entry probe
scans, so leaving them fails that probe. (Its oracle sibling `ReduceClaim.oracleTreeExtractor`
needed no twin: nothing outside its file names it.)
...
| M4 | green | two commits, `feat(CWSS): sequential composition of witness-only extractors` (4a) and `feat(CWSS): CWSS packages at the witness-only extractor` (4b) — SHAs to be filled by the repository owner (committing is disabled in-session; the session hands over `scratchpad/m4/commit-m4.sh`, which reverse-applies `plan-log.patch` and `4b-packages.patch` to recover the 4a tree, commits 4a, re-applies each and commits 4b and the log update, taking each message from a `msg-*.txt` beside it via `-F`; both patches were verified to reverse-apply cleanly from the final tree, and the 4a tree was **built separately** — `Build completed successfully (4149 jobs)` — so the two commits bisect) | **Entry probe:** `rg -n 'toShape_arity_pos' ArkLib/` → `CWSS/Basic.lean:204`; `rg -c 'gluePath' …/TranscriptTree/Composition.lean` → `22`. Ground green before the first edit: `Build completed successfully (4149 jobs)`. **Step 4a gate — IR:** `IR PRESENT: ProtocolSpec.ChallengeTree.somePath`, `IR PRESENT: Extractor.TreeBased.append`, `IR PRESENT: Verifier.GuardedForm.append`, plus the three §9-note-1 decisive probes (`def probeSomePath := @…`, `probeTreeBasedAppend`, `probeGuardedFormAppend`, all elaborated outside any `noncomputable section`) — six lines, no `NO IR`. `ProtocolSpec.ChallengeTree.somePath` **depends on no axioms at all**. **Step 4a gate — axioms:** `#print axioms` on all 16 new tree/CWSS/guarded declarations carries no `sorryAx`, in particular **`Verifier.append_coordinateWiseSpecialSoundWithEscape_of_guardedLeft` → `[propext, Classical.choice, Quot.sound]`** — the retyped `Guarded.lean:141`, **proved** (E15 transcribed), together with its three siblings `append_treeSpecialSoundWith{,_guardedLeft}` and `append_treeSpecialSoundWithEscape{,_guardedLeft}` (E12–E15) and the five guarded seam lemmas. The two lone exceptions are `OracleVerifier.append_coordinateWiseSpecialSoundWith{,Escape}`, which carry `sorryAx` **inherited from the pre-existing sorried `OracleVerifier.append_toVerifier`** — their `*Classical` predecessors already do, verified side by side; the M4 diff adds no `sorry` of its own. **Step 4b gate — IR:** twelve `IR PRESENT` lines — the four canonical package appends (`CWSSPackage.append`, `GCWSSPackage.append`, `EscapeCWSSPackage.append`, `EscapeGCWSSPackage.append`), their four decisive probes, and the four lattice lifts (`CWSSPackage.toGuarded`, `CWSSPackage.toEscape`, `GCWSSPackage.toEscape`, `EscapeCWSSPackage.toGuarded`) — and four **expected** negatives, `NO IR AS EXPECTED (shim lift, pureFormOfIsPure): CoordinateWise.{CWSSPackage,GCWSSPackage,EscapeCWSSPackage,EscapeGCWSSPackage}.ofClassical`, exactly as 4b's gate text prescribes (a lifted package's purity field is `Classical.choice`; recorded here so a later IR sweep does not misread it). **Step 4b gate — runtime:** a toy pure chain built **directly at the new types** (`emptyStruct`/`pureVForm` over `!p[]`, `relIn := Set.univ`, certificates discharged in one line — E25's calibration: the `Prop`/`Set` fields are erased and never block codegen) composed through the canonical `▷`: `#eval chain.extractor () ChallengeTree.leaf (fun _ => some 5)` → `some 11` (the top witnessing reaches the right engine, which doubles at the seam verdict `true`, and the left engine adds one — so `L₁.isPure.verify` is demonstrably on the executable path, not merely present), `(fun _ => none)` → `none` (declining leaves decline the chain), `#eval chain.isPure.verify () (fun i => i.elim0)` → `false` (the composed seam verdict of `PureForm.append` runs). Both are kernel-`rfl`-checked (`chain_eval`, `chain_none`, axioms `[propext, Classical.choice, Quot.sound]`, no `sorryAx`), and `IR PRESENT` on `leftPkg`, `rightPkg`, `chain`. **`▷` dispatch, both directions:** `leftPkgClassical ▷ rightPkgClassical` type-checks at an ascribed `CWSSPackageClassical` (so every un-migrated consumer keeps its type — the whole-repo build is the real witness), while `leftPkg ▷ rightPkgClassical` and `leftPkgClassical ▷ rightPkg` type-check at `CWSSPackage`, i.e. a half-migrated chain composes. Both mixed chains must be marked `noncomputable` — the `ofClassical` lift goes through `pureFormOfIsPure` — which is why no `#eval` is gated on them. **Prototypes re-run, all four green** (§1 rule 3's standing choice: patch the prototypes): `CM_gates` → `some 42`, `some 5`, `some 4`, five `IR PRESENT`; `CM_append` → `some 11`, `some 211`, ten `IR PRESENT`; `CM_enginecerts` → `some 21`, `none`, seven `IR PRESENT`; `CM_presentation` → `2, 3, 5, 2, 2, some 2`, six `IR PRESENT` + `NO IR AS EXPECTED (the negative control): CMP.liftPackageLikeM`; **zero `sorryAx` mentions in all four**. `CM_append`'s Part E copy of `somePath` was deleted in favour of the library one, so its IR gate now reports `IR PRESENT: ProtocolSpec.ChallengeTree.somePath` against the **library** declaration and both guarded theorems exercise it. **Exit gate — `./scripts/validate.sh`:** `Build completed successfully (4149 jobs)`; `No ArkLib/Data non-sorry warnings found.`; `✓ All imports are up to date!` over `325 imports`; `All documentation integrity checks passed.`; `Knowledge base lint passed.` All six touched `.lean` files emit **zero** messages under `lake env lean`, except `Guarded.lean`'s two expected `declaration uses 'sorry'`. **Zero new long lines** (codepoint-counted per the M3 note; the rename-free additions still needed a rewrap pass, since 100-column prose is easy to overrun). **Sorry census unchanged**, verified at the diff level rather than by counting tokens (three docstrings now contain the word): exactly one real `sorry` added — `Verifier.GuardedForm.append`'s `verify_eq` — and exactly one removed, `Verifier.IsGuarded.append`'s, which is now *proved* from it. `Guarded.lean` therefore still has two, and after M9 deletes the `*Classical` twin it will have one: the plan's "comes out with one fewer `sorry`", on schedule. **Deviations / notes:** (a) *Home of the guarded seam lemmas.* 4a's text sends `append_run_guardedLeft`, `append_run_outputs_guardedLeft`, `outputs_guarded_subsingleton`, `guarded_accepting_of_mem`, `guarded_verdict_mem_outputs` to `CWSS/Composition.lean`; that is impossible, and the plan's own constraints say so. All five take `hV₁ : V₁.IsGuardedWith`, and `IsGuardedWith` lives in `Guarded.lean`, which is **downstream** of `Composition.lean` (`Basic → Composition → Package → Guarded → Escape`), while §4.4 pins the `IsGuarded*` classes as untouched. The five lemmas and the two generic tree-level guarded theorems (E14, E15) are therefore homed in `Guarded.lean`, next to the class they mention. The *pure* seam lemma `append_run_outputs` does land in `CWSS/Composition.lean` as specified. (b) *Home of `somePath`.* Placed in `TranscriptTree/Basic.lean` beside `onlyPath` rather than in `TranscriptTree/Composition.lean`: it is `onlyPath`'s positive-arity sibling, same shape and same purpose, and M0 deviated the same way for the same reason (`make-computable.md` step 4: a new executable algorithm belongs next to its structure). Strictly *shallower* than the plan's home, so nothing that needs it is out of reach. (c) *The plain guarded CWSS append is proved from E14 directly*, not as a corollary of the escape one at never-firing events (the route the `*Classical` predecessor takes). Both generic theorems are proved now, so each CWSS wrapper instantiates its own via `treeSpecialSoundWith_congr` — which is also why the `rcases … with (hf | ⟨_, hf⟩) | hwit` the plan flagged as unsurvivable simply has no successor. (d) *The `IsGuarded.append` sorry relocates rather than duplicating.* 4a's text says "the sorry moves, the census count does not"; read literally, that means `IsGuarded.append` must stop being sorried, so it is now the three-line forgetful corollary of `GuardedForm.append`. (e) *Two sanctioned shim additions the plan does not name*, both mirrors of ones it does, both inside explicitly-labelled shim docstrings and both deleted by M9 with the rest: `Verifier.treeSpecialSoundWithEscape.new_of_old` (the escape twin of 2b's `new_of_old`, required by the two escape-package `ofClassical` lifts) and `Verifier.guardedFormOfIsGuarded` (the `pureFormOfIsPure` mirror, required by the two guarded ones). (f) *`▷` dispatch order.* §4.5 describes canonical-first with a fall-through to the `*Classical` table "if it cannot". Implemented instead as **deterministic dispatch by kind**: two outgoing factors take the outgoing table, so an un-migrated definition still checks against its ascribed `*PackageClassical` type; every other combination takes the canonical table, lifting an outgoing factor with `ofClassical`. Canonical-first would retype *every* existing `▷` in `ProofSystem`/`Hachi` — all of which ascribe `*Classical` types — and recovering from that needs expected-type-driven backtracking inside the elaborator, well past R2's one-hour budget, for no capability the kind dispatch lacks (the migration case is exactly the mixed one, and it works — see the `▷` evidence above). Consequently `packageKindOf` keeps its `Expr → TermElabM Name` type and the lifting lives in two new private helpers, `canonLiftOf` and `liftFactor`, which M9 deletes alongside `univAppendFn`. (g) *The kind-marked synonyms `▷ᵍ`/`▷ₑ`/`▷ₑᵍ` still name the `*Classical` appends.* They are single-target notations, so canonical twins would need fresh tokens or would reintroduce the overload-`choice` blow-up the universal `▷` exists to avoid; M9 repoints them when it deletes the twins. (h) *Commit granularity*: two commits, one per lettered step (§1 rule 5), not one per file group — though each of 4a's three file groups was built green in isolation as it landed. (i) `Package.lean`, `Guarded.lean` and `Escape.lean` keep their `noncomputable section`s: the canonical packages' data gets IR inside one anyway (gated above, and re-checked with the decisive probe *outside* every section), and deleting a section belongs to the milestone that cleans its file (M5 for `CommittedScalar.lean`, M8's sweep for the rest). (j) `CWSSStructure.toShape_arity_pos` discharges D5's added `harity₂` at both downstream package call sites (`GCWSSPackage.append`, `EscapeGCWSSPackage.append`); the two CWSS-level canonical theorems carry the hypothesis explicitly, as D5 prescribes. **Abort criteria not triggered:** no transcription failed. §9 note 6's `appendArity` friction never surfaced beyond the prototype's own `key`-`have` pattern, which transcribed verbatim; R1's `toShape_append` `HEq` transport went through by `HEq.rfl` in all four CWSS wrappers, first try; §9 note 8's `unusedSectionVars` cost four `omit`s in `Guarded.lean` and one dropped section-variable instance (the two generic guarded theorems need neither spec's `SampleableType`). |
| M5 | green | five commits, `feat(CWSS): witness-only single-round engine and certificates` (step 1), `feat(CWSS): witness-only scalar-round engine and committed-scalar package` (steps 2–3), `feat(CWSS): witness-only no-challenge bridges, special soundness and its implications` (steps 4–6), `docs(plans): vendor the M5 runtime demo` (step 7), `docs(plans): record M5 green in the computable-extractor execution log` — SHAs to be filled by the repository owner (committing is disabled in-session; the session hands over `scratchpad/m5/commit-m5.sh`, which stages each commit's *disjoint* file set in order, taking each message from a `msg-*.txt` beside it via `-F`. No patch surgery was needed: M5's steps partition the diff by file. Both intermediate trees were **built separately** — `Build completed successfully (4149 jobs)` after step 1 alone and after steps 1–3 — so the commits bisect, and the restored tree was verified byte-for-byte identical to the final one) | **Entry probe:** `rg -n 'TreeBased.append' …/TranscriptTree/Composition.lean` → `1131:def TreeBased.append (verify₁ : Stmt₁ → pSpec₁.FullTranscript → Stmt₂)`; `rg -n 'PureForm' …/CWSS/Package.lean` → non-empty (`150:  isPure : verifier.PureForm`). Ground green before the first edit: `Build completed successfully (4149 jobs)`. **Exit gate — `./scripts/validate.sh`:** `Build completed successfully (4149 jobs)`; `No ArkLib/Data non-sorry warnings found.`; `✓ All imports are up to date!` over `325 imports`; `All documentation integrity checks passed.`; `Knowledge base lint passed.` **Step 7 demo — the milestone's one constructive deliverable, vendored as `docs/plans/prototypes/CM_m5demo.lean`:** a real `CWSSPackage` (`extractor := SingleRound.treeExtractor mk`, `isCWSS := coordinateWiseSpecialSoundWith_of_mkWitness`, `struct := foldStructure` at `r = 0`, purity as `PureForm` data) composed through the **canonical** `▷` with a synthetic zero-round closing package certified by `coordinateWiseSpecialSoundWith_of_isEmpty_challengeIdx`. Output: `#eval chain.extractor 3 T (fun _ => none)` → **`some 17`** — note the **constant-`none`** top-level witnessing: the tail closes the recursion, so §4.6's "a chain closed by a terminal link runs as a computable function of `(stmtIn, tree)` alone" is exhibited literally rather than argued. `#eval leftPkg.extractor 3 T1 (fun _ => none)` → **`none`** (the *open* engine declines instead of inventing junk) and `(fun _ => some 7)` → **`some 17`** (it reads the witnessing at both `branchPathOf` paths). `#eval (chain.isPure.verify 3 trT).2.1` → **`5`**: `PureForm.append`'s composed seam verdict reads the composite transcript at runtime. All four kernel-`rfl`-checked (`chain_eval`, `left_none`, `left_some`, `seam_reads_message`), plus `chain_extracts : ∃ w, chain.extractor 3 T (fun _ => none) = some w ∧ (3, w) ∈ chain.relIn := ⟨17, rfl, rfl⟩` — the notion's own conclusion, discharged by `rfl` against a concretely-defined `relIn` (constraint 2). **IR gate (the demo's own `run_cmd`):** fourteen `IR PRESENT` lines, no `NO IR` — `CoordinateWise.SingleRound.{treeExtractor, branchPathOf, collect}`, `CoordinateWise.ScalarRound.{treeExtractorScalar, branchPathOf, readFam}`, `CoordinateWise.CommittedScalar.{treeExtractor, package, verifierPureForm}`, and `CMM5Demo.{leftPkg, tailPkg, chain, T, T1}` — plus the four §9-note-1 decisive probes (`def probeSingleRoundTreeExtractor := @…`, `probeScalarTreeExtractor`, `probeCommittedScalarTreeExtractor`, `probeCommittedScalarPackage`) elaborated outside any `noncomputable section`. **Axioms:** all fourteen prints free of `sorryAx`; `CoordinateWise.ScalarRound.treeExtractorScalar` depends on `[propext, Quot.sound]` — **no `Classical.choice` at all** — and the rest carry it only through `Prop`-side instance arguments (E25's calibration point, erased at codegen). **Inventory probe (§9 note 2) re-run at M0's exact target list, now resolving to the CANONICAL names:** exactly **4** noncomputable definitions remain on that surface — `CoordinateWise.{CWSSPackage, GCWSSPackage, EscapeCWSSPackage, EscapeGCWSSPackage}.ofClassical`, i.e. only the shim lifts 4b's gate text already predicted (`pureFormOfIsPure`), which M9 deletes. A widened probe over *both* layers lists 24: those 4, the 12 Hachi definitions (M8), `ReduceClaim.{treeExtractor, oracleTreeExtractor}` and `RingSwitching.Lift.{treeExtractor, package}` (M6), and the four new outgoing twins below. **`noncomputable section` deletions:** `CommittedScalar.lean`'s (**mandatory** per the exit gate) is gone, and so are `SpecialSoundness.lean`'s and `Implications.lean`'s — all three files build clean without them. **Prototypes re-run, all four green** (§1 rule 3's standing choice; **no patch was needed this time** — M5's transcriptions land in `CoordinateWise.*`, which does not collide with the prototypes' `CM`/`CMSR`/`CMSC` namespaces): `CM_gates` → `some 42`, `some 5`, `some 4`, five `IR PRESENT`; `CM_append` → `some 11`, `some 211`, ten `IR PRESENT`; `CM_enginecerts` → `some 21`, `none`, seven `IR PRESENT`; `CM_presentation` → `2, 3, 5, 2, 2, some 2`, six `IR PRESENT` + `NO IR AS EXPECTED (the negative control): CMP.liftPackageLikeM`; zero `sorryAx` in all four. **Sorry census unchanged**, verified at the diff level: the M5 diff adds and removes **zero** `sorry` tokens (297 under `ArkLib/` before and after). **Zero new long lines** (codepoint-counted per the M3 note). All eight touched `.lean` files emit **zero** messages under `lake env lean`, except `Implications.lean`'s eight pre-existing `declaration uses 'sorry'` warnings in the round-by-round section M5 never touches. **Deviations / notes:** (a) *The engines needed `*Classical` twins that §4.5's rename table does not list.* 2a suffixed `SingleRound`/`ScalarRound`'s `*_of_mkWitness*` **certificates** but not the `treeExtractor` **engines** they name; retyping an engine in place therefore breaks its own outgoing certificate, and with it `QuadEval/Soundness.lean` (M8) and `RingSwitching/Lift/Reduction.lean` (M6). So M5 applies §4.5's own discipline one step further and renames four more declarations to the outgoing layer: `SingleRound.treeExtractorClassical`, `ScalarRound.treeExtractorScalarClassical`, `CommittedScalar.treeExtractorClassical`, `CommittedScalar.packageClassical` (its `coordinateWiseSpecialSoundWithEscapeClassical` was already suffixed at 2a). All four sit under explicit `## The outgoing …` headers, and M9's suffix grep sees them, so they are deleted with the rest of the shim. This is the one place M5 touches files outside its own set: three token repoints, `QuadEval/Soundness.lean` (3 sites) and `Lift/Reduction.lean` (1 site). (b) *`CommittedScalar.lean` keeps two explicitly-marked `noncomputable` declarations*, against the exit gate's "nothing in that file is legitimately noncomputable": `treeExtractorClassical` and `packageClassical` must survive until M6 migrates `Lift/Reduction.lean`. The gate's actual purpose is met — the **section** is deleted, so the two are visible to every later sweep instead of hidden, and the canonical `treeExtractor`/`package` both gate `IR PRESENT`. (c) *`CommittedScalar.treeExtractor` drops its `checkAt` argument* (`hk K project`, was `hk K checkAt project`): the branch openings now come from the witnessing, so `rel K checkAt` is no longer read anywhere in the extractor. The outgoing twin keeps the old argument list. (d) *The `NoChallenge` bridges need no purity and no `isValid_iff_pure`*, contrary to step 4's parenthetical. The extractor `fun stmtIn tree _ => some (e stmtIn tree.onlyPath.fullTranscript)` never consults its witnessing, so the validity premise is simply unused and today's `h` hypothesis discharges the conclusion directly — strictly more general than a purity-gated restatement, and exactly the *closing* shape §4.6 wants on the right of a chain (the step-7 demo's `tailPkg` is built from it). (e) *Step 5's "textbook recovery corollary" is stated with its conclusion written out*, as `Verifier.specialSound.old_of_new`, following 2b's precedent for `old_of_new`: it produces a total `E : StmtIn → ChallengeTree … → WitIn` correct on every structured accepting tree, rather than routing through `specialSoundClassical`, so it survives M9 unchanged. It costs `[Inhabited WitIn]` and nothing else. (f) *`Implications.lean` gains outgoing twins of both bridges* (`coordinateWiseSpecialSoundClassical_ofSpecialSoundClassical_iff`, `Verifier` + `OracleVerifier`) rather than being retyped in place, since §1 rule 7 forbids leaving the canonical names pointing at the outgoing notions and §4.5 keeps the outgoing bridges available until M9. (g) *`SingleRound.collect` is homed in `SingleRound.lean` and reused by `ScalarRound`* (which imports it), exactly as the prototype's `CMSR.collect` is reused by `CMSC`. (h) *`branch_relOut_language` survives in both engine files* although only the outgoing certificates now use it: it is a true, documented lemma about the shape at a pure verifier, and D4/§4.4's "untouched things stay" applies. After M9 it will have no consumers. (i) *The prototypes were not edited.* Unlike M0/M2/M3/M4 there is no `has already been declared` collision, because M5's declarations land in `CoordinateWise.SingleRound`/`ScalarRound` while the prototype's copies live in `CMSR`/`CMSC`. `CM_enginecerts.lean` therefore stays whole — it is still M6's transcription source for Part B, and §6's E27–E29 stay reproducible verbatim. (j) *Commit granularity*: five commits over disjoint file sets rather than one per numbered step — steps 2–3 cannot bisect apart (retyping `ScalarRound.treeExtractorScalar` breaks `CommittedScalar.treeExtractor` in the same breath) and steps 4–6 are three independent additive file edits with no ordering between them. **Abort criteria not triggered:** no transcription failed. §9 note 11's support-membership friction never surfaced (the certificates consume `isValid_iff_pure`, which M2 already landed with the `mem_support_bind_iff` route baked in); note 12's `change` move was needed exactly where the prototype uses it (`collect_branch_data`, twice) and note 13's post-`rw [hcol]` `rfl` in all four certificates, both transcribed verbatim. E29's predicted `Fin`-numeral friction *did* fire, on the demo's composite tree — resolved not with `Fin.cast` but by using M4's `ChallengeTree.somePath` at M3's `CWSSStructure.toShape_arity_pos`, which needs no numeral at an unreduced arity projection. |
| M6 | green | three commits, `feat(CWSS): witness-only ReduceClaim engines and certificates` (step 1 + the six Hachi repoints), `feat(CWSS): witness-only zero-round component statements` (step 2: `CheckClaim`, `SendClaim`, `SendWitness`), `feat(CWSS): computable Lift delegate and package` (step 3 + the `liftPackage` repoint), plus `docs(plans): record M6 green in the computable-extractor execution log` — SHAs to be filled by the repository owner (committing is disabled in-session; the session hands over `scratchpad/m6/commit-m6.sh`, which stages each commit's *disjoint* file set in order, taking each message from a `msg-*.txt` beside it via `-F`. No patch surgery was needed: M6's steps partition the diff by file. Both intermediate trees were **built separately** — `Build completed successfully (4149 jobs)` after step 1 alone and after steps 1–2 — so the commits bisect) | **Entry probe:** `rg -n 'branchPathOf' …/CWSS/SingleRound.lean` → non-empty (`264:def branchPathOf (tree : ChallengeTree (pSpec CarrierCom C r) arity 0)`); `rg -n 'noncomputable def cyclotomicPresentation' ArkLib/` → nothing. Ground green before the first edit: `Build completed successfully (4149 jobs)`. **Exit gate — `./scripts/validate.sh`:** `Build completed successfully (4149 jobs)`; `No ArkLib/Data non-sorry warnings found.`; `✓ All imports are up to date!` over `325 imports`; `All documentation integrity checks passed.`; `Knowledge base lint passed.` **IR gate:** eight `IR PRESENT` lines, no `NO IR` — `ReduceClaim.treeExtractor`, `ReduceClaim.oracleTreeExtractor`, `RingSwitching.Lift.treeExtractor`, `RingSwitching.Lift.package`, plus the four §9-note-1 decisive probes (`def probeRcTreeExtractor := @ReduceClaim.treeExtractor`, `probeRcOracleTreeExtractor`, `probeLiftTreeExtractor`, `probeLiftPackage`) elaborated outside any `noncomputable section` (neither file has one). **Axioms — E17 reproduced exactly, the milestone's abort criterion:** `'ReduceClaim.treeExtractor' depends on axioms: [propext, Quot.sound]` and `'ReduceClaim.oracleTreeExtractor' depends on axioms: [propext, Quot.sound]` — **no `Classical.choice` at all**, matching the prototype's `'CMEngines.rcTreeExtractor' depends on axioms: [propext, Quot.sound]` byte-for-byte; the *certificates* `ReduceClaim.{verifier, oracleVerifier}_coordinateWiseSpecialSoundWith` carry `[propext, Classical.choice, Quot.sound]`, which E17 says is legitimate (Mathlib's `probEvent_eq_one_iff`). `RingSwitching.Lift.{treeExtractor, package}` carry `Classical.choice` only through `Prop`-side instance arguments (E25's calibration point — `Rq`'s ring instances; the IR gate, not the axiom print, is the computability judge there). No print carries `sorryAx`. **Runtime — real extraction (constraint 2), kernel-`rfl`-checked:** on a concrete zero-round leaf tree with `mapWitInv := fun _ w => w + 1`, `#eval ReduceClaim.treeExtractor mapWitInv D 3 T (fun _ => some 7)` → **`some 8`** (the returned witness is the *witnessing's* witness pulled back, not a choice-selected stand-in) and `(fun _ => none)` → **`none`** (the *open* engine declines rather than inventing junk — the classical version returned `Classical.ofNonempty` here). Both `rfl`-provable with axioms `[propext, Quot.sound]`. **Prototypes re-run, all five green** (§1 rule 3's standing choice; **no patch was needed** — M6 renames only `ReduceClaim.{treeExtractor, verifier_coordinateWiseSpecialSoundWith}` and `RingSwitching.Lift.{treeExtractor, package}`, none of which any prototype names; `CM_enginecerts.lean` Part B stays whole as E17's evidence): `CM_gates` → `some 42`, `some 5`, `some 4`, five `IR PRESENT`; `CM_append` → `some 11`, `some 211`, ten `IR PRESENT`; `CM_enginecerts` → `some 21`, `none`, seven `IR PRESENT`; `CM_presentation` → `2, 3, 5, 2, 2, some 2`, six `IR PRESENT` + `NO IR AS EXPECTED (the negative control): CMP.liftPackageLikeM`; `CM_m5demo` → `some 17`, `none`, `some 17`, `5`, fourteen `IR PRESENT`; **zero `sorryAx` mentions in all five**. **Inventory probe (§9 note 2) re-run:** the **canonical** surface now carries exactly **4** noncomputable definitions — `CoordinateWise.{CWSSPackage, GCWSSPackage, EscapeCWSSPackage, EscapeGCWSSPackage}.ofClassical`, the shim lifts M9 deletes — unchanged from M5, i.e. M6 added none. The widened probe over *both* layers lists **23**, down from M5's 24: those 4, the 12 Hachi definitions (M8), and **7** outgoing twins (M5's four plus M6's three). M6 removed four canonical entries (`ReduceClaim.{treeExtractor, oracleTreeExtractor}`, `RingSwitching.Lift.{treeExtractor, package}`) and added three twins; `oracleTreeExtractor` needed none. **`noncomputable section` deletions:** none available — no file in M6's set has one (verified by grep); the only markers left in the set are the three outgoing twins and M1's by-design `Lift.rowSum`. **Sorry census unchanged**, verified at the diff level: the M6 diff's only `sorry`-token difference is a docstring rewrap of the phrase "sorry-free" in `Rlin.lean` (one `-`, one `+`); zero real `sorry`s added or removed. **Zero new long lines** (codepoint-counted per the M3 note; four lines crossed 100 on the 9-character rename and were rewrapped). All thirteen touched `.lean` files emit **zero** messages under `lake env lean` except pre-existing `declaration uses 'sorry'` warnings (`SendWitness.lean:392`, `ZBatchBridge.lean:78, :106`, `Sumcheck/Bridge.lean:57`, `ZeroCheck/Batch.lean:81`, `Rounds.lean:197, :227`), all outside the diff hunks. **Deviations / notes:** (a) **The dominant one — M6 could not fully vacate `*Classical` from `ReduceClaim.lean`, so M8's entry probe was amended.** Six Hachi package literals (`QuadEval/Bridge.lean:213, :244`, `ZBatchBridge.lean:133`, `Rlin.lean:486`, `Sumcheck/Bridge.lean:81`, `Sumcheck/Rounds.lean:290`, `ZeroCheck/Batch.lean:107`) are `*PackageClassical`-typed, so their `extractor` field demands `Extractor.TreeBasedClassical`; retyping `ReduceClaim.treeExtractor` in place would have broken all six, and fixing them means retyping the whole Hachi packages — M8's job, not M6's. So M6 applied M5's own precedent (deviation (a), which the session-global list already generalizes) one step further: `ReduceClaim.{treeExtractorClassical, verifier_coordinateWiseSpecialSoundWithClassical}` under an explicit `## The outgoing layer` header, canonical names on the new engines, the Hachi sites repointed (18 tokens across six files). The same shape hit `RingSwitching.Lift`: `liftPackage` (`Hachi/RingSwitch/Reduction.lean:224`) is `EscapeCWSSPackageClassical`-typed, so `Lift.{treeExtractorClassical, coordinateWiseSpecialSoundWithEscapeClassical, packageClassical}` stay too, with `liftPackage` repointed. M8's entry probe read `rg '[A-Za-z_]+Classical\b' ReduceClaim.lean` *returns nothing*, which is now unsatisfiable and would have made an M8 session stop under §7 protocol step 2, so it was replaced by a probe for the canonical engine plus an explicit instruction to delete all five twins when M8 repoints (M9's entry probe scans `ArkLib/ProofSystem`, so they must not survive M8). This is the only edit M6 made outside its own §11 row. (b) *The oracle pair has no twin.* Nothing outside `ReduceClaim.lean` names `oracleTreeExtractor` or `oracleVerifier_coordinateWiseSpecialSoundWith`, so both were retyped in place — the fully-migrated shape the plan intends. (c) *`RingSwitching.Lift.treeExtractor` drops four arguments* (`φF getM getY sideCond`), for the same reason M5's `CommittedScalar.treeExtractor` dropped `checkAt`: the branch openings now come from the witnessing, so nothing in the extractor reads `checkAt P φF getM getY sideCond` and Lean drops the unmentioned section variables. Its signature is `treeExtractor P K hd`; the outgoing twin keeps the old list. (d) *`Lift.package`'s `isPure` field is `CommittedScalar.verifierPureForm K`*, not the inline anonymous literal it used to be — the literal happens to have exactly `PureForm`'s two fields and would still elaborate, but naming M5's data keeps one source of truth for the seam verdict. (e) *The three zero-round components (`CheckClaim`, `SendClaim`, `SendWitness` ×2) are statement-only migrations*, as the plan predicts: `coordinateWiseSpecialSoundWithClassical → coordinateWiseSpecialSoundWith`, the bridge name likewise, and the extractor `(fun _ _ => ())`/`(fun _ tree => tree.onlyPath.fullTranscript 0)` → `(fun _ _ _ => some ())`/`(fun _ tree _ => some (tree.onlyPath.fullTranscript 0))`. Every proof body survived untouched — each ends `have h := …; exact h`, and `exact` absorbs the beta-redex the canonical bridge's extractor introduces. These are the *closing* shape (§4.6): witnessing-agnostic, now documented as such in each docstring. (f) *No twin was needed for those three*: nothing outside `ArkLib/ProofSystem/Component/` consumes their CWSS theorems (the `Spartan`/`Sumcheck/Spec` mentions are commented out or name `oracleReduction`, not the certificates). (g) *M6's sizing note held.* Each `ReduceClaim` engine really is inlined twice — once in the `def`, once inside its certificate — but at the witness-only shape the second inlining collapses to a one-line `change`, so the two certificates are 12 and 13 lines against the classical versions' 14 and 16. (h) *Commit granularity*: three content commits over disjoint file sets, one per file group, rather than one per bullet — step 1's Hachi repoints cannot bisect apart from step 1 (renaming the engine breaks them in the same breath), and steps 2 and 3 are independent additive edits. **Abort criteria not triggered:** the milestone's own abort test is exactly the E17 axiom print, and both engines came out `[propext, Quot.sound]` on the first build; no transcription failed (Part B went in verbatim modulo the repo's fully-qualified `ProtocolSpec.ChallengeTree.LeafWitnesses.isValid_iff_pure` and `Verifier.support_init_nonempty_of_accepting` spellings — `ReduceClaim.lean` opens no namespaces). |
| M7 | green | one content commit, `feat(Hachi): purity data for every CWSS package verifier`, plus `docs(plans): record M7 green in the computable-extractor execution log` — SHAs to be filled by the repository owner (committing is disabled in-session; **M7 and M8 ran in the same session, so they share one handover script**, `scratchpad/m8/commit-m7-m8.sh`: it reverse-applies `m8.patch` to recover the M7-only tree, commits M7's eleven files and then the log update, re-applies the patch, and commits M8's four themes and its log update, each message from a `msg-*.txt` beside it via `-F`. The patch round trip was verified in-session — reverse-apply reproduces the reconstructed M7 tree exactly, re-apply reproduces the final tree byte-for-byte. M7 lands as **one** content commit, following M1's precedent: §1 rule 5's per-commit rule names *lettered* steps and M7 has none. M7's two commits are both green; M8's are not — see M8's deviation (a)) | **Entry probe:** `rg -n 'PureForm' …/CWSS/Package.lean` → non-empty (`150:  isPure : verifier.PureForm`, i.e. M4 landed the data-typed fields). Ground green before the first edit: `Build completed successfully (4149 jobs)`, working tree clean on `tr/computable-extractors`. **The twelve values landed** (each `def`, each beside the verdict function it names): `verifierPureForm` (`QuadEval/Reduction.lean`, the pass-through triple), `bridgeVerifierPureForm` (`QuadEval/Bridge.lean`), `rlinVerifierPureForm` (`RingSwitch/Rlin.lean`), `batchVerifierPureForm` (`ZeroCheck/Batch.lean`), `zeroCheckVerifierPureForm` (`ZeroCheck/Reduction.lean`), `sumcheckBridgeVerifierPureForm` (`Sumcheck/Bridge.lean`), `roundVerifierGuardedForm` + `roundsBaseVerifierPureForm` (`Sumcheck/Rounds.lean`), `finalEvalVerifierGuardedForm` (`Sumcheck/FinalEval.lean`), `partialEvalVerifierPureForm` (`Recursion/PartialEval.lean`), `zBatchVerifierPureForm` (`Recursion/ZBatchBridge.lean`), `handoffVerifierGuardedForm` (`Recursion/TraceHandoff.lean`). **Exit gate — IR:** twenty-five lines, **no `NO IR`** — `IR PRESENT` on all twelve, on `CoordinateWise.CommittedScalar.verifierPureForm` (the thirteenth package verifier's data, already upstream — see deviation (b)), and on the twelve §9-note-1 decisive probes (`def probeQuadEvalVerifierPureForm := @ArkLib.Lattices.Ajtai.InnerOuter.verifierPureForm`, …), all elaborated outside any `noncomputable section` — **no Hachi file has one** (verified by grep), so nothing here is swallowed and the gate is the real judge. **Exit gate — the abort criterion, checked mechanically rather than by eye:** a `run_cmd` walks the entire transitive *value* dependency graph of each of the twelve and reports any hit on `ZeroCheck/Constraints.lean`'s five noncomputable tables. Output: `banned table confirmed noncomputable:` ×5 (`wTable`, `wTableMleEval`, `hAlphaEvals`, `hZero`, `hAlpha` — the scan first proves its own targets are the right ones), then `NO NONCOMPUTABLE TABLE in the value graph of …` for all twelve, over 275–1210 constants each (`roundVerifierGuardedForm` the largest at 1210, `partialEvalVerifierPureForm` the smallest at 275). The near miss worth recording: `sumcheckBridgeVerifierPureForm`'s verdict *does* reach into `Constraints.lean`, at `zcTargetAlpha` — but that is a plain sorried `def`, not one of the tables, and its docstring already says it is "computable by the verifier from the statement alone"; the scan confirms the distinction. **Axioms:** eight of the twelve are free of `sorryAx`; four carry it and every one is attributable to a pre-existing sorried skeleton def *that the verifier itself already calls*, not to anything M7 wrote — `sumcheckBridgeVerifierPureForm` via `zcTargetAlpha`, `finalEvalVerifierGuardedForm` via `finalCheck`, `partialEvalVerifierPureForm` via `deriveFamily`, `handoffVerifierGuardedForm` via `traceCheck`/`toNextQuadEvalStatement`. Printed side by side as the attribution: the baseline `finalEvalVerifier_isGuarded` and `handoffVerifier_isGuarded` already carry `sorryAx` themselves, while `roundVerifier_isGuarded` does not and neither does its new data form (`[propext, Classical.choice, Quot.sound]`) — the four data forms have exactly the axiom sets of the instances they mirror. **Exit gate — `./scripts/validate.sh`:** `Build completed successfully (4149 jobs)`; `No ArkLib/Data non-sorry warnings found.`; `✓ All imports are up to date!` over `325 imports`; `All documentation integrity checks passed.`; `Knowledge base lint passed.` **Prototypes re-run, all five green** (§1 rule 3's standing choice; **no patch needed** — M7 renames nothing and its new names are fresh in `ArkLib.Lattices.Ajtai.InnerOuter`, which no prototype declares): `CM_gates` → `some 42`, `some 5`, `some 4`, five `IR PRESENT`; `CM_append` → `some 11`, `some 211`, ten `IR PRESENT`; `CM_enginecerts` → `some 21`, `none`, seven `IR PRESENT`; `CM_presentation` → `2, 3, 5, 2, 2, some 2`, six `IR PRESENT` + `NO IR AS EXPECTED (the negative control): CMP.liftPackageLikeM`; `CM_m5demo` → `some 17`, `none`, `some 17`, `5`, fourteen `IR PRESENT`; **zero `sorryAx` mentions in all five**. **Sorry census unchanged**, verified at the diff level: the M7 diff is `11 files changed, 166 insertions(+)` — **insertions only, zero deletions** — and contains **zero** `sorry` tokens on either side. All eleven touched files emit **zero** messages under `lake env lean` beyond their pre-existing `declaration uses 'sorry'` warnings (none of which lands on a new declaration: the `hasSorry` check is syntactic, and the new values reference the sorried helpers as constants). **Zero new long lines** (codepoint-counted per the M3 note; one line at 102 was rewrapped). **Deviations / notes:** (a) **M7 lands twelve values, not the eight its own text names.** The named scope is "the seven packages named in M8, plus `handoffPackage`'s" — but three of M8's twelve definitions are *chains* (`roundsChainAux`/`roundsChain`, `openCore`, `openingChain`), and they compose four further packages (`zeroCheckPackage`, `roundPackage`, `finalEvalPackage`, `partialEvalPackage`) plus `roundsChainAux`'s anonymous base literal (`Sumcheck/Rounds.lean`, the `| 0 =>` case). A composed package's `isPure` field is `Verifier.PureForm.append` of its factors' data, so a single factor left on `pureFormOfIsPure` costs `Classical.choice` and denies IR to the whole chain — i.e. the five unnamed verifiers are exactly as load-bearing for M8's three hardest IR gates as the named eight. Landing them here keeps those gates unconditional and keeps M8 a pure retype; the additions are the same mechanical relocation, IR-gated identically. (b) **`liftPackage` needed no new value.** Its verifier is the generic `CommittedScalar.verifier`, whose purity data M6 already landed upstream as `CoordinateWise.CommittedScalar.verifierPureForm` and wired into the canonical `RingSwitching.Lift.package`'s `isPure` field; M8 fills `liftPackage` by repointing `Lift.packageClassical → Lift.package`, not by supplying data. Gated here anyway (the thirteenth `IR PRESENT`), so M7's census is complete over all thirteen package verifiers behind the twelve. (c) *Homes.* Each value sits beside the **verifier's definition** — or beside the `*_isGuarded` theorem, for the three guarded ones — rather than beside its package, following `CommittedScalar.verifierPureForm`'s precedent (the verdict function's home is the verifier, §4.2). Two consequences: `verifierPureForm` lives in `QuadEval/Reduction.lean`, one file upstream of `quadEvalPackage`; and the five zero-round `ReduceClaim` heads that have no Hachi-local verifier `def` at all (`rlin`, `batch`, `sumcheckBridge`, `zBatch`, and the rounds base) get their data next to the package literal that instantiates the head, since there is no other site. (d) *Purely additive, deliberately.* The existing `Verifier.IsPure`/`IsGuarded` instances are untouched (§4.4: the classes and every instance stay) — `roundVerifier_isGuarded`, `finalEvalVerifier_isGuarded`, `handoffVerifier_isGuarded` keep their proofs, and the pure links keep the inline anonymous witnesses in their package fields until M8 replaces them. The twelve new values therefore *duplicate* those verdict functions for now; the duplication is checked, not assumed, because `isPure : verifier.PureForm` only elaborates at the verifier the value is stated for, so M8's field fill is where the compiler forces the two to agree. Re-proving each instance as `(…Form …).isPure`/`.isGuarded` was considered and rejected: it would make the milestone non-additive for a one-line gain. (e) *Naming.* Data forms are camelCase `<link>Verifier{PureForm,GuardedForm}` (`CommittedScalar.verifierPureForm`'s convention); the `Prop`-valued instances keep their `_isGuarded` suffix. (f) Two signatures needed explicit index pinning that the surrounding code does not: `sumcheckBridgeVerifierPureForm` must pass `(n := n) (μ := μ)` to `toRoundStatement` and `batchVerifierPureForm`/`roundsBaseVerifierPureForm` must ascribe `id`'s type, because a *partially applied* `mapStmt` handed to `ReduceClaim.verifier` leaves `StmtIn`/`StmtOut` as unresolved metavariables (the package literals escape this only because their ascribed package type pins both). **Abort criteria not triggered:** no package verifier's verdict function needs a noncomputable table — established by the transitive value-graph scan above, not by inspection. |
| M8 | green | four content commits, `feat(Hachi): canonical QuadEval and RingSwitch packages` (the QuadEval column + `rlinPackage`/`liftPackage`), `feat(Hachi): canonical zero-check, sumcheck and recursion packages` (the remaining nine package/chain files), `refactor(CWSS): delete the ProofSystem outgoing twins` (M6's five, now consumer-free), `refactor(CWSS): drop the blast radius' noncomputable sections`, plus `docs(plans): record M8 green in the computable-extractor execution log` — SHAs to be filled by the repository owner (committing is disabled in-session; the handover is the shared `scratchpad/m8/commit-m7-m8.sh` described in M7's row, whose second half stages M8's four *file-disjoint* themes in order. The ordering is **not** bisectable per commit here — see deviation (a)) | **Entry probe (amended by M6):** `rg -l 'PureForm\|GuardedForm' …/Hachi/` → **11 files** (M7's data); `rg -n '^def treeExtractor ' …/ReduceClaim.lean` → `190:def treeExtractor (mapWitInv : StmtIn → WitOut → WitIn)` (M6's canonical engine); `rg -n 'noncomputable def cyclotomicPresentation' ArkLib/` → nothing (M1). **Exit gate — IR over all 12, unconditional:** twenty-nine lines, **no `NO IR`** — `IR PRESENT` on the seven packages (`batchPackage`, `zBatchPackage`, `sumcheckBridgePackage`, `bridgePackage`, `quadEvalPackage`, `liftPackage`, `rlinPackage`) and five chains (`roundsChainAux`, `roundsChain`, `evalChain`, `openCore`, `openingChain`), on the **five further packages M8 retyped as chain factors** (`zeroCheckPackage`, `roundPackage`, `finalEvalPackage`, `partialEvalPackage`, `handoffPackage`), and on twelve §9-note-1 decisive probes (`def probeOpeningChain := @…openingChain`, …) elaborated in a file with no `noncomputable section`. The same `run_cmd` additionally asserts **no surviving `noncomputable` marker** on any of the seventeen (`Lean.isNoncomputable` → `logError` if set): zero errors. **Exit gate — the `*Classical` grep over Hachi:** `rg -n '\b[A-Za-z_]*Classical\b' ArkLib/Commitments/Functional/Hachi/` returns **thirteen lines, every one a legitimate `Classical.choice` prose mention inside a docstring** (twelve of them M7's "reading it off the `IsPure` existential would cost `Classical.choice`" rationale) plus `QuadEval/Basic.lean:38`'s axiom note — exactly the gate's carve-out, nothing else. **M9's entry probe already passes:** `rg -n '[A-Za-z_]+Classical\b' ArkLib/ProofSystem ArkLib/Commitments` returns **nothing at all**. **Exit gate — `noncomputable section`s:** all five remaining in the blast radius deleted (`CWSS/Basic.lean`, `CWSS/Composition.lean`, `Package.lean`, `Guarded.lean`, `Escape.lean`), completing the nine §7 lists — and **not one declaration needed a new per-declaration marker**, i.e. the sections were pure risk: the only genuinely noncomputable definitions in those files (the four `ofClassical` lifts, `pureFormOfIsPure`, `guardedFormOfIsGuarded`) already carried explicit markers. `CWSS/Basic.lean`'s section is replaced by a comment saying why the file must not have one. **The headline census — §9 note 2's inventory probe re-run over BOTH layers** (widened to all ten target types): **8** noncomputable definitions remain repo-wide, down from M6's 23, and every one is shim scaffolding M9 deletes — `CoordinateWise.{CWSSPackage, GCWSSPackage, EscapeCWSSPackage, EscapeGCWSSPackage}.ofClassical` (4b's predicted `pureFormOfIsPure` cost) and M5's four `Security/**` outgoing twins (`SingleRound.treeExtractorClassical`, `ScalarRound.treeExtractorScalarClassical`, `CommittedScalar.treeExtractorClassical`, `CommittedScalar.packageClassical`). **Zero on the canonical surface; zero anywhere under `ArkLib/ProofSystem/` or `ArkLib/Commitments/`.** The plan's goal — "remove every noncomputable extractor definition in the CWSS stack" — is met modulo the shim. **Axioms:** the four sorry-free rows stay sorry-free — `bridgePackage`, `quadEvalPackage`, `rlinPackage`, `liftPackage`, `evalChain`, and both Lemma-8 certificates (`quadEval_coordinateWiseSpecialSoundWithEscape`, `eval_coordinateWiseSpecialSoundWithEscape`) all print `[propext, Classical.choice, Quot.sound]`, **no `sorryAx`** (the `Classical.choice` is E25's calibration point, entering through `Prop`-side instance arguments and erased at codegen). **Exit gate — `./scripts/validate.sh`:** `Build completed successfully (4149 jobs)`; `No ArkLib/Data non-sorry warnings found.`; `✓ All imports are up to date!` over `325 imports`; `All documentation integrity checks passed.`; `Knowledge base lint passed.` Every one of the sixteen touched `.lean` files emits **zero** messages under `lake env lean` beyond pre-existing `declaration uses 'sorry'` warnings. **Prototypes re-run, all five green** (no patch needed — M8 renames nothing the prototypes import and deletes only declarations none of them names): `CM_gates` → `some 42`, `some 5`, `some 4`, five `IR PRESENT`; `CM_append` → `some 11`, `some 211`, ten `IR PRESENT`; `CM_enginecerts` → `some 21`, `none`, seven `IR PRESENT`; `CM_presentation` → `2, 3, 5, 2, 2, some 2`, six `IR PRESENT` + `NO IR AS EXPECTED (the negative control): CMP.liftPackageLikeM`; `CM_m5demo` → `some 17`, `none`, `some 17`, `5`, fourteen `IR PRESENT`; **zero `sorryAx` mentions and zero errors in all five**. **Sorry census unchanged**, verified at the diff level rather than by counting tokens: the diff's only three `sorry`-bearing lines are docstring prose (Composition.lean's new "the only `sorry` left on the composition path" sentence, and a two-line rewrap of Bridge.lean's "All proofs in this file are sorry-free"), which is exactly why the repo-wide `sorry`-**line** count moves 302 → 303 while the number of real `sorry` terms does not change. The two deleted twin blocks (42 lines in `ReduceClaim.lean`, 54 in `Lift/Reduction.lean`) were proved, so nothing was lost. **Zero new long lines** in `.lean` files (codepoint-counted per the M3 note). **Deviations / notes:** (a) **The four commits do not bisect, and cannot.** Retyping a Hachi package breaks `Composition.lean` until every factor is retyped, and `Composition.lean` cannot be retyped first — so the always-green-at-every-commit invariant (§4.5) holds only across M8 as a unit here. The commits are still cut by theme for review, and each *file* was built green as it landed (the intermediate whole-repo state was red between the first package retype and `Composition.lean`'s). This is the one place in the plan where a milestone's internal commits are not individually green; recorded rather than papered over. (b) **`liftPackage`'s retype is one token.** `Lift.package` and `Lift.packageClassical` had identical argument lists, so the whole change is `Lift.packageClassical → Lift.package` plus dropping the marker — M6's `isPure := CommittedScalar.verifierPureForm K` had already done the work. (c) **`quadEval_coordinateWiseSpecialSoundWithEscape` transcribed verbatim.** The canonical and outgoing `SingleRound.coordinateWiseSpecialSoundWithEscape*_of_mkWitness` differ only in `[Nonempty WitOut]` (dropped) and in the extractor of the conclusion (`treeExtractor mk` vs `treeExtractorClassical relOut mk`), so Hachi's *proved* Lemma 8 certificate needed no proof change at all — its `refine … (fun stmtIn v fam resp hbranch hstar => ?_)` skeleton and `buildWitness_break_or_mem_relIn` application are untouched. Row 4's `liftPackage.isCWSS` likewise. (d) *Signatures shrank, as the session-global note predicts.* `ReduceClaim.treeExtractor` takes **no `mapStmt`** (its witness-only body never mentions it, so Lean omits the section variable) and no `relOut`: every one of the six zero-round call sites becomes `ReduceClaim.treeExtractor (fun _ w => w) CWSSStructure.ofIsEmpty`. (e) **`handoffExtractor`'s `WitOut` is the *next iteration's* witness type** — `QuadEvalWitness Φ' innerRows' (2 ^ m') messageDigits' (2 ^ r') innerDigits'`, not `LiftedWitness Φ μ n`. This is §4.6's open recursion seam made explicit in a *type*: the handoff's leaf witnessing carries the openings iteration `i+1` will produce. The other four sorried leaf extractors take `WitOut = WitIn` (the witness type is preserved across their seams). (f) *`roundsChainAux`'s anonymous base literal* was covered mechanically as predicted: `EscapeCWSSPackage.toGuarded`, `roundsBaseVerifierPureForm`, `ReduceClaim.treeExtractor`, and `Verifier.coordinateWiseSpecialSoundWith.withEscape` on the certificate; `roundsChainAux`'s IR gate certifies it. (g) *The `IsPure`/`IsGuarded` instances M7 left in place are now consumer-free* (`ReduceClaim.instIsPure`, `roundVerifier_isGuarded`, `finalEvalVerifier_isGuarded`, `handoffVerifier_isGuarded`), and stay — §4.4 pins the classes and their instances as untouched, and `PureForm.isPure`/`GuardedForm.isGuarded` are the forgetful maps that keep them honest. (h) *Two docstrings that described the deleted twins were rewritten* rather than left dangling (`Lift.treeExtractor`'s "unlike its outgoing twin", `ReduceClaim.oracleTreeExtractor`'s "It has no outgoing twin"), and `Composition.lean`'s sorry inventory now names `GuardedForm.append`'s `verify_eq` as the single remaining generic obligation — M4 proved both guarded append theorems, so the old text naming `append_coordinateWiseSpecialSoundWithEscapeClassical_of_guardedLeft` as "the fundamental obligation" was stale. (i) **No runtime `#eval` of the composed chain, by design and by honesty**: `openingChain.extractor` now *has* IR, but five of its leaf extractors are `sorry`, so running it panics — the pre-existing skeleton state each of those docstrings already advertises. The runtime evidence for the machinery is M5's vendored `CM_m5demo.lean` (`some 17` through the canonical `▷`), which still reproduces. **Abort criteria not triggered:** every one of the 12 passed its IR gate on the first build once its dependencies were IR-clean — no new classical root appeared. §4.6's warning was heeded: `handoffExtractor` stays `sorry` and no `psiInv` work was attempted. |
| M9 | green | two commits, `refactor(CWSS): delete the migration shim` (step 1 — the whole `*Classical` layer, the four `ofClassical`s, both `*Form`-from-class shims, the second `▷` table, and the `CM_gates.lean` patch that keeps the prototypes runnable) and `docs: land the computable-extractor plan and its lessons` (step 3 — `repo-map`, `make-computable`, both status lines, `prototypes/README`, and this log row) — SHAs to be filled by the repository owner (committing is disabled in-session; the session hands over `scratchpad/m9/commit-m9.sh`, which stages each commit's *disjoint* file set in order and takes each message from a `msg-*.txt` beside it via `-F`) | **Entry probe:** `rg -l '[A-Za-z_]+Classical\b' ArkLib/ProofSystem ArkLib/Commitments` → **no output** (every consumer already off the shim; M8's five `ProofSystem` twins were deleted there as M6 predicted), and §11 shows M2–M8 green. Ground green before the first edit: `Build completed successfully (4149 jobs)`. **Step 1 — what was deleted.** The whole §4.5 rename table: `Extractor.TreeBasedClassical`; the eight `Verifier.treeSpecialSound*Classical` / `coordinateWiseSpecialSound*Classical` notions plus the four `OracleVerifier` mirrors and `Verifier`/`OracleVerifier.specialSoundClassical`; every forgetful/transport lemma (`toCWSSClassical`, `toEscapeClassical`, `toTreeSpecialSoundClassical`, `*_iff_exists`, `*_congr`, `monoClassical`, `withEscapeClassical`, `*_false_iff`); the four `*PackageClassical` structures with their 16 appends, `toGuardedClassical` and `toEscapeClassical`; every certificate-producing twin (the two generic `append_treeSpecialSoundWith*Classical`, the four `append_coordinateWiseSpecialSound*Classical` across `Verifier`/`OracleVerifier`, both `*_of_guardedLeft`, the three `*_of_isEmpty_challengeIdx`, and `SingleRound`/`ScalarRound`'s `*_of_mkWitness*`); M5's four extra outgoing engines (`SingleRound.treeExtractorClassical`, `ScalarRound.treeExtractorScalarClassical`, `CommittedScalar.{treeExtractorClassical, packageClassical}`); both `Implications` bridges; the four `ofClassical` package lifts, `Extractor.TreeBased.ofClassical` and the two `new_of_old` shims; `Verifier.pureFormOfIsPure` and `Verifier.guardedFormOfIsGuarded`; and the `▷` elaborator's second dispatch table with its lift machinery (`univAppendFn`, `canonLiftOf`, `liftFactor`) — **one 16-entry `canonAppendFn` left**. 16 `.lean` files, **1911 deletions against 198 insertions**. **Step 1 gate — the suffix grep.** `rg -n '\b[A-Za-z_]*Classical\b' ArkLib/` returns **only** genuine library uses: `open Classical in` / `open scoped Classical` / `open … Classical`, `Classical.{choice, choose, choose_spec, decEq, typeDecidableEq, decPred, propDecidable, em, arbitrary}`, and prose naming `Classical.choice`. The sharper `rg -oN '[A-Za-z_][A-Za-z_0-9.]*Classical\b' ArkLib/ | sort -u` — every identifier *ending* in `Classical` — returns **nothing at all**. Repo-wide (`--glob '!.lake/**'`, plan file excluded) the only remaining hits are `docs/plans/prototypes/{CM_gates.lean, README.md}`, which §7 step 1 puts out of scope. **Exit gate — IR.** 36 `IR PRESENT` lines, `MISSING IR: 0`, over the whole surface: `Extractor.TreeBased.append`, `Verifier.{PureForm, GuardedForm}.append`, the four canonical package appends, the four lattice lifts, the six engines/delegates (`SingleRound.treeExtractor`, `ScalarRound.treeExtractorScalar`, `CommittedScalar.{treeExtractor, package}`, `ReduceClaim.{treeExtractor, oracleTreeExtractor}`, `RingSwitching.Lift.{treeExtractor, package}`) and **all 17 Hachi packages and chains** — `quadEvalPackage`, `bridgePackage`, `evalChain`, `rlinPackage`, `liftPackage`, `zeroCheckPackage`, `batchPackage`, `sumcheckBridgePackage`, `roundPackage`, `roundsChainAux`, `roundsChain`, `finalEvalPackage`, `partialEvalPackage`, `zBatchPackage`, `handoffPackage`, `openCore`, `openingChain`. Sixteen §9-note-1 **decisive probes** (`def probe := @X`) elaborated in the same file, outside any `noncomputable section`, with no error. **Exit gate — the goal, measured.** The §9-note-2 **inventory probe** now logs `TOTAL noncomputable over the CWSS surface: 0` — against M0's **20**. Repo-wide `noncomputable def` count **257 → 247**: exactly the ten this milestone deleted (four `ofClassical`s, two `*Form`-from-class shims, four outgoing engines). Every `noncomputable` marker left in `ArkLib/{OracleReduction, ProofSystem, Commitments}` belongs to an unrelated subsystem (Binius, FRI, STIR, `RingSwitching/Packing`, Sumcheck, RoundByRound, …) or is `ChallengeTree.canonWitnesses`, which is classical **by design** (§4.1: the top-level closer, erased at codegen). **Exit gate — sorry census.** `Guarded.lean`'s real `sorry` count **2 → 1** (the deleted twin was the sorried `append_coordinateWiseSpecialSoundWithEscapeClassical_of_guardedLeft`; its canonical restatement was proved in 4a). The one survivor is `Verifier.GuardedForm.append`'s `verify_eq`. `CoordinateWiseSpecialSoundness/` + `TranscriptTree/` + `SpecialSoundness.lean` therefore hold **one** `sorry` in total — §7's "comes out with one fewer `sorry`", delivered. **Exit gate — validation.** `./scripts/validate.sh --docs`: exit 0, `Build completed successfully (4149 jobs)` then `(8570 jobs)` for the doc build, `No ArkLib/Data non-sorry warnings found.`, `✓ All imports are up to date!` over `325 imports`, `All documentation integrity checks passed.`, `Knowledge base lint passed.`, `All requested validation checks passed.` `./scripts/validate.sh --lint --docs` exits 1 on the repo's **pre-existing** style-lint backlog (~30 files: STIR, Sumcheck, `ToMathlib`, `Data/CodingTheory`, …) — that pass has never been clean. Measured per-file against `HEAD` instead: over the 16 touched `.lean` files M9 **adds zero** findings and **removes two** (`ERR_CLN` in `CWSS/Basic.lean`, on deleted declarations). The single finding still reported in a touched file, `TranscriptTree/Basic.lean` `ERR_IBY`, is `canonWitnesses_isValid`'s isolated `by`, present verbatim at `HEAD`. One long line **was** introduced and fixed before the gate (`Package.lean`'s rewritten `append` docstring; codepoint-counted per M3's note). **Exit gate — prototypes, all five green** (§1 rule 3's standing choice: patch the prototypes). `CM_gates` → `some 42`, `some 5`, `some 4`, five `IR PRESENT`; `CM_append` → `some 11`, `some 211`, ten `IR PRESENT`; `CM_enginecerts` → `some 21`, `none`, seven `IR PRESENT`; `CM_presentation` → `2, 3, 5, 2, 2, some 2`, six `IR PRESENT` + `NO IR AS EXPECTED (the negative control): CMP.liftPackageLikeM`; `CM_m5demo` → `some 17`, `none`, `some 17`, fourteen `IR PRESENT`. **Zero `sorryAx` mentions and zero errors in all five** — the recorded expected outputs, unchanged. **Step 3 — docs contract closed.** `docs/wiki/repo-map.md`: the `TranscriptTree` entry now describes the witness-only extractor, `LeafWitnesses`/`IsValid`, the `canonWitnesses` closer and `NonVacuity` as the standing gate; the `Guarded` entry no longer calls the guarded escape append sorried (both are proved) and names `GuardedForm` plus the one real remaining `sorry`. `docs/skills/make-computable.md`: a new **Widening an interface** section — the durable lesson, that *architectural* means the interface is wrong and an interface is something you can change — with the three recurring shapes (a missing argument; a `Prop`-valued field a consumer must read as data; a kept `Type`-valued binder carrying noncomputable data) and the two facts that shape the job (a chain is only as computable as its weakest factor; land it always-green behind a time-boxed suffix shim). This plan's status line and `docs/plans/README.md`'s entry are flipped to **landed**; `prototypes/README.md` records that `TranscriptTree/NonVacuity.lean` is now the living copy of the gates and that two shims the prototypes reference no longer exist in the library. **Deviations / notes:** (a) *The classical statement shape had to survive, privately, in two places.* `TranscriptTree/NonVacuity.lean`'s G0 kill (`oldStatement` / `classical_refutable`) is literally a statement **about** the total-extractor form, so deleting `Extractor.TreeBasedClassical` outright would delete the gate. It is replaced by a `private def TotalExtractor` local to that file, alongside the other rejected variants the file already declares `private` — no public API, and the suffix grep is clean. `docs/plans/prototypes/CM_gates.lean` needed the same and now carries `CM.TreeBasedClassical` (it already carried its own `CM.pureFormOfIsPure`). (b) *`guardedFormOfIsGuarded` went too.* §4.5 only asks M9 to grep `pureFormOfIsPure`; both had **zero** organic uses once the four `ofClassical` lifts were gone, so both were deleted — M4's deviation (e) introduced the guarded mirror as a shim addition and this is its matching removal. (c) *The `▷` deletion took M4's shape, not §4.5's.* §4.5 describes widening `packageKindOf` to `Expr → TermElabM (Name × Expr)` and deleting "the lift cases"; M4 deviation (f) had instead kept `packageKindOf : Expr → TermElabM Name` and put the lifting in `canonLiftOf`/`liftFactor`, so M9 deleted those two plus `univAppendFn`. Same end state: one table, dispatch by kind. (d) *The three kind-marked synonyms were repointed*, as M4 deviation (g) deferred to this milestone: `▷ᵍ` now names `GCWSSPackage.append` (moved below it in `Guarded.lean`), and `▷ₑ` / `▷ₑᵍ` name `EscapeCWSSPackage.append` / `EscapeGCWSSPackage.append` (re-declared after `end CanonicalEscapePackage` in `Escape.lean`, since `inherit_doc` needs its target in scope). No call site uses them, so they were smoke-tested separately: at abstract factors each elaborates all the way to its seam autoparam, leaving exactly `⊢ L₁.relOut = L₂.relIn` at the **canonical** structure types — which is the notation's normal behaviour on non-definitional seams, and pins the target. The universal `▷` is exercised for real by the whole-repo build and by `CM_m5demo.lean`'s `#eval` to `some 17`. (e) *Prose was the bulk of the non-deletion diff.* The outgoing layer was named in ~30 docstrings and 8 section headers across the 16 files; each was rewritten to describe the single surviving layer rather than merely dropping the suffix — including three file-level module docstrings (`Package`, `Guarded`, `Escape`) whose titles named `*PackageClassical`. **Abort criterion not triggered:** deleting the layer left no consumer red — the first full build after the deletion pass was green, so no M2–M8 site was ever a straggler. |
...
- The CWSS import DAG is a chain: `Basic → Composition → Package → Guarded → Escape`, with
  `NoChallenge` importing only `Basic`. So anything mentioning `Verifier.IsGuardedWith` /
  `GuardedForm` must be homed in `Guarded.lean` or deeper — which is why M4's five guarded seam
  lemmas and two generic guarded append theorems could not go where 4a's text sent them (M4).
- `lake env lean <file>` elaborates against the **last built** `.olean`s of its imports, not against
  the working tree. After editing an upstream file, `lake build` that module before running
  `lake env lean` on a downstream one, or you get spurious `unknown identifier` errors for
  declarations you just added (M4; cost one confused detour).
- `▷` dispatches **by kind, deterministically**: two `*Classical` factors take the outgoing table,
  anything else takes the canonical table with `ofClassical` lifts. Not canonical-first-with-
  fall-through — that would retype every existing `▷` in `ProofSystem`/`Hachi` (M4 deviation (f)).
- A package produced by `ofClassical` — hence any definition built from a *mixed* `▷` chain — needs a
  `noncomputable` marker, because the purity field is `Classical.choice`. Expected, per 4b's gate
  text; only fully-migrated chains are computable (M4).
- M5's engines needed four **additional** outgoing renames that §4.5's table does not list —
  `SingleRound.treeExtractorClassical`, `ScalarRound.treeExtractorScalarClassical`,
  `CommittedScalar.treeExtractorClassical`, `CommittedScalar.packageClassical`. 2a suffixed the
  `*_of_mkWitness*` *certificates* but not the *engines* they name, so retyping an engine in place
  breaks its own outgoing certificate. Expect the same shape wherever a milestone retypes a
  definition whose outgoing certificate 2a renamed (M5).
- The `NoChallenge` bridges are **witnessing-agnostic**: their extractor ignores its leaf witnessing,
  so they need no purity hypothesis and no `isValid_iff_pure`. That is the *closing* shape — a factor
  of this kind on the right of a chain is what lets the composed extractor run on `(stmtIn, tree)`
  alone (M5; demonstrated by `CM_m5demo.lean`).
- A `Fin` numeral at an unreduced arity projection (`(D.toShape.arity ⟨1, rfl⟩)`) has no `OfNat`
  instance. Rather than `Fin.cast`, use `ChallengeTree.somePath` with
  `CWSSStructure.toShape_arity_pos` — it needs no numeral at all (M5).
- The outgoing-twin pattern M5 predicted fired again in M6, and it is now the **rule**: a milestone
  that retypes a definition consumed by an un-migrated `*PackageClassical` literal must keep an
  outgoing twin, because a `*PackageClassical`'s `extractor` field demands
  `Extractor.TreeBasedClassical` and nothing lifts canonical → classical. M6's three:
  `ReduceClaim.treeExtractorClassical` (+ its certificate) and
  `RingSwitching.Lift.{treeExtractorClassical, packageClassical}`. **M8 must delete these five
  declarations when it repoints Hachi**, since they sit in `ArkLib/ProofSystem/**`, which M9's
  entry probe scans (M6; M8's section now says so).
- A component retyped for the witness-only notion **drops the arguments it only needed for the
  classical inversion** — Lean omits unmentioned section variables, so the signature shrinks:
  `CommittedScalar.treeExtractor` lost `checkAt` (M5), `RingSwitching.Lift.treeExtractor` lost
  `φF getM getY sideCond` (M6). Expect call sites to shorten, not just retype.
- A Hachi **chain**'s purity field is `PureForm.append` of its factors', so *every* factor of a
  composed package needs genuine `PureForm`/`GuardedForm` data — one factor on `pureFormOfIsPure`
  denies IR to the whole chain. That is why M7 covers thirteen package verifiers, not the eight its
  text names: the four packages and one anonymous literal behind `roundsChain`/`openCore`/
  `openingChain` are as load-bearing as the named ones (M7).
- The five `noncomputable section`s left in the CWSS blast radius were **pure risk**: deleting all
  five required **zero** new per-declaration markers, because everything genuinely noncomputable in
  those files (the four `ofClassical` lifts, `pureFormOfIsPure`, `guardedFormOfIsGuarded`) already
  carried one. §10 R3's worry was justified and its remedy costless (M8).
- A Hachi package retype cannot bisect: `Composition.lean` breaks until *every* factor is canonical,
  and it cannot go first. M8's internal commits are therefore themed, not individually green — the
  only place in the plan where §4.5's always-green invariant holds across a milestone rather than
  across each of its commits (M8).
- No file under `ArkLib/Commitments/Functional/Hachi/` has a `noncomputable section`; every
  noncomputable definition there carries a per-declaration marker. So Hachi IR gates are honest by
  construction, and M8's marker drops are the whole of its computability work (M7).
- The G0 non-vacuity gate is stated *about* the classical total-extractor shape, so that shape must
  outlive the shim: `TranscriptTree/NonVacuity.lean` keeps it as a `private def TotalExtractor`.
  A deletion milestone should expect one or two such "the gate names what you are deleting" sites
  (M9; the vendored `CM_gates.lean` is the other).
- `pureFormOfIsPure` had **no** organic uses at M9 — nor did its guarded mirror. Both went (M9).
