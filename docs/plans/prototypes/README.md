...
The first four files hold the design: the notion with its gates, transports and purity-as-data;
...
transcribe from them, so a wrong declaration name here is a defect. The fifth,
`CM_m5demo.lean`, is the opposite direction: it is a *product* of M5 rather than a source for it,
and runs entirely against the landed library. All five are **not** part of the library build
(nothing imports them; they are not under `ArkLib/`). Check one with:
...
output. All of them additionally print `#eval` values and `IR PRESENT` probes;
