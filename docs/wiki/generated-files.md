...
- Do not commit `docs/kb/_generated/**` changes from ordinary feature PRs. They are refreshed on
  `main` by `.github/workflows/kb-generated.yml`.
- Missing cited-paper stubs under `docs/kb/papers/` and `docs/kb/sources/` are also scaffolded by
  the main-branch KB workflow. Commit paper pages in feature PRs only when they contain real
  ArkLib-specific content.
