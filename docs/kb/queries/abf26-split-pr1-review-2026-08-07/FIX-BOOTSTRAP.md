...
> remain. A follow-up review then closed B3 semantically; that intentionally supersedes A1's
> original instruction to retain four `[Finite F]` bridge instances.
...
|---|---|---|---|
| **A1** | Four lemmas carry `[Finite F]` that is **provably unnecessary**. All four are the *`Lambda`-bound → consumer* direction, where the hypothesis itself forces the point list finite (`Set.finite_of_encard_le_coe`), or where `ncard = 0` on an infinite set makes the bound free. Re-proved without it. | `Data/CodingTheory/ListDecodability.lean` — `ncard_closeCodewordsRel_le_Lambda`, `listDecodable_of_Lambda_le_natCast`, `Lambda_le_floor_of_toENNReal_le_ofReal`, `listDecodable_of_toENNReal_le_ofReal` | Initially drop `[Finite F]` from exactly these four. The follow-up B3 semantic correction makes `listDecodable` carry point-list finiteness itself, so the final tree also removes the instance from `Lambda_le_iff_listDecodable`, `Lambda_le_floor_iff_listDecodable`, `_nnreal`, and `Lambda_le_floor_of_listDecodable`; see the fix-session record. |
...
| **B3** | The pre-existing `listDecodable` is still vacuous on infinite alphabets (`Set.ncard` = 0), and its docstring asserts the cardinality "is a natural number anyway" — false at that generality. `uniqueDecodable` inherits it. | `ListDecodability.lean` | Initially scoped to documentation only. Follow-up authorization closed the semantic defect by requiring each point list to be finite alongside its real-valued `ncard` bound; the STIR-facing type remains unchanged. |
