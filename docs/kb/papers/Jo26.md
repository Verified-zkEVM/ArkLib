...
- ArkLib's canonical generator-level value is `CoreDefinitions.mcaError`; the ABF26 affine-line
  spelling `epsMCA` is a reducible adapter rather than an independent error function.
...
- The affine-line exact-transfer corollary is present directly on `mcaError` with the source's
  ...
  `ProximityGap.mcaError_affineLine_interleaved_le` is externally admitted; the reverse inequality
  is derived from `TensorMCA.isMCAGenerator_of_moduleInterleavedCode`, and
  `mcaError_affineLine_interleaved_eq` follows by antisymmetry.
