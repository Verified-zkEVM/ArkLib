...
Huang, Mao and Zhang introduce a **ring-switching lift** for lattice-style proof systems: a
linear relation `M z = y` over a quotient ring `R_q = Z_q[X]/(f)` holds iff the canonical
representatives satisfy `M(X)·z(X) = y(X) + f(X)·r(X)` over `Z_q[X]` for a quotient vector `r`
of degree `< deg f`; evaluating the lifted identity at a random point `α` of an extension field
`F ⊇ Z_q` turns the ring statement into field arithmetic, where sumcheck-style protocols run
efficiently. Hachi ([NOZ26]) adopts this lift as the entry of its §4.3 sumcheck chain
(Figure 4 / Lemma 9).
...
- The lift as a two-round subprotocol (commit to the lifted witness, evaluate at a random `α`)
  ...
  `ArkLib/ProofSystem/RingSwitching/Lift/` over any monic-modulus presentation
...
- [`ArkLib/ProofSystem/RingSwitching/Lift/Presentation.lean`](../../../ArkLib/ProofSystem/RingSwitching/Lift/Presentation.lean)
  ...
  interpolation engine, and the CWSS reduction over the committed-scalar shell.
- [`ArkLib/OracleReduction/Security/CoordinateWiseSpecialSoundness/CommittedScalar.lean`](../../../ArkLib/OracleReduction/Security/CoordinateWiseSpecialSoundness/CommittedScalar.lean)
  — the generic commitment-anchored scalar phase and escape/collision/common-opening CWSS
  framework.
...
- ArkLib never sends the lifted witness `(z, r)` in the clear: it is the output-relation
  witness of the composed chain (Hachi design D6), and the verifier is a pure
  statement-extending pass-through; the paper's final-message checks live in the output
  relation `relLift`.
- The commitment is abstract (`LiftCom`, an alias of `CoordinateWise.BindingCommitment`) with a
  *norm-conditioned* collision
  axiom (weak binding, [NOZ26] Remark 2 / Lemma 7), not the paper's plain binding.
...
- The modulus is an arbitrary monic cyclotomic `Φ.φ` (paper: `X^d + 1`), and the extension
  field is abstract with an embedding (paper: `F_{q^k}`).

## Version Notes

- Cited as ePrint 2025/199; Hachi ([NOZ26]) cites the same report. Track the version if proof
  obligations start depending on exact statements.
