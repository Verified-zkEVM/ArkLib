...
`ArkLib/Data/CodingTheory/ProximityGap/` generator layer, and records the two places where the
formalization departs from the paper as printed.
...
| Def 3.19 (polynomial generator) | `CoreDefinitions.IsPolynomialGenerator` | |
| Def 4.3 (tensor generator) | `CoreDefinitions.TensorGenerator`, `CoreDefinitions.TensorGenerator_Explicit` | Agree under `tensorProductPiFunEquiv` |
...
## Lemma 4.4 is formalized in two forms, and neither is the printed statement
...
form 2's, so the two do not subsume one another.
...
- **Thm 9.2** (Reed–Solomon, list-decoding regime). The base MCA comes from Lemma 9.3, which is
  ...
  `BCIKS20` Thm 5.1 and factors `disc*_Y(Q)`. The interleaving `RS^ℓ ⊆ (F^ℓ)ⁿ` is not a
  Reed–Solomon code, so the interleaved hypothesis is not available at error `ϵ′MCA`. Lemma 10.1
  supplies it at `ℓ · ϵ′MCA`, which is what the union-bound form reaches directly.

So the deviation is invisible at Thm 8.2 and material at Thm 9.2.
