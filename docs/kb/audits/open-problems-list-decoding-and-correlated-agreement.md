...
| `T2.18` | FRS and UM are subspace-design | present (**PROVEN**; FRS half only) | [SubspaceDesign.lean](../../../ArkLib/Data/CodingTheory/SubspaceDesign.lean) | `CodingTheory.frs_is_subspaceDesign_gk16` | GK16 Theorem 14 is proved via the folded-Wronskian toolkit. Generic determinant, root-multiplicity, and finite-dimensional helpers live in `ArkLib/ToMathlib`. The UM half remains open pending a multiplicity-code analogue of the Wronskian argument. The statement includes the load-bearing generator and admissibility hypotheses documented under D2.14. Statement corrected 2026-07-21: restored GK16's ω-generator hypothesis `orderOf ω = \|F\|−1` — the unguarded form is **false**, counterexample `ω = −1` over `𝔽₁₀₁` (PAPER_REVS #13). |
...
| Corollary 3.3 MDS coarse Johnson corollary | present | `mds_johnson_lambda_le` in [ArkLib/Data/CodingTheory/JohnsonBound/Family.lean](../../../ArkLib/Data/CodingTheory/JohnsonBound/Family.lean) | **Partial: the field-linear specialization is proved**, from Theorem 3.2 plus the rate-distance bridge `LinearCode.IsMDS_iff_rate_distance` in [ArkLib/Data/CodingTheory/Basic/LinearCode.lean](../../../ArkLib/Data/CodingTheory/Basic/LinearCode.lean). The Lean statement quantifies over `LinearCode ι F` (field alphabet), so it cannot express the module-alphabet MDS codes — e.g. interleaved RS — that the paper's C3.3 preamble singles out; the general form needs the module-alphabet rate-distance bridge (T3.2 itself is already alphabet-generic). |
...
| Definition 6.4 erasure correction | missing (algorithm/cost model) | supporting metric theorem `CodingTheory.eq_of_consistent_with_erased` in [Erasure.lean](../../../ArkLib/Data/CodingTheory/Erasure.lean) | The recovery algorithm and `ecor` cost are not represented. ArkLib deliberately omits the unrestricted existential-over-functions shadow because it is true of every code and therefore does not formalize D6.4. |
| Lemma 6.5 every additive code supports erasure correction | missing (algorithmic bound) | `CodingTheory.eq_of_consistent_with_erased`; generic disagreement lemma `Code.eq_of_disagreementCols_subset_of_card_lt_minDist` in [Distance.lean](../../../ArkLib/Data/CodingTheory/Basic/Distance.lean) | The substantive metric uniqueness ingredient is proved generally. The additive-code decoder and `O((s·n)³)` operation bound are not formalized. |
