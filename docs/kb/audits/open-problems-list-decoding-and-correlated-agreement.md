...
| `D2.20` | Extension code `C_F` | present | [ExtensionCodes.lean](../../../ArkLib/Data/CodingTheory/ExtensionCodes.lean) | encoder: `CodingTheory.extensionEncode`; image: `extensionCode` / `extensionCodeSubmodule` | The encoder form, coordinate equation `coord_extensionEncode`, injectivity preservation `extensionEncode_injective`, systematic identity `extensionEncode_comp_algebraMap_of_isSystematic`, and range bridge `range_extensionEncode` are proved. The image-level coordinate definition, closure laws, `Submodule` packaging, systematic membership bridge, and presentation independence are also present. The separately attributed Diamond–Posen equality `δ_min(C_F) = δ_min(C_B)` is proved as `minDist_extensionCode`, via the generic metric bridge `minDist_extensionCode_eq_interleaved`. |
