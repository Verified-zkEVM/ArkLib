...
/-!
# A full-family ring-switch consumer of the GF16 Binius oracle
...
/-- The exact commitment relation is the production unique-distance relation. -/
theorem relation (p : PackedField⦃≤ 1⦄[X Fin 2]) (o : InitialOracles) :
    pc.commitsTo o p ↔ commitment.initialCompatibility (p, o) := Iff.rfl

/-- The adapter's honest constructor is the production oracle constructor. -/
theorem honest_constructor : pc.commit (coordinate 0) = honestOracle (coordinate 0) := rfl

/-- Reading the oracle through the production accessor yields the novel-basis codeword. -/
...
/-- Rank-four batching in the field of sixteen elements has error three sixteenths. -/
theorem batch_error : batch.error = (3 : ℝ≥0) / 16 := by
  change ((4 - 1 : ℕ) : ℝ≥0) / (Fintype.card PackedField : ℝ≥0) = _
  rw [field_card]
  norm_num

/-- The tensor-profile functionality premise is also discharged by the commitment theorem. -/
theorem profile_functional : commitment.Functional :=
  FRIBinius.binaryBasefold_functional 2 PackedField (ZMod 2) binaryBasis 2 1 1 (by decide)

/-- The full-family source relation has a concrete witness on the nonconstant commitment. -/
...
/-- The generic commitment cannot interpret the same oracle as the other coordinate. -/
theorem rejects_other_coordinate : ¬ pc.commitsTo oracle (coordinate 1) :=
  honestOracle_rejects_other_coordinate

/-- The production phase accepts every batching challenge and returns the identical oracle. -/
...
/-- The packed polynomial and the same oracle satisfy the sumcheck output relation. -/
...
/-- The materialized verifier aborts on that false family for every challenge. -/
...
/-- Worst-case knowledge uses the production binding proof and its same-oracle KSF. -/
...
/-- Perfect completeness is uniform over initial oracle states. -/
