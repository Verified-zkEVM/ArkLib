...
/-- The exact first-order list bound covers a singleton solution family over two points. -/
example : (({(0 : Polynomial ℚ)} : Finset (Polynomial ℚ)).card : ℚ) ≤
    firstOrderTightListWeight 2 2 1 2 4 1 1 := by
  ...
  have h := finite_firstOrder_agreement_solutions_card_le_tight_of_exponent
  ...
  simpa [S, firstOrderTightListWeight, firstOrderCurveFiberStageOne,
    firstOrderTaylorTotalCap, firstOrderTaylorDerivativeCap] using h

/-- The exponent `2 * K` gives the uniform first-order list bound on the same instance. -/
example : (({(0 : Polynomial ℚ)} : Finset (Polynomial ℚ)).card : ℚ) ≤
    ((2 * firstOrderListWeight 2 1 1 : ℕ) : ℚ) / 2 := by
  let Q : DifferentialPolynomial ℚ 1 := firstOrderUnitEquation ℚ
  let S : Finset (Polynomial ℚ) := {(0 : Polynomial ℚ)}
  have hQ : Q ≠ 0 := by simp [Q, firstOrderUnitEquation]
  have hdegree : jetTotalDegree Q ≤ 1 := by
  ...
  have haccept : ∀ P ∈ S,
    ...
    · norm_num
    · norm_num [firstOrderAgreementTestDomain, firstOrderAgreementTestReceived]
  have h := finite_firstOrder_agreement_solutions_card_le_sharp
    ...
    (by norm_num) (by norm_num) (by norm_num) (Or.inl (by simp)) S hsol haccept
  norm_num [S, firstOrderListWeight] at h ⊢
