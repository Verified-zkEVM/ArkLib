...
* At `d = 1`, `m = 2`, `W = 0` the budget is `1 · 1 + 1 · 2 = 3`, so the local rank on every
  partition support space with these parameters is at most `3`, whatever `D` and `L` are.
...
for every `D` and `L`, including `D = 0`. -/
example (D : ℕ) (L : ℝ) (center received : ℚ) :
    Module.finrank ℚ (LinearMap.range (partitionSupportLocalConstraint (D := D) (d := 1) (W := 0)
      (L := L) 2 center received)) ≤ 3 :=
  (finrank_partitionSupportLocalConstraint_le center received).trans (by decide)
