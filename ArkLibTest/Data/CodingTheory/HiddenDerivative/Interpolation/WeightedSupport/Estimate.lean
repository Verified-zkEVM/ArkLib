...
The form of `weighted_dimension_integral` with pointwise simplex hypotheses and the form of
`weighted_dimension_probability` as an integral against the uniform probability measure on the
simplex, derived from the general statements; a concrete bound at `d = 1`, where the simplex is a
single point; and the case `W = 0`.
-/
