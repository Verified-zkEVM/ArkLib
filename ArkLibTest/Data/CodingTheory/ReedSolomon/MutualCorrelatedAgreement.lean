...
-- A zero tuple on one point has exact power agreement at every challenge.
example : ∃ exceptional : Finset (ZMod 3), exceptional.card ≤ 0 ∧
    ∀ z ∉ exceptional,
      HasExactPowerAgreement domain (fun _ : Fin 1 => fun _ => (0 : ZMod 3))
        (RingHom.id (ZMod 3)) 1 z 0 := by
  classical
  have h := exists_exceptional_exactPowerAgreement (α := Fin 1) (ℓ := 0) (k := 1) (L := 0)
    domain (fun _ : Fin 1 => fun _ => (0 : ZMod 3))
    (fun _ => (0 : (ZMod 3)[X])) (RingHom.id (ZMod 3))
    (by intro t; norm_num) (by omega)
  simpa [powerBatchedPolynomial, Fintype.card_fin] using h

-- The same empty exceptional set works for a singleton family of zero tuples.
example : ∃ exceptional : Finset (ZMod 3), exceptional.card ≤ 0 ∧
    ∀ P ∈ ({(fun _ : Fin 1 => (0 : (ZMod 3)[X]))} :
      Finset (Fin 1 → (ZMod 3)[X])), ∀ z ∉ exceptional,
      HasExactPowerAgreement domain (fun _ : Fin 1 => fun _ => (0 : ZMod 3))
        (RingHom.id (ZMod 3)) 1 z 0 := by
  classical
  have h := exists_exceptional_exactPowerAgreement_family
    (α := Fin 1) (ℓ := 0) (k := 1) (L := 0) domain
    (fun _ : Fin 1 => fun _ => (0 : ZMod 3)) (RingHom.id (ZMod 3))
    {fun _ : Fin 1 => (0 : (ZMod 3)[X])} (by
      intro P hP t
