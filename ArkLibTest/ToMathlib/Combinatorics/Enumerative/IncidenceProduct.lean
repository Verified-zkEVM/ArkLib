...
comparison, `A ≤ n` and `0 < b` in `one_le_incidenceFactor`, and `d ≤ k + 1`, `k ≤ A`, and
`A ≤ n` in writing the dimension-sensitive product over thresholds. Monotonicity also needs
`A ≤ n` and `0 < b`. The shifted-ratio estimate needs `δ ≤ 1` and `δ * x ≤ y`.
-/
