...
With a coefficient map that raises degrees, the bound of
`MvPolynomial.totalDegree_clearedSubstitution_le_of_coeff` charges each monomial the degree of its
mapped coefficient. For `Q = X 0 * Y ^ 2`, whose coefficient `X 0` has degree `1`, and
`S = X 0`, `N = X 0 ^ 2`, `d = 1`, `H = 2`, the bound is `2 * 1 + (1 + 2) = 5`, and the cleared
numerator `X 0 ^ 5` attains it.
-/
...
/-- The polynomial `X 0 * Y ^ 2`, with the coefficient `X 0` taken in `ℚ[X 0]`. -/
private noncomputable abbrev coeffSquareQ : MvPolynomial (Fin 1) (MvPolynomial (Fin 1) ℚ) :=
  monomial (Finsupp.single 0 2) (X 0)

private theorem support_coeffSquareQ : coeffSquareQ.support = {Finsupp.single 0 2} := by
  simp [coeffSquareQ, support_monomial, X_ne_zero]

/-- The bound with the identity coefficient map: the cleared numerator of `X 0 * Y ^ 2` has total
degree at most `5`. -/
example :
    (clearedSubstitution (RingHom.id _) (X 0 : MvPolynomial (Fin 1) ℚ) (fun _ ↦ X 0 ^ 2)
      (fun _ ↦ 1) 2 coeffSquareQ).totalDegree ≤ 5 := by
  have h := totalDegree_clearedSubstitution_le_of_coeff (RingHom.id _)
    (X 0 : MvPolynomial (Fin 1) ℚ) (fun _ ↦ X 0 ^ 2) (fun _ ↦ 1) 2 1 3 coeffSquareQ (by simp)
    (fun _ ↦ by simp [totalDegree_X_pow])
    (by simp [support_coeffSquareQ, Finsupp.weight_single])
    (by simp [support_coeffSquareQ, coeffSquareQ])
  simpa using h

/-- The bound `5` is attained: the cleared numerator is `X 0 ^ 5`. -/
example :
    (clearedSubstitution (RingHom.id _) (X 0 : MvPolynomial (Fin 1) ℚ) (fun _ ↦ X 0 ^ 2)
      (fun _ ↦ 1) 2 coeffSquareQ).totalDegree = 5 := by
  have h : clearedSubstitution (RingHom.id _) (X 0 : MvPolynomial (Fin 1) ℚ) (fun _ ↦ X 0 ^ 2)
      (fun _ ↦ 1) 2 coeffSquareQ = X 0 ^ 5 := by
    simp [clearedSubstitution, support_coeffSquareQ, coeffSquareQ, Finsupp.weight_single]
    ring
  rw [h, totalDegree_X_pow]
