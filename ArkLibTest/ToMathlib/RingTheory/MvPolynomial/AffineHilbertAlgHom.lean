...
are derived from the general ones. The quotient map itself recovers `natDegree P_I ≤ Nat.card σ`,
and `ℚ[x₀, x₁] ⧸ (x₁)` is generated by the class of `x₀`, so its Hilbert polynomial has natural
degree at most `1`.
-/
...
/-- The quotient map `k[σ] → k[σ] ⧸ I` recovers `natDegree_affineHilbertPolynomial_le`. -/
example {k σ : Type*} [Field k] [Finite σ] (I : Ideal (MvPolynomial σ k)) :
    (affineHilbertPolynomial I).natDegree ≤ Nat.card σ :=
  natDegree_affineHilbertPolynomial_le_card_of_surjective (Ideal.Quotient.mkₐ k I)
    (Ideal.Quotient.mkₐ_surjective k I)

/-- `ℚ[x₀, x₁] ⧸ (x₁)` is generated by the class of `x₀`, so its Hilbert polynomial has natural
degree at most `1`. -/
example : (affineHilbertPolynomial
    (Ideal.span {X 1} : Ideal (MvPolynomial (Fin 2) ℚ))).natDegree ≤ 1 := by
  set I : Ideal (MvPolynomial (Fin 2) ℚ) := Ideal.span {X 1}
  let x : Fin 1 → MvPolynomial (Fin 2) ℚ ⧸ I := fun _ ↦ Ideal.Quotient.mkₐ ℚ I (X 0)
  have hall (f : MvPolynomial (Fin 2) ℚ) :
      Ideal.Quotient.mkₐ ℚ I f ∈ Algebra.adjoin ℚ (Set.range x) := by
    induction f using MvPolynomial.induction_on with
    | C a => exact Subalgebra.algebraMap_mem _ a
    | add f g hf hg => simpa only [map_add] using Subalgebra.add_mem _ hf hg
    | mul_X f i hf =>
      rw [map_mul]
  ...
  have h := natDegree_affineHilbertPolynomial_le_card_of_adjoin_eq_top x
    (eq_top_iff.mpr fun y _ ↦ by
      obtain ⟨f, rfl⟩ := Ideal.Quotient.mkₐ_surjective ℚ I y
      exact hall f)
  simpa using h
