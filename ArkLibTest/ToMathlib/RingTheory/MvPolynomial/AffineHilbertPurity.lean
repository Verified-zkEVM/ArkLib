...
needs injectivity. The next examples restate purity, the Bézout bound and the cut
potential with primality of `P` as an explicit argument. The final example computes the
