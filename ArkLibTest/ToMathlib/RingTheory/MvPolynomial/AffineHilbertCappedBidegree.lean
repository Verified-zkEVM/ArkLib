...
form `h * (2 * b * c - c ^ 2) + 2 * a * (j * c + r * (b - c))` for `c < b`, check the cases
`c = b` and `b < c`, and show that the bound needs `g ≠ 0` and `0 < c`.
-/
...
example {k : Type*} [Field k] {a b c h j r : ℕ} (ha : 0 < a) (hb : 0 < b) (hc : 0 < c)
  ...
  refine (affineDegree_comap_cappedBidegree_span_singleton_le ha hb hc hg0 hg).trans_eq ?_
  rw [cappedBidegreeMixedVolume_eq hcb.le, mul_assoc h, Nat.mul_sub, sq, mul_comm c (2 * b)]
...
example {k : Type*} [Field k] {a b c h j r : ℕ} (ha : 0 < a) (hb : 0 < b) (hc : 0 < c)
    ...
    ∑ Q ∈ ((Ideal.span {g}).comap
        ...
        affineDegree Q ≤
      (h * (2 * b * c - c ^ 2) + 2 * a * (j * c + r * (b - c)) : ℕ) := by
  refine (sum_affineDegree_minimalPrimes_comap_cappedBidegree_span_singleton_le ha hb hc hg0
    hg).trans_eq ?_
  rw [cappedBidegreeMixedVolume_eq hcb.le, mul_assoc h, Nat.mul_sub, sq, mul_comm c (2 * b)]
...
example {k : Type*} [Field k] {a b h j r : ℕ} (ha : 0 < a) (hb : 0 < b)
    ...
    affineDegree
      ...
      (h * b ^ 2 + 2 * j * a * b : ℕ) := by
  refine (affineDegree_comap_cappedBidegree_span_singleton_le ha hb hb hg0 hg).trans_eq ?_
  rw [cappedBidegreeMixedVolume_eq le_rfl, show 2 * b - b = b by omega, Nat.sub_self]
  push_cast
  ring

/-- For `b < c`, the bound is `h * b * c + 2 * a * j * c`. -/
example {k : Type*} [Field k] {a b c h j r : ℕ} (ha : 0 < a) (hb : 0 < b) (hbc : b < c)
    {g : MvPolynomial (Option (Fin 2)) k} (hg0 : g ≠ 0)
    (hg : g ∈ restrictCappedBidegree (Fin 2) k 1 h j r) :
    affineDegree
        ((Ideal.span {g}).comap (monomialMap k (cappedBidegreeExponents (Fin 2) 1 a b c))) ≤
      (h * b * c + 2 * a * j * c : ℕ) := by
  refine (affineDegree_comap_cappedBidegree_span_singleton_le ha hb (hb.trans hbc) hg0
    hg).trans_eq ?_
  rw [cappedBidegreeMixedVolume, cappedDegreeMixedVolume, cappedDegreeMixedVolume,
    Nat.sub_eq_zero_of_le hbc.le]
  push_cast
  ring
...
example : ¬affineDegree ((Ideal.span {(0 : MvPolynomial (Option (Fin 2)) ℚ)}).comap
    ...
    (cappedBidegreeMixedVolume 0 0 0 1 1 1 : ℕ) := by
  rw [not_le]
  refine lt_of_eq_of_lt (by simp [cappedBidegreeMixedVolume, cappedDegreeMixedVolume])
    (affineDegree_pos fun htop ↦ ?_)
...
/-- `affineDegree_comap_cappedBidegree_span_singleton_le` needs `0 < c`: for `c = 0`, the
hypersurface `X none = 0` has bounds `(1, 0, 0)` and the formula gives `0`, but its pullback is
proper and has positive affine degree. -/
example : ¬affineDegree ((Ideal.span {(X none : MvPolynomial (Option (Fin 2)) ℚ)}).comap
      (monomialMap ℚ (cappedBidegreeExponents (Fin 2) 1 1 1 0))) ≤
    (cappedBidegreeMixedVolume 1 0 0 1 1 0 : ℕ) := by
  rw [not_le]
  exact lt_of_eq_of_lt (by simp [cappedBidegreeMixedVolume, cappedDegreeMixedVolume])
    (affineDegree_pos (Ideal.comap_ne_top _ span_X_none_ne_top))
